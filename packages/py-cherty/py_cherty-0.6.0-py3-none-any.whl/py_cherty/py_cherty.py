...
class Cherty:
    def __init__(self, host='127.0.0.1', port=1337):
        ...
        if is_git_repo():
            git_info['commit_hash'] = get_commit_hash()
            git_info['diffs'] = get_git_diff()
            git_info['remote_url'] = get_remote_url()
            git_info['repo_name'] = get_repo_name(git_info['remote_url'])
            git_info['branch_name'] = get_current_branch()
            git_info['commit_message'] = get_last_commit_message()
            git_info['commit_author'] = get_last_commit_author()
            git_info['commit_date'] = get_last_commit_date()
