...
class DarwinAPI(foua.AnnotationAPI):
    ...
    def upload_annotations(self, samples, anno_key, backend):
        ...
        """
        logger.info("upload annotations")
        ...
        if external_storage and not Groups:
            logger.info("Registering External Storage Items")
        ...
        # Multislot external storage registration
            ...
            logger.info("Registering Grouped Datasets")
        ...
        # Multislot direct upload
            ...
            logger.info("Uploading Local Grouped Datasets")
        ...
        wait_until_items_finished_processing(
            dataset.dataset_id, dataset.team, backend.config.api_key
        )
    ...
    def _add_props_label_schema(self, backend, team_slug, label_schema):
        ...
        for field in label_fields:
            ...
            for cls in label_data["classes"]:
                ...
                logger.info(f"Properties added for class {cls} in label schema {field}")
        ...
        with (
            ...
            for label_field, label_info in label_schema.items():
                ...
                for annotation_path in export_path.glob("*.json"):
                    ...
                    # Video annotations
                    if is_darwin_video:
                        logger.info("Processing video annotations")
                        ...
                        for annotation in video_annotations:
                            ...
                            if "polygon" in frame_annotation and label_type in [
                                ...
                                if len(frame_annotation["polygon"]["paths"]) == 1:
                                    logger.info("Processing simple polygons")
                                    ...
                                    logger.info("Processing complex polygons")
                                    voxel_annotation = _v7_to_51_polygon(
                                        annot_name,
                                        frame_annotation["polygon"],
                                        height,
                    ...
                    # Image annotations
                        ...
                        logger.info("Processing image annotations")
...
def _multislot_registration(
    ...
    Only readwrite currently supported
    """
    logger.info("Multi slot item registration started")
    ...
    for k in groups:
        ...
        for chunk in _chunk_list(payload["items"], 10):
            ...
            while True:
                ...
                if response.ok:
                    logger.info(f"Multi slot item registration complete for {name}")
                    break
                elif response.status_code == 429:
                    logger.warning(
                        f"Rate limit exceeded. Retrying in {backoff} seconds..."
...
def _upload_multislot_items(groups, api_key, dataset_slug, team_slug, base_url):
    ...
    for k in groups:
        ...
        if response.ok:
            logger.info(f"Multi slot item upload complete for group {group_id}")
        else:
            logger.error(
...
def wait_until_items_finished_processing(dataset_id, team_slug, api_key):
    """
