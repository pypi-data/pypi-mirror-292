...
class ApiClient(object):
    ...
    STATUS_CODES_MAPPING: typing.Dict[int, str] = {
        200: "OK",
    ...
    def request_with_retry(self,
        ...
        logger.info(
            f"Request Status {response_data.status} ({self.STATUS_CODES_MAPPING.get(response_data.status)}) Retrying..."
        )
        # Query retry support
        result_endpoint = response_data.getheader('Location')
        if response_data.status == 202 and result_endpoint is None:
            raise InvalidResponseError(
                f"{self.STATUS_CODES_MAPPING.get(response_data.status)} result endpoint is missing"
            )

        if _request_timeout is None:
            _request_timeout = self.DEFAULT_RETRY_TIMEOUT_SECONDS
        ...
        while True:
            ...
            if response_data.status in rest.RETRY_STATUS_CODES:
                logger.info(
                    f"{self.STATUS_CODES_MAPPING.get(response_data.status)} detected. Sleeping for {wait_time} seconds before checking again"
                )
                ...
                logger.info(
                    f"{self.STATUS_CODES_MAPPING.get(response_data.status)} query completed"
                )
                return response_data
        ...
        raise RetryTimeoutError(
            f"{response_data.status} {self.STATUS_CODES_MAPPING.get(response_data.status)} query timeout"
        )
