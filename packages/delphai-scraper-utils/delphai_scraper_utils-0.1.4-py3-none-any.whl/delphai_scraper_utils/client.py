...
import ssl
import time
from typing import Any, Callable, Iterable, Iterator, List, Mapping, Union
...
class ScraperClient(AsyncClient):
        ...
        if response.status_code == 200:
            try:
                await asyncio.get_running_loop().run_in_executor(
                    None,
                    robot_file_parser.parse,
                    self.take_until_timeout(response.iter_lines(), timeout=5),
                )
            except TimeoutError:
    ...
    def take_until_timeout(self, items: Iterable[str], timeout: float) -> Iterator[str]:
