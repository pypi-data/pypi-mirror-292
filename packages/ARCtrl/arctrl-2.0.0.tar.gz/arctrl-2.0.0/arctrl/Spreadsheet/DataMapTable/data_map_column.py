...
def to_fs_columns(dc: Array[DataContext]) -> FSharpList[FSharpList[FsCell]]:
    ...
    def _arrow1149(__unit: None=None, dc: Any=dc) -> IEnumerable_1[FSharpList[FsCell]]:
        def _arrow1148(__unit: None=None) -> IEnumerable_1[FSharpList[FsCell]]:
            def _arrow1147(dc_4: DataContext) -> FSharpList[FsCell]:
                dc_3: DataContext = dc_4
                def _arrow1145(__unit: None=None) -> IEnumerable_1[FsCell]:
                    def _arrow1137(__unit: None=None) -> FSharpList[FsCell]:
                    ...
                    def _arrow1144(__unit: None=None) -> IEnumerable_1[FsCell]:
                            ...
                            def _arrow1142(__unit: None=None) -> IEnumerable_1[FsCell]:
                                def _arrow1141(__unit: None=None) -> IEnumerable_1[FsCell]:
                                    def _arrow1140(__unit: None=None) -> IEnumerable_1[FsCell]:
                                        ...
                                        return append(create_text(DataContext__get_GeneratedBy(dc_3)), delay(_arrow1139))

                                    return append(create_text(DataContext__get_Description(dc_3)), delay(_arrow1140))

                                return append(create_term(DataContext__get_ObjectType(dc_3)), delay(_arrow1141))

                            return append(create_term(DataContext__get_Unit(dc_3)), delay(_arrow1142))

                        return append(create_term(DataContext__get_Explication(dc_3)), delay(_arrow1143))

                    return append(_arrow1137(), delay(_arrow1144))

                return to_list(delay(_arrow1145))

            return map_1(_arrow1147, dc)

        return append(singleton_1(headers), delay(_arrow1148))

    return transpose(to_list(delay(_arrow1149)))
