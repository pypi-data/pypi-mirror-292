...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow2024(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2023(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2021(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2020(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow2020))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow2021))

            return append(singleton("{"), delay(_arrow2023))

        return join("\n", to_list(delay(_arrow2024)))
...
ValidationPackage_reflection = _expr2031
