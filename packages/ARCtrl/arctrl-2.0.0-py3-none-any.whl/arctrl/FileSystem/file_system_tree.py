...
class FileSystemTree(Union):
    ...
    def add_file(path: str) -> Callable[[FileSystemTree], FileSystemTree]:
        def _arrow374(tree: FileSystemTree) -> FileSystemTree:
    ...
    def from_file_paths(paths: Array[str]) -> FileSystemTree:
        def loop(paths_1: Array[Array[str]], parent: FileSystemTree) -> FileSystemTree:
            ...
            class ObjectExpr379:
                ...
                def Equals(self) -> Callable[[str, str], bool]:
                    def _arrow378(x_3: str, y_2: str) -> bool:
                        return x_3 == y_2
            ...
            folders: Array[FileSystemTree] = map(mapping_2, Array_groupBy(projection, filter(predicate_1, paths_1), ObjectExpr379()), None)
            if parent.tag == 1:
                def _arrow383(__unit: None=None, paths_1: Any=paths_1, parent: Any=parent) -> IEnumerable_1[FileSystemTree]:
                    def _arrow382(__unit: None=None) -> IEnumerable_1[FileSystemTree]:
                        return folders

                    return append(files, delay(_arrow382))

                return FileSystemTree.create_folder(parent.fields[0], to_array(delay(_arrow383)))
        ...
        class ObjectExpr386:
            ...
            def Equals(self) -> Callable[[Array[str], Array[str]], bool]:
                def _arrow385(x: Array[str], y: Array[str]) -> bool:
                    def _arrow384(x_1: str, y_1: str) -> bool:
                        return x_1 == y_1

                    return equals_with(_arrow384, x, y)
        ...
        return loop(Array_distinct(map(split, paths, None), ObjectExpr386()), FileSystemTree.create_folder(FileSystemTree.ROOT_NAME(), []))
