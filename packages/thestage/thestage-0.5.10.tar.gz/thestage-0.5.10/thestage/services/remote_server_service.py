...
class RemoteServerService(object):
    ...
    def __read_remote_path_items_new(
        ...
        try:
            ...
            app_logger.exception(f"Can not read remote list files: {ex}")
            typer.echo(__('Not found source path, check command params'))
            typer.Exit(1)
        except Exception as ex:
            app_logger.exception(f"Error on code: {ex}")
            typer.echo(__('Error on file '))
            typer.Exit(1)
