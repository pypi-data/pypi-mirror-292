...
class PAModel(Model):
    ...
    def __init__(self, id_or_model: Union[str, "Model", None] = None,
        ...
        self.membrane_sector = membrane_sector
    ...
    def make_lb_ub_constraint(
        ...
        if f"{rxn.id}_lb" in m.constraints.keys():
            ...
            ub_constraint = m.problem.Constraint(
                Zero, name=f"{rxn.id}_ub", ub=upper_bound
            )
            lb_constraint = m.problem.Constraint(
                Zero, name=f"{rxn.id}_lb", ub=-lower_bound
            )
    ...
    def add_membrane_constraint(self):
        coefficients = {
            self.reactions.get_by_id(self.BIOMASS_REACTION).forward_variable: -self.membrane_sector.slope
        }
        # variables = [self.reactions.get_by_id(self.BIOMASS_REACTION).forward_variable]
        for enz_complex in self.enzyme_variables:
            enzymes = enz_complex.id.split("_")
            alpha_numbers_in_complex = [0] # zero if enzyme is not in membrane

            #  and self.membrane_sector.cog_class[enz][0] in ['C', 'P', 'G', 'E', 'I', 'H', 'F']
            for enz in enzymes:
                if enz in self.membrane_sector.alpha_numbers_dict.keys() and self.membrane_sector.enzyme_location[enz] == 'Cell membrane':
                        alpha_numbers_in_complex.append(self.membrane_sector.alpha_numbers_dict[enz])

            alpha_numbers_for_complex = max(alpha_numbers_in_complex)

            coefficients[enz_complex.forward_variable] = (1e-6 * alpha_numbers_for_complex #correction for the solver issue
                                                         * self.membrane_sector.area_alpha
                                                         * self.membrane_sector.unit_factor
                                                         / self.membrane_sector.max_membrane_area)
            # variables.append(enz_complex.forward_variable)

            coefficients[enz_complex.reverse_variable] = (1e-6 * alpha_numbers_for_complex # correction for the solver issue
                                                         * self.membrane_sector.area_alpha
                                                         * self.membrane_sector.unit_factor
                                                         / self.membrane_sector.max_membrane_area)
            # variables.append(enz_complex.reverse_variable)

        occupied_membrane = self.problem.Constraint(0, lb=0, ub=self.membrane_sector.intercept, name='membrane')
        self.add_cons_vars(occupied_membrane)
        self.solver.update()
        occupied_membrane.set_linear_coefficients(coefficients=coefficients)

        # # Debug: Print coefficients after setting them to verify
        # print(f"Set coefficients for membrane constraint: {coefficients}")
        #

        #print('Stored linear coefficients: ', self.constraints['membrane'].get_linear_coefficients(variables))
    ...
    def calculate_occupied_membrane(self):
        ...
        for enz_complex in self.enzyme_variables:
            ...
            alpha_numbers_for_complex = max(alpha_numbers_in_complex)
            occupied_membrane += (enz_complex_concentration * 1e-6 * alpha_numbers_for_complex #correction for the solver issue
                                * self.membrane_sector.area_alpha
                                * self.membrane_sector.unit_factor)

        # for enz in self.enzyme_variables:
        #     if (enz.id in self.membrane_sector.alpha_numbers_dict.keys()
        #             and self.membrane_sector.enzyme_location[enz.id] == 'Cell membrane'):
        #         enz_concentration = (enz.forward_variable.primal + enz.reverse_variable.primal) * 1e-6
        #         occupied_membrane += (enz_concentration
        #                               * self.membrane_sector.alpha_numbers_dict[enz.id]
        #                               * self.membrane_sector.area_alpha
        #                               * self.membrane_sector.unit_factor*0.57)

        available_membrane = self.membrane_sector.slope * self.objective.value + self.membrane_sector.intercept
