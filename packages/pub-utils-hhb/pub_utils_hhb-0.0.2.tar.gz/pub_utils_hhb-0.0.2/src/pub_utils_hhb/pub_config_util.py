...
from pub_utils_hhb.pub_valid_util import validate_kwargs
...
class JsonFile:
    ...
    def load(self):
        try:
            ...
            if self._ignore_file_not_found:
                self._content = {}
                self._logger.warning(f'load json file failed: {self._path}, ignore: file not found')
                return
    ...
    def _init_default_logger():
        ...
        logger = logger_manager.get_logger(logger_name=__class__.__name__)
...
class ConfigFileManager:
    ...
    expected_args = [
        Arg().set_arg_name('path').set_arg_type(str).set_required(True),
        Arg().set_arg_name('read_only').set_arg_type(bool).set_default_value(False),
        Arg().set_arg_name('ignore_file_not_found').set_arg_type(bool).set_default_value(False),
        Arg().set_arg_name('logger').set_arg_type(Logger).set_default_value(None),
    ]
    ...
    def get_config_file(cls, **kwargs) -> ConfigFile:
        ...
        """
        validate_kwargs(expected=ConfigFileManager.expected_args, actual=kwargs)
