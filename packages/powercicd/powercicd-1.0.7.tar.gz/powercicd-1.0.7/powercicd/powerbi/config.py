...
class PowerBiComponentConfig(ComponentConfig):
    ...
    show_pages           : Annotated[List[str]                        , Field(default_factory=lambda: [], description="The list of pages to show in the report (show is applied before hide). `*` can be as a wildcard")]
    hide_pages           : Annotated[List[str]                        , Field(default_factory=lambda: [], description="The list of pages to hide in the report (hide is applied after show). `*` can be as a wildcard")]
