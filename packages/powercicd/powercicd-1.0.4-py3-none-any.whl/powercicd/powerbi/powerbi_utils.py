...
def convert_src_code_to_original_layout(
    ...
    for section_match in parse("$.sections[*]").find(layout):
        ...
        matching_show      = [pattern for pattern in show_pages if fnmatch.fnmatch(section_name, pattern)]
        matching_hide      = [pattern for pattern in hide_pages if fnmatch.fnmatch(section_name, pattern)]
        new_visibility     = 1 if len(matching_hide) > 0 else 0 if len(matching_show) > 0 else old_visibility
        if new_visibility == 1:  # hidden
            log.info(f"Hidding section '{section_name}' (show rules: {matching_show}, hide rules: {matching_hide})")
            section_value["config"]["visibility"] = new_visibility
        if new_visibility == 0:
            log.info(f"Showing section '{section_name}' visible (show rules: {matching_show}, hide rules: {matching_hide})")
            section_value["config"]["visibility"] = new_visibility
