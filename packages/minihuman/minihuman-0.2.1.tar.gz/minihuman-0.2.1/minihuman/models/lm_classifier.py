...
class Experiment_Pipeline():
    def __init__(self, max_length, log_folder, load_model_type, num_labels, random_seed = 4, lr=1e-4):
        self.max_length = max_length
        ...
        self.model_init(load_model_type,num_labels,lr)
    ...
    def help(self):
        help_str = '''
            def run_exp_bertKT():
                ...
                experiment_pipeline = Experiment_Pipeline(512,log_folder,load_model_type='none',num_labels=3,lr=5e-7)
                ...
                experiment_pipeline = Experiment_Pipeline(512,log_folder,load_model_type='best',num_labels=3,lr=5e-7)
                experiment_pipeline.dataset_prepare(dataframe_train,dataframe_valid,dataframe_test,input_column_list = ['past_question_answer_prompt','future_question_prompt'],label_column = 'correctness',trim_type = 'longest_first',batch_size = 16)
                experiment_pipeline.model_eval(eval_mode='test')
    ...
    def model_init(self,load_model_type,num_labels,lr=1e-4):
        ...
        if load_model_type in ['best','last']:
            ...
            checkpoint = torch.load(self.checkpoint_last_path,map_location=self.device) if load_model_type == 'last' else torch.load(self.checkpoint_best_path,map_location=self.device)
            ...
            self.model = BertForSequenceClassification.from_pretrained('bert-base-uncased', num_labels=num_labels).to(self.device)
            self.optimizer = optim.AdamW(self.model.parameters(), lr=lr)
            self.epoch_exist = 0
            self.train_losses = []
            self.val_losses = []
