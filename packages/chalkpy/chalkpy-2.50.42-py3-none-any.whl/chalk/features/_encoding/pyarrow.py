...
from typing import TYPE_CHECKING, Any, Dict, FrozenSet, List, Set, Tuple, Type, cast
...
def rich_to_pyarrow(python_type: Type, name: str, in_struct: bool = False) -> pa.DataType:
    """
    ...
    if origin is not None:
        ...
        if origin in (list, List, set, Set, frozenset, FrozenSet):
            ...
            arg = cast("tuple[Any, ...]", args)[0]  # pyright: ignore[reportUnnecessaryCast]
        ...
        if origin in (dict, Dict):
            ...
            try:
                pa_value_type = rich_to_pyarrow(value_type, "value")
                ...
                return pa.map_(key_type=rich_to_pyarrow(key_type, "key"), item_type=value_field)
    ...
    if _is_features_cls(python_type):
        ...
        for field_name, type_annotation in annotations.items():
            if _is_features_cls(type_annotation) or _is_chalk_dataframe(type_annotation):
                continue
    ...
    if _is_chalk_dataframe(python_type):
        if python_type.__pydantic_model__ is None:
            ...
            return pa.large_list(rich_to_pyarrow(python_type=python_type.__pydantic_model__, name="item"))
...
def pyarrow_to_primitive(pyarrow_typ: pa.DataType, name: str) -> Type[TPrimitive]:
    ...
    if pa.types.is_map(pyarrow_typ):
