...
async def scrape(sdk: SDK, url: str, *args: Any, **kwargs: Any) -> None:
    page: Page = sdk.page
    while True:
        await sdk.page.wait_for_selector("tbody.ui-datatable-data.ui-widget-content")
        links = await sdk.page.query_selector_all(
            "tbody.ui-datatable-data.ui-widget-content > tr > td:nth-child(1) a"
        )

        for link in links:
            await sdk.enqueue(await link.get_attribute("href"))

        try:
            next_button = await sdk.page.query_selector(
                ".ui-paginator-next:not(.ui-state-disabled) > .ui-icon-seek-next"
            )
...
if __name__ == "__main__":
    ...
    base_url = "https://epro.sbcounty.gov/bso/view/search/external/advancedSearchBid.xhtml?openBids=true"
    metadata = {
        "category": "government",
    ...
    asyncio.run(
        create_end2end_examples(
            base_url, metadata, scrape, None, "bananalyzer-examples"
        )
    )
