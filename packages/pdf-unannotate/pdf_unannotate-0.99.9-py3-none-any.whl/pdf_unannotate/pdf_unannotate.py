...
def _unannotate_pdfs(files: list[str], suffix: str, remove_suffix: str) -> None:
	...
	for file in tqdm(files):
		...
		outfile = re.sub(fr'{remove_suffix}\.pdf$', '.pdf', file)
		outfile = re.sub(r'\.pdf$', f'{suffix}.pdf', outfile)
...
def unannotate_pdfs(s) -> None:
	...
	remove_suffix = [arg.split('=')[1] for arg in s[:-1] if arg.startswith('remove-suffix=')]
	remove_suffix = remove_suffix[0] if remove_suffix else ''
	...
	# remove any duplicate files (i.e., that might match more than one glob)
	globbed = list(dict.fromkeys(globbed))
	
	try:
		_unannotate_pdfs(files=globbed, suffix=suffix, remove_suffix=remove_suffix)
