...
class BaseSettings:
    schema_path: str = ""
    url: str = os.environ.get(ALPHASENSE_API_URL) or "https://api.alpha-sense.com"
    api_endpoint: str = os.environ.get(ALPHASENSE_API_ENDPOINT) or "/gql"
    schema_endpoint: str = (
        os.environ.get(ALPHASENSE_GRAPHQL_SCHEMA_ENDPOINT) or "/gql/client-sdk/schema"
    )
    ...
    def __post_init__(self):
        ...
        if (
            self.remote_schema_headers.get(ALPHASENSE_HEADER_X_API_KEY) is None
            and self.api_key != ""
        ):
            self.remote_schema_headers[ALPHASENSE_HEADER_X_API_KEY] = (
                os.environ.get("ALPHASENSE_API_KEY") or self.api_key
            )

    @property
    def remote_schema_url(self):
        return f"{self.url}{self.schema_endpoint}"
...
class ClientSettings(BaseSettings):
    ...
    def __post_init__(self):
        ...
        assert_path_exists(self.auth_path)
        assert_path_is_valid_file(self.auth_path)
    ...
    def _set_default_base_client_data(self):
        ...
        if not self.auth_path:
            self.auth_path = DEFAULT_AUTH_PATH.as_posix()

        if not self.client_config_path:
            self.client_config_path = DEFAULT_CLIENT_CONFIG_PATH.as_posix()
    ...
    def used_settings_message(self) -> str:
        ...
        return dedent(
            ...
            Coping auth from '{self.auth_path}'.
...
class AuthSettings(BaseSettings):
    ...
    def __post_init__(self):
        ...
        if (
            self.remote_schema_url
            and self.remote_schema_headers.get(ALPHASENSE_HEADER_X_API_KEY) is None
        ):
            self.remote_schema_headers[ALPHASENSE_HEADER_X_API_KEY] = (
                os.environ.get(ALPHASENSE_API_KEY) or self.api_key
            )
