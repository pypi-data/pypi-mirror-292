...
from rekuest_next.structures.registry import StructureRegistry, get_current_structure_registry

from rekuest_next.state.registry import StateRegistry, get_default_state_registry, get_current_state_registry
...
def inspect_state_schema(cls: Type[T], structure_registry: StructureRegistry) -> Optional[StateSchemaInput]:
    ...
    return StateSchemaInput(
        ports=ports,
        name=cls.__rekuest_state__
    )
...
def state(name: str, registry: Optional[StateRegistry] = None, structure_reg: Optional[StructureRegistry] = None) -> Callable[[Type[T]], Type[T]]:
