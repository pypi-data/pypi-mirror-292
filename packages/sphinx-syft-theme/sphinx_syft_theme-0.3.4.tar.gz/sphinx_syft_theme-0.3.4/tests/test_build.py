"""All the tests performed in the sphinx-syft-theme test suit."""
...
from sphinx_syft_theme.utils import escape_ansi
...
@pytest.mark.parametrize(
    ...
    (
        [dict(), "Syft Tests  documentation - Home"],
...
def test_logo_no_image(sphinx_build_factory) -> None:
    ...
    sphinx_build = sphinx_build_factory("base", confoverrides=confoverrides).build()
    index_html = sphinx_build.html_tree("index.html")
    assert "Syft Tests" in index_html.select(".navbar-brand")[0].text.strip()
...
def test_logo_external_image(sphinx_build_factory) -> None:
    ...
    test_url = "https://callezenwaka.org/wp-content/uploads/2019/06/callezenwaka-logo-final.png"
    confoverrides = {
        "html_theme_options": {
            "logo": {
                "image_dark": test_url,
...
def test_navbar_no_in_page_headers(sphinx_build_factory, file_regression) -> None:
    ...
    # https://github.com/callezenwaka/sphinx-syft-theme/issues/302
    sphinx_build = sphinx_build_factory("test_navbar_no_in_page_headers").build()
...
def test_included_toc(sphinx_build_factory) -> None:
    ...
    # https://github.com/callezenwaka/sphinx-syft-theme/pull/347
...
def test_footer(sphinx_build_factory) -> None:
    ...
    assert "Syft community" in footer_sta.text
