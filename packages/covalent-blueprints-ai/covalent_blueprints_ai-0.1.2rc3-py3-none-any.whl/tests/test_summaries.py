...
class TestSummaries:
    ...
    def setup_class(cls):
        ...
        cls.blueprint_objs = {
            name: bp() for name, bp in cls.blueprint_initializers.items()
        }
    ...
    def test_args_tuple_is_empty(self):
        ...
        # TODO: positional arguments should be eliminated in a future version
        for bp_name, bp in self.blueprint_objs.items():
            msg = f"Blueprint '{bp_name}' has a non-empty args tuple."
            assert bp.inputs.args == (), msg
    ...
    def test_every_kwarg_documented(self):
        ...
        for bp_name, bp in self.blueprint_objs.items():
            for kwarg in bp.inputs.kwargs:
                assert kwarg in bp.inputs.docs, f"Missing doc for '{kwarg}' in '{bp_name}'"
    ...
    def test_every_doc_matches_kwarg(self):
        ...
        for bp_name, bp in self.blueprint_objs.items():
            ...
            for kwarg_name in docs_:
                if kwarg_name in bp.inputs.kwargs:
                    missing.append(kwarg_name)

            for kwarg in missing:
                msg = f"Documented kwarg '{kwarg}' for '{bp_name}' does not exist in `.inputs.kwargs`"
                assert kwarg in bp.inputs.kwargs, msg

    def test_doc_keys_match_argument_type(self):
        """Check that every input is documented with '*' or '**' in name if required."""
        for bp_name, bp in self.blueprint_objs.items():
            ...
            for param in bp_initializer_params.values():
                ...
                assert _param in bp.inputs.docs, f"Missing doc for '{_param}' in '{bp_name}'"
