...
class Preparer:
	...
	def Prepare(self, applicationExecuteType : ApplicationExecuteType) -> None:
		...
		cachePath : str = os.path.join(temporaryPath, "dduk-python", "application", "projects", rootName).replace(BACKSLASH, SLASH)
		...
		settings : dict = self.GetSettingsFromVSCodeSettingFile(vscodeSettingsFilePath, preparedData.Type)
		if settings:
			# 심볼 목록.
			...
			if SYMBOLS in settings:
				...
				for symbol in symbols:
					symbol = symbol.upper()
					if symbol in preparedData.Symbols:
						continue
					preparedData.Symbols.append(symbol)
			# 인자 목록.
	...
	#--------------------------------------------------------------------------------
	def GetSettingsFromVSCodeSettingFile(self, vscodeSettingsFilePath : str, applicationExecuteType : ApplicationExecuteType) -> dict:
		try:
