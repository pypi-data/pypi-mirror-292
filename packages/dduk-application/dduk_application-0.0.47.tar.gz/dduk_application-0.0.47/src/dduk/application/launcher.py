...
EMPTY : str = ""
FROZEN : str = "frozen"
PROJECT_MAIN_MODULE_NAME : str = "__main__"
PROJECT_MAIN_FUNCTION_NAME : str = "Main" # def Main(arguments : list[str]) -> int: rerturn 0
BACKSLASH : str = "\\"
SLASH : str = "/"
UTF8 : str = "utf-8"
READ : str = "r"
...
class Launcher:
	...
	def Launch(self) -> int:
		...
		if internalData.IsBuild():
			...
			currentFilePath = os.path.abspath(sys.modules[PROJECT_MAIN_MODULE_NAME].__file__).replace(BACKSLASH, SLASH)
		...
		YAMLWrapper.SetConstructor("!!python/object:dduk.application.prepareddata.PreparedData", YAMLWrapper.PreparedDataConstructor)
		...
		preparedData : PreparedData = YAMLWrapper.LoadYAMLFromFile(prepareFilePath)
		InternalRepository.Link(preparedData)

		#--------------------------------------------------------------------------------
		# 로거 생성.
		logger = Logger(preparedData.Name)
		...
		internalData.SetSymbols(preparedData.Symbols)
		...
		if preparedData.Type == ApplicationExecuteType.BUILD:
			builtins.print("__BUILD__")
			logType = LogType.ERROR
		# 서비스.
		elif preparedData.Type == ApplicationExecuteType.SERVICE:
			builtins.print("__SERVICE__")
			logType = LogType.INFO
		# 소스.
		elif preparedData.Type == ApplicationExecuteType.SOURCE:
		...
		if sourcePath not in sys.path: sys.path.append(sourcePath)
		...
		try:
			...
			mainModuleFilePath : str = Application.GetSourcePathWithRelativePath(f"{PROJECT_MAIN_MODULE_NAME}.py")
			if not os.path.isfile(mainModuleFilePath):
				raise FileNotFoundError(mainModuleFilePath)
			mainModuleSpecification = importlib.util.spec_from_file_location(PROJECT_MAIN_MODULE_NAME, mainModuleFilePath)
			mainModule = importlib.util.module_from_spec(mainModuleSpecification)
			mainModuleSpecification.loader.exec_module(mainModule)
			if not hasattr(mainModule, PROJECT_MAIN_FUNCTION_NAME):
				raise AttributeError(f"\"{PROJECT_MAIN_FUNCTION_NAME}\" function not found in {mainModuleFilePath}")
			mainFunction = builtins.getattr(mainModule, PROJECT_MAIN_FUNCTION_NAME)
			exitCode : int = mainFunction(sys.argv)
			return exitCode				
