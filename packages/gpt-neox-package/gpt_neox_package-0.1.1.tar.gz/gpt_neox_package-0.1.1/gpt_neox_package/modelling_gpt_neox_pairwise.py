...
from gpt_neox_package.configuration_gpt_neox import GPTNeoXConfig
...
class GPTNeoXPreTrainedModel(PreTrainedModel):
    ...
    base_model_prefix = "gpt_neox_sym"
...
class GPTNeoXAttention_Pairwise(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.num_attention_heads = config.num_attention_heads
        ...
        self.norm_factor = self.head_size**-0.5
        # change to 2 times config
        self.query_key_value = nn.Linear(config.hidden_size, 2 * config.hidden_size, bias=config.attention_bias)
        self.dense = nn.Linear(config.hidden_size, config.hidden_size, bias=config.attention_bias)
        self.attention_dropout = nn.Dropout(config.attention_dropout)
        self.is_causal = True

        # Added pairwise factors initialization
        self.pairwise_factors = nn.Parameter(torch.randn((self.num_attention_heads, self.head_size, self.head_size)))
    ...
    def forward(
        ...
        qkv = self.query_key_value(hidden_states)
        # change for 2
        new_qkv_shape = qkv.size()[:-1] + (self.num_attention_heads, 2 * self.head_size)
        qkv = qkv.view(*new_qkv_shape)

        query = qkv[..., : self.head_size].permute(0, 2, 1, 3) 
        key = query 
        # key = qkv[..., self.head_size : 2 * self.head_size].permute(0, 2, 1, 3)
        value = qkv[..., 1 * self.head_size :].permute(0, 2, 1, 3)
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        query = query.view(batch_size * num_attention_heads, query_length, attn_head_size)
        key = key.view(batch_size * num_attention_heads, key_length, attn_head_size)
        pf_expanded =self.pairwise_factors.unsqueeze(0).expand(batch_size, -1, -1, -1).contiguous().view(batch_size * num_attention_heads, attn_head_size, attn_head_size) 
        ...
        attn_scores = torch.baddbmm(
            attn_scores,
            torch.bmm(query, pf_expanded),
            key.transpose(1, 2),
            beta=1.0,
            alpha=self.norm_factor,
        )
        # check matrix mult (interm. matrix)
        # Modified attention score calculation with pairwise factors (TO CHECK)
        # attn_scores = (query @ self.pairwise_factors @ key.transpose(1, 2)) * self.norm_factor
        attn_scores = attn_scores.view(batch_size, num_attention_heads, query_length, key_length)
...
class GPTNeoXLayer(nn.Module):
    def __init__(self, config):
        ...
        if config.plpi_affinity_function == "pairwise":
            self.attention = GPTNeoXAttention_Pairwise(config)
        else:
