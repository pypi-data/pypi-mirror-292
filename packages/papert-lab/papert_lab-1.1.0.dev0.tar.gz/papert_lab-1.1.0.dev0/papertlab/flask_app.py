...
def initialize_coder():
    ...
    try:
        ...
        # Ensure main_model is set
        if coder.main_model is None:
            coder.main_model = models.Model(DEFAULT_MODEL)

        current_model = coder.main_model.name
        ...
        try:
            ...
            try:
                ...
                if command == 'code':
                    ...
                    for output in autopilot_coder.run_autopilot(message):
                        yield f"data: {json.dumps({'chunk': output})}\n\n"
                    temp_coder = autopilot_coder
                    result = "Autopilot execution completed."
                else:
