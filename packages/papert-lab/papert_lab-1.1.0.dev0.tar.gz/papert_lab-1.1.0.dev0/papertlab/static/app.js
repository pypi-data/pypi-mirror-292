...
const App = () => {
    ...
    const handleEditorClose = async () => {
        if (selectedFile && editorRef.current && editorRef.current.editor) {
            const content = editorRef.current.editor.getValue();
            await handleEditorSave(selectedFile, content);
        }
        setSelectedFile(null);
    };

    const handleEditorSave = async (file, content) => {
        try {
            const response = await fetch('/api/update_file', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            const data = await response.json();
            console.log(data.message);
            setFileContent(content);  // Update the file content in the state
            // Optionally, update the file tree or show a success message
        } catch (error) {
            console.error('Error saving file:', error);
