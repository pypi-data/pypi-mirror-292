...
def remove_null_bytes_deep(obj: JSONType) -> JSONType:
    if isinstance(obj, dict):
        return {
            (remove_null_bytes(key) if isinstance(key, str) else key): (
                remove_null_bytes_deep(value)
            )
            for key, value in obj.items()
        }

    if isinstance(obj, (list, set, tuple)):
        return type(obj)([remove_null_bytes_deep(element) for element in obj])
