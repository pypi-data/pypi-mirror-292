...
import numpy as np
from sasktran.tir.climatology import (
    ClimatologySpeciesCustomPT,
    ClimatologySpecies,
    ClimatologyAtmosphericState,
    ClimatologyFull,
)
...
class TestClimatologyTIR(unittest.TestCase):
    ...
    def test_atmospheric_state(self):
        ...
        pressure = climatology.get_parameter(
            "SKCLIMATOLOGY_PRESSURE_PA", 0, 0, test_altitudes, 54372
        )
        temperature = climatology.get_parameter(
            "SKCLIMATOLOGY_TEMPERATURE_K", 0, 0, test_altitudes, 54372
        )

        np.testing.assert_array_almost_equal_nulp(
            pressure, np.array([26500.0, 1197.0, 79.78]), nulp=100
        )
        np.testing.assert_array_almost_equal_nulp(
            temperature, np.array([223.3, 226.5, 270.7]), nulp=100
        )
    ...
    def test_climatology_species(self):
        ...
        co2_dens = climatology.get_parameter(
            "SKCLIMATOLOGY_CO2_CM3", 0, 0, test_altitudes, 54372
        )

        np.testing.assert_array_almost_equal_nulp(
            co2_dens,
            np.array([2836532788301347.5, 126315487132049.84, 7044277132027.684]),
            nulp=100,
        )
    ...
    def test_climatology_custom_pt(self):
        ...
        climatology = ClimatologySpeciesCustomPT(
            "CO2", atmospheric_state, latitude=0, longitude=0, mjd=54372
        )
        ...
        np.testing.assert_array_almost_equal_nulp(
            co2_dens,
            np.array([2856239131629593.0, 124356334698626.53, 7641532126161.376]),
            nulp=100,
        )
    ...
    def test_fascode_dataset(self):
        ...
        for clim in climatology_ids:
            for species in species_major + species_minor:
                climatology = ClimatologySpecies(
                    species, dataset=dataset, climatology=clim
                )
                self._basic_access_tests(
                    "SKCLIMATOLOGY_" + species + "_CM3", climatology
                )
