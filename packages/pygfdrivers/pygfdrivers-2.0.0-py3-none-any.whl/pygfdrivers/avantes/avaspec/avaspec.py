...
if 'linux' in sys.platform:  # Linux will have 'linux' or 'linux2'
    ...
    if ctypes.sizeof(ctypes.c_void_p) == 8:  # 64 bit
        ...
        lib_path = os.path.join(driver_dir, 'avaspecx64.dll')
        lib = ctypes.WinDLL(lib_path)
        func = ctypes.WINFUNCTYPE
    else:
        WM_MEAS_READY = 0x0401
        lib_path = os.path.join(driver_dir, 'avaspec.dll')
        lib = ctypes.WinDLL(lib_path)
        func = ctypes.WINFUNCTYPE
...
def AVS_MeasureCallback(handle, cb, nummeas):
    ...
    _AVS_MeasureCallback = prototype(("AVS_MeasureCallback", lib), paramflags)

    ret = _AVS_MeasureCallback(handle, cb.callback, nummeas)
    if ret != 0:
        raise OSError(f"AVS_MeasureCallback failed with error code {ret}")
    return ret
...
def AVS_SetDstrStatusCallback(handle, cb):
    ...
    _AVS_SetDstrStatusCallback = prototype(("AVS_SetDstrStatusCallback", lib), paramflags)

    ret = _AVS_SetDstrStatusCallback(handle, cb.callback)
    if ret != 0:
        raise OSError(f"AVS_SetDstrStatusCallback failed with error code {ret}")
    return ret
