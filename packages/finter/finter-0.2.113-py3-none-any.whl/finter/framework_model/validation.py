...
class ValidationHelper(IValidation):
    ...
    def __init__(self, model_path=None, model_info=None):
        ...
        self.lmit_secs = FREQ_TO_DURATION_SECS[model_info["freq"]]
        ...
        assert re.match(
            r"^[a-zA-Z0-9_]+$", model_name
        ), f"Invalid model name: {model_name}, No space or dot in model name"
    ...
    def validate(self):
        ...
        self.validate_combine_getter(self.end)

        # self.validate_start_dependency()
        # self.validate_end_dependency()

    def validate_combine_getter(self, end):
        try:

            # Validate index
            print("start: %s" % self.days_before(end, 90))
            start_time = datetime.now()
            test_position = self.getter(
                self.__MODEL,
                self.days_before(end, 90, self.exchange),
                self.days_before(end, 1, self.exchange),
            )
            ...
            expected_idx = pd.DatetimeIndex(excpected_idx)
