        ...
        with Session(self.engine) as session:
            for text in texts:
                try:
                    ...
                    node = SQLModelNode(text=text, embedding=embedding_vector, metadata=metadata or {})
                    session.add(node)
                except Exception as e:
    ...
    def search(self, query: str, top_k: int = 5) -> SearchResponse:
        try:
            ...
            with Session(self.engine) as session:
                ...
                statement = select(SQLModelNode).order_by(
                    sum(SQLModelNode.embedding[i] * query_vector[i] for i in range(len(query_vector))).desc()
                ).limit(top_k)
