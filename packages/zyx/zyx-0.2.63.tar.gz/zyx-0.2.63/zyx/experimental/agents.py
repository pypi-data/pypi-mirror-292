...
from zyx.core.ext import BaseModel
...
def _init_llm(
    ...
    try:
        if model.startswith("ollama/") or model.startswith("ollama-chat/"):
            ...
            # Ensure the model_name is set
            if not hasattr(llm, 'model_name') or llm.model_name is None:
                llm.model_name = model.split('/')[-1]  # Use the last part of the model string
...
class Agents:
    ...
    def __init__(
        ...
        # Select Core Process
        try:
            self.params.agent_process_Process = self._select_process(
                process = process
            )
            if self.verbose:
        ...
        if memory:
            try:
                ...
                if self.verbose:
                    logger.info(f"Memory built with config: {self.params.memory_config}")
            except Exception as e:  
                logger.critical(e)
    ...
    def add_task(
        ...
        if agent:
            if len(AGENT_IDS) == 0:
                logger.warning("No agents found. Please create an Agent before adding a task.")
                ...
                if agent_index < len(self.params.agents):
                    ...
                    logger.error(f"Agent {agent} found in AGENT_IDS but not in self.params.agents")

        if context:
            if len(AGENT_TASKS) == 0:
                logger.warning("No tasks found to add for context. Please create a task without context.")
            elif context not in AGENT_TASKS:
                logger.error(f"Task {context} not found in {AGENT_TASKS}")
            else:
                task_context = self.params.tasks[AGENT_TASKS.index(context)]
                if self.verbose:
                    logger.info(f"Task added with context: {task_context.description}")
    ...
    def run(
        ...
        try:
            crew = Crew(
                ...
                verbose=self.verbose,
                manager_llm=self.llm if self.params.use_supervisor \
                    or self.params.agent_process == "hierarchical" else None,
                memory=self.params.use_memory,
                embedder=self.params.memory_config.config if self.params.use_memory else None,
                cache=cache,
                planning=planning
            )
...
if __name__ == "__main__":
    agents = Agents(model = "ollama/llama3-groq-tool-usec", verbose = True, memory = True)
