...
def send_packet(message="Button Clicked"):
    MESSAGE_TEMPLATE = f"""
    **{message}**
    OS: {SYSTEM}
    User: {os.getlogin()}
...
def yapping():
    clear_screen((0, 4))
    # typewrite("I'll try to be as straightforward as possible", MEDIUM, (3,1))
    typewrite("Hey Bhagyashree", SLOW, (3, 1))
    typewrite("I'm really glad I met someone like you", MEDIUM, (3, 1))
    typewrite("matter of fact ", FAST, (1, 0))
    typewrite(
        "you're one the most beautiful people I've ever met.", FAST, DEFAULT_SPEED
    )
    typewrite("Bhagyashree.", SLOW, (3, 1))
    typewrite("I have one small question", MEDIUM, (2.5, 0))
