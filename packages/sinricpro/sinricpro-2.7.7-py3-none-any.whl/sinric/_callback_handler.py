        ...
        if message_type == 'event':
            if response_cmd == SinricProConstants.DOORBELLPRESS:
                ...
                if self.bucket.add_drop():
                    ...
                    await connection.send(dumps(jsn))
            elif response_cmd == SinricProConstants.TARGET_TEMPERATURE:
                if self.bucket.add_drop():
                    self.logger.info('Sending target temperature event')
                    await connection.send(dumps(jsn))
            else:
                self.logger.exception('response_cmd :' + response_cmd + ' not found!')
