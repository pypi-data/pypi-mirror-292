...
class JwtProvider(betterproto.Message):
    ...
    other value will be overwritten.
    """
...
class JwtProviderNormalizePayload(betterproto.Message):
    ...
    token with a claim ``scopes: "email profile"`` is translated to dynamic
    metadata  ``scopes: ["email", "profile"]`` if this field is set value
    ``["scopes"]``. This special handling of ``scopes`` is recommended by
