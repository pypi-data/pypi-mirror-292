...
class OAuth2Config(betterproto.Message):
    """OAuth config [#next-free-field: 18]"""
    ...
    """
    If set to true, preserve the existing authorization header. By default
    Envoy strips the existing authorization header before forwarding upstream.
    ...
    otherwise the authorization server expiration or ``defaul_expires_in`` is
    used.
    """

    disable_id_token_set_cookie: bool = betterproto.bool_field(17)
    """
    If set to true, Envoy will not set a cookie for ID Token even if one is
    received from the Identity Provider. This may be useful in cases where the
    ID Token is too large for HTTP cookies (longer than 4096 characters).
