...
class Cluster(betterproto.Message):
    """Configuration for a single upstream cluster. [#next-free-field: 58]"""
    ...
    """
    [#not-implemented-hide:] A list of metric names from ORCA load reports to
    propagate to LRS. For map fields in the ORCA proto, the string will be of
    the form ``<map_field_name>.<map_key>``. For example, the string
    ``named_metrics.foo`` will mean to look for the key ``foo`` in the ORCA
    ``named_metrics`` field. The special map key ``*`` means to report all
    entries in the map (e.g., ``named_metrics.*`` means to report all entries
    in the ORCA named_metrics field). Note that this should be used only with
    trusted backends. The metric names in LRS will follow the same semantics as
    this field. In other words, if this field contains ``named_metrics.foo``,
    then the LRS load report will include the data with that same string as the
    key.
    """
...
class UpstreamConnectionOptionsHappyEyeballsConfig(betterproto.Message):
    first_address_family_version: "UpstreamConnectionOptionsFirstAddressFamilyVersion" = betterproto.enum_field(
        1
    )
    """
