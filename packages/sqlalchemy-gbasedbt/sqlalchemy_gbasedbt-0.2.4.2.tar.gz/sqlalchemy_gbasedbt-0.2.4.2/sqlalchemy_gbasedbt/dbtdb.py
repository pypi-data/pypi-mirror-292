...
class GBasedbtExecutionContext(default.DefaultExecutionContext):
    ...
    def post_exec(self):
        if self._select_lastrowid:
            # Massive hack to get the last inserted serial (sqlca.sqlerrd1) or serial8
            # Apparently both are never set
            cursor = self.create_cursor()
            cursor.execute("SELECT dbinfo('sqlca.sqlerrd1'), dbinfo('serial8') FROM dual")
...
class GBasedbtDialect(default.DefaultDialect):
    ...
    def _get_default_schema_name(self, connection):
        connection.exec_driver_sql("select current_user from dual").scalar()
