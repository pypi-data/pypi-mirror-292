...
class DataFrame:
    """
    Class to hold the two types of dataframes (instrumented and non-instrumented)
...
class ModelModule(CoreModule):
    ...
    def execute(self, indir=None, outdir=None):
            ...
            if not any(e for e in ["PGV", "PGA"] if e in self.imt_out_set):
                ...
                if gmpe is not None:
                    ...
                    if gmpe is not None:
                        ...
                        raise TypeError(
                            "Your gmpe set can't make PGV or PGA, please pick an IPE "
                            "instead of the VirtualIPE"
                        )
        ...
        if (
            ...
            self.ipe_extent = VirtualIPE.__fromFuncs__(self.ipe_gmpe, self.gmice)
        else:
        ...
        if self.use_simulations:
            ...
            #
            combined_imt_set = set([x.split("/")[-1] for x in self.sim_imt_paths])
            self.sim_df = {}
        ...
        if isinstance(self.rupture_obj, PointRupture):
            ...
            if self.do_grid:
                if self.true_grid:
                    griddict = ffsim.true_grid(
                        vs30grid, self.gmpe_dict, list(self.oqimt_dict.values())
                    )
                else:
                    ...
                    for oqimt in self.oqimt_dict.values():
                        amp_grid = gsa.applySiteAmp(
                            griddict[oqimt]["mean"], griddict[oqpga]["mean"], oqimt
                        )
                ...
                shapeset = set(shapes)
                if len(shapeset) != 1:
                    raise ValueError("All sites elements must have same shape.")
                ...
                if isinstance(self.ipe, VirtualIPE):
                    ...
                    for sdt in stddev_types:
                        ...
                        if sdt == oqconst.StdDev.INTER_EVENT:
                            ...
                            griddict[immi][sdt] = np.sqrt(gm2mi_var + gm_var_in_mmi)
                else:
                    ffsim = FFSimmer(self.rff, measure="mean")
                    if self.true_grid:
                        mmidict = ffsim.true_grid(
                            self.sites_obj_out.getVs30Grid(), {immi: self.ipe}, [immi]
                        )
                    else:
            ...
            shapeset = set(shapes)
            if len(shapeset) != 1:
                raise ValueError("All dists and sites elements must have same shape.")
            orig_shape = list(shapeset)[0]
            for imt_str in self.full_out_set:
                ...
                if len(pout_sd) > 1:
                    griddict[oqimt][oqconst.StdDev.INTER_EVENT] = pout_sd[1].reshape(
                        orig_shape
                    )
                    griddict[oqimt][oqconst.StdDev.INTRA_EVENT] = pout_sd[2].reshape(
                        orig_shape
                    )
    ...
    def _set_output_params(self):
        ...
        if self.use_simulations:
            ...
            if self.config["interp"]["prediction_location"]["extent"]:
                self.west, self.south, self.east, self.north = self.config["interp"][
                    "prediction_location"
                ]["extent"]
            else:
                self.west, self.east, self.south, self.north = get_extent(
                    config=self.config, ipe=self.ipe_extent, rupture=self.rupture_obj
                )
    ...
    def _populate_data_frames(self):
        ...
        for dfid in self.dataframes:
            ...
            if self.do_directivity is True:
                ...
                self.logger.debug(
                    "Directivity %d evaluation time: %f sec ", dfid, directivity_time
                )
            ...
            for oqimt, pval in pdict.items():
                ...
                if self.do_outliers and (
                    ...
                    self.logger.debug("IMT: %s, flagged: %d", imtstr, np.sum(flagged))
            ...
            if isinstance(self.rupture_obj, PointRupture):
                dd = get_distance(
                    ["rhypo"], df["lat"], df["lon"], df["depth"], self.rupture_obj
                )
                df["rrup"] = dd["rhypo"]
            else:
                dd = get_distance(
        ...
        if isinstance(self.rupture_obj, PointRupture):
            ...
            for oqimt in oqimtlist:
                ...
                pmean_rock, pstddev_rock = self._gmas(
                    gmpedict[oqimt], dfn.sx_rock, dfn.dx, oqimt, self.apply_gafs
                )
                pmean_soil, pstddev_soil = self._gmas(
                    gmpedict[oqimt], dfn.sx_soil, dfn.dx, oqimt, self.apply_gafs
                )
                pdict[oqimt] = {
                    "site": {
                        "mean": pmean,
                ...
                if len(pstddev) > 1:
                    pdict[oqimt]["site"][oqconst.StdDev.INTER_EVENT] = pstddev[1]
                    pdict[oqimt]["site"][oqconst.StdDev.INTRA_EVENT] = pstddev[2]
                    pdict[oqimt]["rock"][oqconst.StdDev.INTER_EVENT] = pstddev_rock[1]
                    pdict[oqimt]["rock"][oqconst.StdDev.INTRA_EVENT] = pstddev_rock[2]
                    pdict[oqimt]["soil"][oqconst.StdDev.INTER_EVENT] = pstddev_soil[1]
                    pdict[oqimt]["soil"][oqconst.StdDev.INTRA_EVENT] = pstddev_soil[2]

            immi = imt.from_string("MMI")
            pmean, pstddev = self._gmas(self.ipe, dfn.sx, dfn.dx, immi, self.apply_gafs)
            pmean_rock, pstddev_rock = self._gmas(
            ...
            if len(pstddev) > 1:
                ...
                pdict[immi]["rock"][oqconst.StdDev.INTER_EVENT] = pstddev_rock[1]
                pdict[immi]["rock"][oqconst.StdDev.INTRA_EVENT] = pstddev_rock[2]
                pdict[immi]["soil"][oqconst.StdDev.INTER_EVENT] = pstddev_soil[1]
                pdict[immi]["soil"][oqconst.StdDev.INTRA_EVENT] = pstddev_soil[2]
        return pdict
    ...
    def _derive_imts_from_mmi(self):
        ...
        for oqimt in imtlist:
            ...
            if oqconst.StdDev.INTER_EVENT in pdict[oqimt]["site"]:
                pstddev.append(pdict[oqimt]["site"][oqconst.StdDev.INTER_EVENT])
                pstddev.append(pdict[oqimt]["site"][oqconst.StdDev.INTRA_EVENT])
    ...
    def _derive_mmi_from_imts(self):
        ...
        df1 = this_df.df
        gmice_imts = [
            str(imt) for imt in self.gmice.DEFINED_FOR_INTENSITY_MEASURE_TYPES
        ]
        ...
        for imtstr in this_df.imts:
            if not (
                imtstr in gmice_imts or (imtstr.startswith("SA") and "SA" in gmice_imts)
            ):
                continue
    ...
    def _compute_mvn(self, imtstr, ddict):
        ...
        if total_only:
            ...
            self.psd_raw[imtstr] = np.sqrt(pout_sd[0] ** 2 - self.tsd[imtstr] ** 2)
        else:
        ...
        for iy in range(self.smny):
            ...
            #
            np.subtract(
                t_y0[ss:se, :], np.dot(rcmatrix_phi, t_d, out=c_tmp1), out=big_c
            )
            #
            ...
            #
            s_tmp1r = np.dot(big_c, self.mu_h_yd[imtstr], out=s_tmp1).reshape((-1,))
    ...
    def _get_info(self, moutgrid):
        ...
        if "df2" in self.dataframes:
            ...
            info[ip][ei]["intensity_observations"] = str(np.size(df2.df["lon"]))
        else:
        ...
        if "df1" in self.dataframes:
            ...
            for imtstr in df1.imts:
                if "MMI" in imtstr:
                    continue
                all_flagged |= df1.df[imtstr + "_outliers"] | np.isnan(df1.df[imtstr])
    ...
    def _store_point_data(self, oc):
        ...
        if isinstance(self.rupture_obj, PointRupture):
            for dm in get_distance_measures():
                ...
                if dm_arr is not None:
                    oc.setArray(
                        ["distances"], dm, dm_arr.flatten(), metadata=distance_metadata
                    )
        else:
            for dm in get_distance_measures():
                dm_arr = getattr(self.dx_out, dm, None)
...
def main():
    """
    The main function for cases where this function is called in standalone mode.
    """
