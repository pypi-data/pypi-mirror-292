...
class DockerRegistryService(RegistryService, RegistryServiceBase):
    ...
    async def authorize(self, ws_session: str) -> Result[None, str]:
        ...
        logger.debug("DockerRegistryService.authorize() - ws_session: %s", ws_session)
        try:
            ...
            async with websockets.connect(ws_session) as ws:
                ...
                logger.debug(
                    "DockerRegistryService.authorize() - auth_msg: %s", auth_msg
                )
                ...
                logger.debug(
                    "DockerRegistryService.authorize() - creds: %s",
                ...
                logger.debug(
                    "DockerRegistryService.authorize() - registry: %s",
                ...
                logger.info(
                    "Authentication to registry @ [ %s ] using username: %s ...",
                    url,
                    username,
                )
                ...
                if result.is_err():
                    return result
                logger.info("Authenticated to registry [ %s ] successfully", url)
    ...
    async def _docker_login(
        ...
        try:
            logger.debug(
                "DockerRegistryService._docker_login() - url: %s, username: %s",
                url,
                username,
            )
            ...
            logger.debug(
                "DockerRegistryService._docker_login() - login_resp: %s", login_resp
            )
