...
async def startup():
    ...
    try:
        ...
        if not jinja_template_location.exists():
            logger.warning("Jinja template file not found: %s", jinja_template_location)
        word_art_path = package_location / config.settings.word_art_path
        if not word_art_path.exists():
            logger.warning("Word art file not found: %s", word_art_path)
            return
        logger.debug("Word Art path: %s", word_art_path)
        logger.info("\n%s\n", open(word_art_path.absolute()).read())
        logger.info(
            "Started at: %s\t\t\tVersion:%s",
