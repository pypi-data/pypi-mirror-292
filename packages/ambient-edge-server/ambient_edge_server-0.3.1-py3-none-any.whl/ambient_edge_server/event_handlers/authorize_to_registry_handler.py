...
class AuthorizeRegistryHandler(BaseHandler):
    ...
    async def handle(self, msg: Message) -> None:
        logger.info("Handling message for topic: %s", msg.topic)
        msg_data: dict = json.loads(msg.message)
        logger.info("Message data: %s", json.dumps(msg_data, indent=4))
        ws_session: Optional[str] = msg_data.get("ws_session", None)
        logger.debug("AuthorizeRegistryHandler.handle() - ws_session: %s", ws_session)
        ...
        logger.debug(
            "AuthorizeRegistryHandler.handle() - registry_svc: %s - %s",
            registry_svc,
            type(registry_svc),
        )
        ...
        logger.debug(
            "AuthorizeRegistryHandler.handle() - authorize method: %s",
            registry_svc.authorize,
        )
        result = await registry_svc.authorize(ws_session=ws_session)
        if result.is_err():
            logger.error("Failed to authorize registry: %s", result.unwrap_err())
            return
