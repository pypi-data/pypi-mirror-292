...
#     p_inf = 1.0/(1.0 + exp(-(V + 35.0)/10.0));  tau_p = tau_max/ ( 3.3*exp((V+35.0)/20.0) + exp(-(V+35.0)/20.0) )
    ...
    du[93] = 0 # (1/4e-4)*( -dIPump_a -IBK - IKirAS  - IKirAV - IleakA  - ITRP_a )   # Va = -90
    du[94] = 0 # vLeakNag-3*vPumpg +vgstim # Na_a
    du[95] = 0 # JNaK_a + ( - IleakA - IBK - IKirAS - IKirAV)/(4e-4*843.0*1000.) - RateDecayK_a*(K_a - K_a0) # W2013  # K_a
    ...
    du[96] = 0 # SmVn/F*ik_density*(eto_n/eto_ecs) - 2*vPumpn*(eto_n/eto_ecs) - 2*(eto_a/eto_ecs)*vPumpg - JdiffK  -( ( - IleakA - IBK - IKirAS - IKirAV)/(4e-4*843.0*1000.))     #  K_out
    ...
    du[98] = 0 # 1/Cm*(-IL-ina_density-ik_density-ICa-ImAHP-dIPump)  # VNeu
    
    du[99] = 0 # vLeakNan-3*vPumpn + vnstim # Na_n
    
    du[100] =  0 #phi*(hinf-h)/tauh # h               
    du[101] =  0 #phi*(ninf-n)/taun # n               
    du[102] =  0 #-SmVn/F*ICa-(Ca_n-Ca_n0)/tauCa  # Ca_n 2+ 
    du[103] =  0 #phi*(p_inf - pgate)/tau_p # pgate    
    du[104] =  0 #psiBK*cosh(  (Va - (-0.5*v5BK*tanh((Ca_a - Ca3BK)/Ca4BK )  + v6BK) )  / ( 2*v4BK  ) ) * ( nBKinf - nBK_a)  # nBK_a  # approx use https://www.cell.com/biophysj/pdfExtended/S0006-3495(13)01032-1 Witthoft 2013 with Ca_n istead of Ca_a 
    du[105] = 0 # set in CB # mGluRboundRatio_a
    
    du[106] =  0 # rhIP3a*((mGluRboundRatio_a + deltaGlutSyn) / (KGlutSyn + mGluRboundRatio_a + deltaGlutSyn)) - kdegIP3a*IP3_a   # IP3_a   
    du[107] =  0 # konhIP3Ca_a*( khIP3Ca_aINH - ( Ca_a + khIP3Ca_aINH)*hIP3Ca_a )       # hIP3Ca_a
    du[108] =  0 # beta_Ca_a*( IIP3_a - ICa_pump_a + Ileak_CaER_a ) - 0.5*ITRP_a/(4e-4*843.0*1000.) #   #S12  # Ca_a
    du[109] =  0 # Ca_r_a Ca ER a
    du[110] =  0 # (Ca_perivasc/tauTRPCa_perivasc)*( sinfTRPV  - sTRP_a )   # sTRP_a
    ...
    du[112] = 0 # VprodEET_a*(Ca_a - CaMinEET_a) - kdeg_EET_a*EET_a # EET_a
