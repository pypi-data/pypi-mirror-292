    ...
    def run(self):
        ...
        try:
            ...
            if attribute:
                ...
                html = json.dumps(
                    f'Value:&nbsp;<span class="value">{value.value}</span>'
                )
...
STATE_CLASSES = dict(
    (state, json.dumps(getStateClasses(state)))
...
class TaurusSynopticWidget(SynopticWidget, TaurusWidget):
    ...
    def setModel(self, image, section=None):
        self.set_url(image)
        self.registry = Registry(self.attribute_listener, self.unsubscribe_listener)
        self.registry.start()
    ...
    def _hovered(self, sec, mods):
        self.registry.get_listener(str(mods))
    ...
    def run_plugin_command(self, plugin, cmd, args):
        try:
            plugins = __import__(f"plugins.{plugin}", globals(), locals(), [cmd], -1)
        except ImportError as e:
            print(f"Could not initialize plugin '{plugin}'!")
            print("Exception: ", e)
            return ""
        return getattr(plugins, cmd)(self, args)

    def handle_subscriptions(self, models=None):
        if models is None:
            models = []
        if self.registry:
    ...
    def unsubscribe_listener(self, unsubscribed):
        ...
        for model in unsubscribed:
            self.js.evaluate(f"synoptic.setClasses('model', {model!r}, {classes})")
    ...
    def attribute_listener(self, model, evt_src, evt_type, evt_value):
        ...
        if isinstance(value, (DevState, tango.DevState, tango._tango.DevState)):
            ...
            if attr.lower() == "state":
                ...
                # too, for convenience.
                self.js.evaluate(f"synoptic.setClasses('model', {device!r}, {classes})")
                self.js.evaluate(
                    f"synoptic.setClasses('model', '{device}/State', {classes})"
                )
                self.js.evaluate(f"synoptic.setData('model', {device!r}, {data!r})")
                self.js.evaluate(
                    f"synoptic.setData('model', '{device}/State', {data!r})"
                )
            else:
                # Apparently it's an attribute of type DevState
                # but it is not the normal "State" attribute.
                self.js.evaluate(f"synoptic.setClasses('model', {model!r}, {classes})")
                self.js.evaluate(f"synoptic.setData('model', {model!r}, {data})")

            self.js.evaluate(f"synoptic.setText('model', {model!r}, '{value}')")
            ...
            self.js.evaluate(
                f"synoptic.setClasses('model', {model!r}, {json.dumps(classes)})"
            )
            self.js.evaluate(f"synoptic.setText('model', {model!r}, '{value}')")
            self.js.evaluate(
                "synoptic.setData('model', {!r}, {})".format(
                    model,
                    json.dumps(
                        {"value": str(value).lower(), "quality": quality_string}
                    ),
                )
            )
        else:
            ...
            if quality == tango.AttrQuality.ATTR_INVALID:
                ...
                # In Taurus 4 all numeric values are pint.Quantity
                try:
                    text = f"{value.magnitude:{evt_src.format_spec}}"
            ...
            if unit in (None, "No unit"):
                unit = ""
            self.js.evaluate(f"synoptic.setText('model', {model!r}, '{text} {unit}')")
            self.js.evaluate(
                "synoptic.setData('model', {!r}, {})".format(
                    model, json.dumps({"value": text, "quality": quality_string})
                )
            )
    ...
    def on_rightclick(self, kind, name):
        ...
        if kind == "model" and self.registry.device_validator.isValid(name):
            if name in self._panels:
                ...
                print(f"Found existing panel for {name}:", widget)
    ...
    def _cleanup_panel(self, w):
        ...
        Panels are added to the internal dict using the device name that was clicked as key.
        This name is saved as Window title.
        """
        if self.registry:
            with self.registry.lock:
                name = w.windowTitle()
                print("cleaning up panel for", name, "...")
                panel = self._panels.pop(name, None)
                if panel is None:
                    print(f"{name} not found in {self._panels}")
                    # Don't set the model to None in case this panel is present
                    # under another device name in the dict
                else:
    ...
    def _update_device_tooltip(self, device):
        ...
        self.js.evaluate(f'synoptic.setTooltipHTML("{device}", {html})')
