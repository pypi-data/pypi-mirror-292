...
class Registry(QtCore.QThread):
    ...
    def subscribe(self, models=None):
        """
            ...
            if self.device_validator.isValid(model):
                ...
                if modelstate not in taurusattrs.keys():
                    try:
                        ...
                        print(
                            f"Failed to create Taurus Attribute for model {model}! {e}"
                        )
            elif self.attribute_validator.isValid(model) or self.eval_validator.isValid(
                model
            ):
                attrs[model] = True
                ...
                print(f"Invalid Taurus model {model}!?")
    ...
    def _add_listener(self, model):
        try:
            ...
            print(f"Failed to subscribe to model {model}! {e}")
        except Exception:
            print(f"Failed to subscribe to model {model}!")
    ...
    def _remove_listener(self, model):
        ...
        try:
            listener._unsubscribeConfEvents()
        except AttributeError:
            # 'EvaluationAttribute' has no attribute '_unsubscribeConfEvents'
            # This method is only for 'TangoAttribute'
            pass
