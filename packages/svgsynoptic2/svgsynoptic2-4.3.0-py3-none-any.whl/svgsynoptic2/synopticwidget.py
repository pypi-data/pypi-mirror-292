...
class SynopticWidget(QtWidgets.QWidget):
    ...
    def on_hover(self, section, models):
        ...
        if section:
            ...
            self.js.evaluate(f"synoptic.setTooltipHTML('{section}')")
        elif models:
            self.js.evaluate("synoptic.showTooltip();")
            self.js.evaluate(f"synoptic.setTooltipHTML('{models[0]}')")
        else:
    ...
    def zoom_to(self, kind, name):
        ...
        """
        self.js.evaluate(f"synoptic.zoomTo({str(kind)!r}, {str(name)!r})")
    ...
    def select(self, kind, names, replace=True):
        ...
        if names:
            for name in names:
                self.js.evaluate(f"synoptic.select({str(kind)!r}, {[str(name)]!r})")
