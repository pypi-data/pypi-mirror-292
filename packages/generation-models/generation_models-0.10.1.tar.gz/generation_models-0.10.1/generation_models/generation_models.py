...
class SolverConfig(BaseModel):
    ...
    def check_solver_config(cls, values):
        if values["solver_specific"] and (values["solver"] not in {"HiGHS", "GLPK"}):
...
class ACExternalGenerationModel(ExternalGenerationModel):
    ...
    :class:`GenerationModelResults` is the results schema
    """
...
class PVGenerationModel(BaseGenerationModel):
    ...
    hybrid simulations. When used for a solar-only simulation, :class:`GenerationModelResults` is the results
    schema
    """
...
class OpsStrategy(Enum):
    dart_pq_qo = MarketConfig(
        da=BidOfferStrategy.price_quantity,
        rt=BidOfferStrategy.quantity_only,
    )
    r"""Price-quantity bid-offers in DAM with quantity-only bid-offers in RTM
    """
    rt_redispatch_qo = MarketConfig(
        da=BidOfferStrategy.awarded,
        rt=BidOfferStrategy.quantity_only,
    )
    ...
    @property
    def da(self) -> BidOfferStrategy:
        return self.value.da

    @property
    def rt(self) -> BidOfferStrategy:
        return self.value.rt

    @property
    def independent_dam(self) -> bool:
        return self.value.independent_dam
...
class PVStorageModel(ImportExportLimitMixin, MarketBase):
    r"""Simulation class for hybrid simulations. :class:`PVStorageModelResults` is the results schema"""
...
class StandaloneStorageModel(ProjectTermMixin, ImportExportLimitMixin, MarketBase):
    ...
    :class:`StandaloneStorageModelWithDownstreamResults` if a downstream system is specified, otherwise the schema
    is :class:`StandaloneStorageModelSimpleResults`"""
...
class ResultsFormat(str, Enum):
    ...
    :class:`~generation_models.generation_models.v0_output_schema.GenerationModelResults`
    """
    v1 = "v1"
    ...
    :class:`~generation_models.generation_models.v2_output_schema.GenerationModelResults`
    """
