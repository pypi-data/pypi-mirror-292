...
class ToolsGPT():
    ...
    def match_or_make(self, text, list_of_options: list = [], direction='Make a simple and general category to classify content like this', number_of_words_in_category_name=2, model=None, loop_limit=20) -> str:
        ...
        if not list_of_options:
            answer = self.make_option_from_zero(text, direction)
            binary = 'yes'
        else:
        ...
        if binary == 'yes':
            ...
            while True:
                if loops >= loop_limit:
                    break
                answer = self.finalize_category(text, answer, number_of_words_in_category_name)
                if self.verbose == True:
    ...
    # internal function
    def finalize_category(self, text, prev_answer, number_of_words=2, timeout=None, temperature=None):
        ...
        return self.retry_llm(custom_prompt=prompt, timeout=timeout, temperature=temperature)
