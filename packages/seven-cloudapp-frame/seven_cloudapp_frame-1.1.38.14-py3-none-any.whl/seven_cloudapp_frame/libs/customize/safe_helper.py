...
@LastEditTime: 2024-07-23 16:18:48
...
class SafeHelper:
    ...
    def __redis_init(self):
        ...
        """
        config_dict = config.get_value("safe_redis")
        if not config_dict:
        ...
        redis_cli = RedisExHelper.init(config_dict=config_dict, decode_responses=True)
        return redis_cli
    ...
    def check_current_limit(self, app_id, current_limit_count, current_limit_minute=1, handler_name=''):
        ...
        if current_limit_minute == 1:
            ...
            if handler_name:
                cache_key = cache_key + ":" + handler_name
        else:
            key_list = []
    ...
    def check_current_limit_by_time_window(self, request_queue_name, limit_count, request_limit_time=1):
        ...
        window_start = f"{request_queue_name}:{window_start}" + ":" + config.get_value("project_name", "default")
