...
@LastEditTime: 2024-07-04 18:10:07
...
class SevenHelper:
    ...
    def is_continue_request(self, cache_key, expire=500):
        ...
        if share_config.get_value("is_pressure_test",False): #是否进行压力测试
            return False
        redis_init = self.redis_init(config_dict=config.get_value("safe_redis"))
    ...
    def redis_check_llen(self,queue_name,queue_lenth=100):
        """
         ...
         :last_editors: HuangJianYi
         """
        return RedisExHelper.check_llen(queue_name, queue_lenth, config_dict=config.get_value("safe_redis"))
    ...
    def redis_lpush(self, queue_name, value, expire):
        """
         ...
         :last_editors: HuangJianYi
         """
        RedisExHelper.lpush(queue_name, value, expire, config_dict=config.get_value("safe_redis"))
    ...
    def redis_lpop(self, queue_name):
        """
         ...
         :last_editors: HuangJianYi
         """
        return RedisExHelper.lpop(queue_name, config_dict=config.get_value("safe_redis"))
    ...
    def redis_acquire_lock(self, lock_name, acquire_time=5, time_out=10):
        ...
        :last_editors: HuangJianYi
        """
        return RedisExHelper.acquire_lock(lock_name, acquire_time, time_out, config_dict=config.get_value("safe_redis"))
    ...
    def redis_release_lock(self, lock_name, identifier):
        ...
        :last_editors: HuangJianYi
        """
        return RedisExHelper.release_lock(lock_name, identifier, config_dict=config.get_value("safe_redis"))
