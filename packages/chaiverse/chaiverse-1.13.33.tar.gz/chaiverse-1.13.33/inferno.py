...
class InfernoUnion(InfernoModel):
    """
    Helper class to do polymorphism with InfernoUnion.
    Example:
    ...
    def __new__(cls, *args, **kwargs):
        return parse_obj_as(Union[cls._polymorphisms], kwargs)

    def __class_getitem__(cls, item):
        cls._validate_union(item)
        new_class = type("InfernoUnion", (InfernoUnion,), {})
        new_class._polymorphisms = item
        new_class.database = item[0].database
        new_class.path = item[0].path
        return new_class
