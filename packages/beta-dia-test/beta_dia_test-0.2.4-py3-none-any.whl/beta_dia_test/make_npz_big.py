...
import fxic
import param_g
from decoy import cal_fg_mz_iso
...
def main():
    ...
    dir_ws = Path(r'/home/mantou/data_4d/HeLa_Train/model_big_13_50')
    ...
    for i_ws, ws in enumerate(dir_raw.glob('*.d')):
        ...
        assert df['pred_im'].min() > 0.5
        assert df['pred_im'].max() < 2.

        # 由于big太大，需要对df先划分
        df['chunk_idx'] = np.random.randint(0, chunk, len(df))
        for chunk_idx in range(chunk):
            df_chunk = df[df['chunk_idx'] == chunk_idx].reset_index(drop=True)
            locus_m = df_chunk['locus'].values.reshape(-1, 1)
            ...
            dir_out = dir_ws / 'hdf'
            if chunk_idx == 0:
                fout = dir_out / (fname + '_eval.hdf')
            else:
                fout = dir_out / (fname + '_train_' + str(chunk_idx) + '.hdf')

            with h5py.File(fout, 'w') as f:
                f.create_dataset('map', data=maps_v)
                f.create_dataset('label', data=labels)
                f.create_dataset('ion_num', data=ion_nums)
