...
module easychem_fortran_source
   ...
   subroutine ec_INVERT_MATRIX_SHORT(lens,matrix,vector,solution_vector)
      ...
      call ec_LU_decomposition(lens,matrix,index,indic)
      if (error) RETURN
      call ec_LU_backsubstitution(lens,matrix,index,solution_vector)
   ...
   subroutine ec_INVERT_MATRIX_LONG(lens,matrix,vector,solution_vector)
      ...
      IF (remove_ions) THEN
         ...
         call ec_LU_decomposition(lens-N_ions,matrix_nions,index_nions,indic)
         if (error) RETURN
         call ec_LU_backsubstitution(lens-N_ions,matrix_nions,index_nions,solution_vector_nions)
         if (error) RETURN
   ...
   subroutine ec_LU_decomposition(length,matrix,row_permutation_indices,row_interchange_parity)
      implicit none
      integer, intent(in)              :: length
      double precision, intent(inout)  :: matrix(length,length)
      integer, intent(out)             :: row_permutation_indices(length)
      double precision, intent(out)    :: row_interchange_parity

      double precision                 :: row_scaling(size(matrix,1))
      double precision, parameter      :: numerical_safety_value = 1d-20
      integer                          :: iter_j, max_index

      row_scaling = maxval(abs(matrix),dim=2)

      if (any(row_scaling == 0d0)) then
         error = .true.
         err_msg = "ERROR: singular matrix detected in the LU decomposition!"
         RETURN
      end if

      row_scaling = 1d0/row_scaling

      row_interchange_parity = 1d0

      do iter_j = 1,length

         ! Finding the pivot row
         max_index = (iter_j-1)+maxloc(row_scaling(iter_j:length)*abs(matrix(iter_j:length,iter_j)), dim=1)

         ! Check if rows and scale factors need to be interchanged
         if (iter_j /= max_index) then
            call lu_swap(length,matrix(max_index,:),matrix(iter_j,:))
            row_interchange_parity = -row_interchange_parity
            row_scaling(max_index) = row_scaling(iter_j)
         end if

         row_permutation_indices(iter_j) = max_index

         if (matrix(iter_j,iter_j) == 0d0) then
            matrix(iter_j,iter_j) = numerical_safety_value
         end if

         ! Dividing the matrix by the pivoting element
         matrix(iter_j+1:length,iter_j) = matrix(iter_j+1:length,iter_j) / matrix(iter_j,iter_j)

         ! Reduce the rest of the matrix
         matrix(iter_j+1:length,iter_j+1:length) = matrix(iter_j+1:length,iter_j+1:length) - &
         (spread(matrix(iter_j+1:length,iter_j), dim=2, ncopies=length-iter_j) * &
                 spread(matrix(iter_j,iter_j+1:length), dim=1, ncopies=length-iter_j))

      end do

   END SUBROUTINE ec_LU_decomposition

   !> LU back substitution
   SUBROUTINE ec_LU_backsubstitution(length,LU_matrix,row_permutation_indices,vector)
      implicit none
      integer, intent(in) :: length
      double precision, intent(in) :: LU_matrix(length,length)
      integer, intent(in) :: row_permutation_indices(length)
      double precision, intent(inout) :: vector(length)
      integer :: i,i_i,l_i
      double precision :: matrix_sum
      ...
      do i=1,length
         ...
         ! Index to undo the row permutations
         l_i=row_permutation_indices(i)

         matrix_sum=vector(l_i)
         vector(l_i)=vector(i)

         if (i_i /= 0) then
            matrix_sum=matrix_sum-dot_product(LU_matrix(i,i_i:i-1),vector(i_i:i-1))
         else if (matrix_sum /= 0d0) then
            i_i=i
         end if
      ...
      ! Back susbtitution step
      do i=length,1,-1
         vector(i) = (vector(i)-dot_product(LU_matrix(i,i+1:length),vector(i+1:length)))/LU_matrix(i,i)
      end do

   END SUBROUTINE ec_LU_backsubstitution

   subroutine lu_swap(length,array1,array2)
      implicit none
      integer, intent(in)              :: length
      double precision, intent(inout)  :: array1(length), array2(length)
      double precision                 :: temporary(length)
