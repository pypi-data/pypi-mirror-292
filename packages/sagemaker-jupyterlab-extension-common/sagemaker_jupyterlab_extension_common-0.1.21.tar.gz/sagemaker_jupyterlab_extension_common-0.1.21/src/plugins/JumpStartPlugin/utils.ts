...
const executeOpenNotebook = async (router: IRouter, app: JupyterFrontEnd, logger: ILogger) => {
  ...
  try {
    ...
    logger.info({ Message: 'Successfully loaded the JumpStart Deeplinking plugin' });
  } catch (error) {
...
const fetchNotebook = async (body: object, logger: ILogger) => {
  ...
  logger.info({ Message: 'Fetching JumpStart Notebook', ClientRequestId: clientRequestId });
  let response;
  try {
  ...
  logger.info({
    Message: 'Successfully fetched JumpStart Notebook',
