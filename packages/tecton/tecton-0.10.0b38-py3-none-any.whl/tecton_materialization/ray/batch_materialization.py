...
def run_batch_materialization(
    ...
    if should_write_to_online_from_offline_store:
        ...
        output_arrow_data_as_json(bulk_backfill_data, materialization_task_params.feature_view)


def output_arrow_data_as_json(bulk_backfill_data: pyarrow.RecordBatchReader, fv: FeatureView):
    ...
    while True:
        ...
        output_arrow_record_batch(batch_bytes, fv)
...
def output_arrow_record_batch(batch_bytes: bytes, fv: FeatureView):
    ...
    job = runner_function.remote(batch_bytes, fv.SerializeToString())
    ray.get(job)
