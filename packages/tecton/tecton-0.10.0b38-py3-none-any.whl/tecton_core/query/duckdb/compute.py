...
class DuckDBCompute(SQLCompute):
    ...
    def register_temp_table_from_offline_store(
        ...
        """
        # TODO (vitaly): will have parity with AWS setup pending support from DuckDB around `read_parquet` from GCS
        # For now we'll use the arrow reader to read from GCS.
        if dataset_root_path.startswith("gs"):
            relation = self.session.from_arrow(dataset)
