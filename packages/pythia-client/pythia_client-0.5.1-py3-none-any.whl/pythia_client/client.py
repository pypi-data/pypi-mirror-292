...
class APIClient:
    ...
    def __init__(self, url: str, api_key: str):
        ...
        self.files_metadata_endpoint = urljoin(str(self.url), "files-metadata")
        ...
        self.pk_list_endpoint = urljoin(
            str(self.url), "/data-extraction/extract-pk-list"
        )
    ...
    def get_keywords_by_group_id(
        ...
        with requests.Session() as session:
            ...
            with session.get(
                    self.files_metadata_endpoint + f"/keywords/by-group-id{group_id}",
                    headers=headers,
            ) as response:
                if response.status_code == 200:
    ...
    def get_keywords_by_pythia_doc_category(
        ...
        with requests.Session() as session:
            ...
            with session.get(
                    ...
                    + f"/keywords/by-pythia-doc-category/{pythia_doc_category}",
                    headers=headers,
            ) as response:
                if response.status_code == 200:
                    response = response.json()
    ...
    def get_docs_by_group_id(
        ...
        with requests.Session() as session:
            ...
            with session.get(
                    self.files_metadata_endpoint + f"/documents/by-group-id{group_id}",
                    headers=headers,
            ) as response:
                if response.status_code == 200:
    ...
    def get_docs_by_pythia_doc_category(
        ...
        with requests.Session() as session:
            ...
            with session.get(
                    self.files_metadata_endpoint
                    + f"/documents/by-pythia-doc-category/{pythia_doc_category}",
                    headers=headers,
            ) as response:
                if response.status_code == 200:
                    api_response = [DocsAvailable(**resp) for resp in response.json()]
    ...
    def list_query_history_by_group_id(
        ...
        with requests.Session() as session:
            ...
            with session.get(
                    self.feedback_endpoint + f"/list-query-history/{group_id}",
                    headers=headers,
    ...
    def extract_pk_list(
        ...
        """Submit one or multiple pk-list/invoice from data extraction. This extraction is asynchronous and returns an s3_key that can be used to check the status of the background task running.
        Args:
            files: The paths to the files OR tuples of filename and file object to extract the pk-list data from.
        ...
        Usage:
        ```python
        pk_list_response = client.extract_pk_list(["path/to/file.pdf"])
        {"message": "Files Submitted. PK-list extraction tasks created.", "s3_keys": ["pk-list/abc123.pdf"]}
        pk_list_response = client.extract_pk_list([("file.pdf", file)])
        {"message": "Files Submitted. PK-list extraction tasks created.", "s3_keys": ["pk-list/abc123.pdf"]}
        ```
        """
        prepared_files = []
    ...
    def get_extract_pk_list_results(
            self,
            s3_key: str,
    ) -> PkTask:
        """Get the status and result of a pk-list task based on its S3 key.
        Args:
            s3_key: The unique S3 key of the pk-list extraction tasK.
        ...
        Usage:
        ```python
        pk_list_results = client.get_extract_pk_list_results(["pk-list/abc123.pdf"])
        ```
        """
        with requests.Session() as session:
            headers = {
            ...
            with session.get(
                    self.pk_list_endpoint + f"/get/{s3_key}",
                    headers=headers,
            ) as response:
                if response.status_code == 200:
                    api_response = PkTask(**response.json())
                else:
                    raise Exception(
                        f"Pythia API Error {response.status_code}: {self._process_error_response(response)}"
                    )
