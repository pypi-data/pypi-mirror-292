...
def test_client_get_docs_by_group_id():
    ...
    assert len(response_get_docs) == 4
...
def test_add_feedback_to_query():
    ...
    response = client_nbd.add_feedback_to_query(
        query_id=ValueStorage.query_id, feedback=1
    )
    assert response is not None
    response_dict = response.model_dump()
...
def test_client_extract_pk_list():
    files = [
        "test/samples/pk-list-1.pdf",
    ]
    response = client.extract_pk_list(files)
    assert response["message"] == "Files Submitted. PK-list extraction tasks created."
    assert len(response["s3_keys"]) == 1
    ValueStorage.pk_list_s3_keys = response["s3_keys"]


def test_client_get_extract_pk_list_results():
    time.sleep(20)
    response = client.get_extract_pk_list_results(ValueStorage.pk_list_s3_keys[0])
    assert response is not None
    response_dict = response.model_dump()
