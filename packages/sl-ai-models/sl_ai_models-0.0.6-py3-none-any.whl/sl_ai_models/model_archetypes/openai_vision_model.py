...
from sl_ai_models.utils.openai_utils import VisionMessageData, CHEAP_VISION_DATA
...
class OpenAiVisionToTextModel(OpenAiTextToTextModel, ABC):
    ...
    async def invoke(self, input: VisionMessageData) -> str:
        response: TextTokenCostResponse =  await self._invoke_with_request_cost_time_and_token_limits_and_retry(input)
        return response.data


    async def _mockable_direct_call_to_model(self, input: VisionMessageData) -> TextTokenCostResponse:
        ...
        response: TextTokenCostResponse = await self._call_online_model_using_api(messages, self.temperature, max_tokens)
        return response


    @staticmethod
    def _get_cheap_input_for_invoke() -> VisionMessageData:
        return CHEAP_VISION_DATA


    def create_messages_from_input(self, input: VisionMessageData) -> list:
        if self.system_prompt is None:
            return OpenAiUtils.put_single_image_message_in_list_using_gpt_vision_input(input)
        else:
            return OpenAiUtils.create_system_and_image_message_from_prompt(input, self.system_prompt)
