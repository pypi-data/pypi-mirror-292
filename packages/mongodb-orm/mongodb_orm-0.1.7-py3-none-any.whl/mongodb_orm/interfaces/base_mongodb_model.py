...
class BaseMongoDBModel:
    ...
    def connect(self, tenant: str, collection: str, options: Options = None, indexes: List[Index] = None,
        ...
        client_database = self.get_client_database(tenant)
    ...
    @staticmethod
    def get_client_database(tenant: str) -> Database:
        return MongoDBClient().get_database(f'{tenant}')

    def get_collections(self, tenant: str) -> List[str]:
        return self.get_client_database(tenant).list_collection_names()
