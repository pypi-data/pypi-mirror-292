...
from zdppy_excel.xml.constants import ARC_STYLE, SHEET_MAIN_NS
...
def apply_stylesheet(archive, wb):
    ...
    if stylesheet.cell_styles:
        ...
        # need to overwrite zdppy_excel defaults in case workbook has different ones
        ...
        warn("Workbook contains no stylesheet, using zdppy_excel's defaults")

    if not wb._named_styles:
        normal = styles['Normal']
        wb.add_named_style(normal)
        warn("Workbook contains no default style, apply zdppy_excel's default")
