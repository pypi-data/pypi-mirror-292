...
from zdppy_excel.pivot.table import TableDefinition
...
# package imports
from zdppy_excel.utils.exceptions import InvalidFileException
...
from zdppy_excel.cell import MergedCell
from zdppy_excel.comments.comment_sheet import CommentSheet

from .strings import read_string_table, read_rich_text
from .workbook import WorkbookParser
from zdppy_excel.styles.stylesheet import apply_stylesheet

from zdppy_excel.packaging.core import DocumentProperties
from zdppy_excel.packaging.custom import CustomPropertyList
from zdppy_excel.packaging.manifest import Manifest, Override

from zdppy_excel.packaging.relationship import (
    RelationshipList,
    get_dependents,
    get_rels_path,
)
...
from zdppy_excel.worksheet._reader import WorksheetReader
from zdppy_excel.chartsheet import Chartsheet
from zdppy_excel.worksheet.table import Table
from zdppy_excel.drawing.spreadsheet_drawing import SpreadsheetDrawing

from zdppy_excel.xml.functions import fromstring
...
def _validate_archive(filename):
    ...
    if not is_file_like:
        ...
        if file_format not in SUPPORTED_FORMATS:
            if file_format == '.xls':
                msg = ('zdppy_excel does not support the old .xls file format, '
                       'please use xlrd to read this file, or convert it to '
                       'the more recent .xlsx file format.')
            elif file_format == '.xlsb':
                msg = ('zdppy_excel does not support binary format .xlsb, '
                       'please convert this file to .xlsx format if you want '
                       'to open it with zdppy_excel')
            else:
                msg = ('zdppy_excel does not support %s file format, '
...
def load_workbook(
    ...
    :param filename: 打开的路径或类似文件的对象
    :type filename:  字符串或以二进制模式打开的类文件对象 c.f., :class:`zipfile.ZipFile`

    :param read_only: 优化阅读，内容不能编辑
    :type read_only: bool

    :param keep_vba: 保留vba内容(这并不意味着你可以使用它)
    :type keep_vba: bool

    :param data_only: 控制具有公式的单元格是否具有公式(默认)或上次Excel读取工作表时存储的值
    :type data_only: bool

    :param keep_links: 是否应保留指向外部工作簿的链接。默认为True
    :type keep_links: bool

    :param rich_text: 如果设置为True, excel将保留单元格中的任何富文本格式。默认为False。
    :type rich_text: bool

    :rtype: :class:`openpyxl.workbook.Workbook`


    注意:当使用延迟加载时，所有的工作表将是 zdppy_excel.worksheet.iter_worksheet.IterableWorksheet，返回的工作簿将是只读的。
    """
    reader = ExcelReader(
        filename, read_only, keep_vba,
