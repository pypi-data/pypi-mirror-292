...
from zdppy_excel.compat import (
    deprecated,
)

# package imports
from zdppy_excel.utils import (
...
from zdppy_excel.cell import Cell, MergedCell
from zdppy_excel.formatting.formatting import ConditionalFormattingList
from zdppy_excel.packaging.relationship import RelationshipList
from zdppy_excel.workbook.child import _WorkbookChild
from zdppy_excel.workbook.defined_name import (
    DefinedNameDict,
)

from zdppy_excel.formula.translate import Translator
...
class Worksheet(_WorkbookChild):
    ...
    use :func:`zdppy_excel.workbook.Workbook.create_sheet` instead
    ...
    def cell(self, row, column, value=None):
        """
        返回基于给定坐标的单元格对象。
        示例: cell(row=15, column=1, value=5)
        当第一次访问cell时，调用cell会在内存中创建cell。
    ...
    def iter_cols(self, min_col=None, max_col=None, min_row=None, max_row=None, values_only=False):
        ...
        :param min_col: 最小列索引(基于1的索引)
        :type min_col: int

        :param min_row: 最小行索引(基于1的索引)
        :type min_row: int

        :param max_col: 最大列索引(基于1的索引)
        :type max_col: int

        :param max_row: 最大行索引(基于1的索引)
        :type max_row: int

        :param values_only: 是否只返回单元格值
        :type values_only: bool
    ...
    def append(self, iterable):
        ...
        * 如果它是一个列表:所有的值按顺序添加，从第一列开始
        * 如果它是一个字典:值被分配给键(数字或字母)所指示的列。
