...
class Expiration:
		...
		with Session(ENGINE) as session:
			while True:
				try:
					...
					print(e)
	def show_all(self,returnable=False):
		with Session(ENGINE) as session:
	...
	def rm_expo(self):
		...
		if which in [None,'d']:
			...
			with Session(ENGINE) as session:
				for s in select:
					r=session.query(Expiry).filter(Expiry.eid==toRm[s].eid).first()
					if r:
						session.delete(r)
						session.commit()
						session.flush()

	def show_warnings(self,barcode=None):
		with Session(ENGINE) as session:
			if barcode == None:
...
{Fore.light_steel_blue}X/{Fore.light_yellow}Y/{Fore.light_red}Z'''
			headers=f'{htext} -> {Fore.light_green}Name|{Fore.cyan}Barcode|{Fore.light_yellow}Note|{Fore.orange_red_1}EntryId|{Fore.light_magenta}eid|{Fore.light_red}BB_Expiry{Style.reset}'
			print(headers)
			for num,i in enumerate(results):
				...
				if datetime.now() >= i.BB_Expiry:
					counter+=1
					iformat=f'{Fore.light_green}{i.Name}|{Fore.cyan}{i.Barcode}|{Fore.light_yellow}{i.Note}|{Fore.orange_red_1}{i.EntryId}|{Fore.light_magenta}{i.eid}|{Fore.light_red}{i.BB_Expiry}{Style.reset}'
