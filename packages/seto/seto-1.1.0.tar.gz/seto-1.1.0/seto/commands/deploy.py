...
from .config import resolve_internal_networks
...
def execute_deploy_command(args, driver: Driver) -> None:
  ...
  # Docker Swarm
  print('Configuring ṣeto networks...')
  internal_networks = resolve_internal_networks()
  swarm = DockerSwarm(
    stack='seto',
    client=client,
    driver=driver,
    config=internal_networks,
  )
  swarm.deploy()
  ...
  traefik_http_provider_config = {
    'file': traefik_http_provider_filename,
  }

  traefik_http_provider_service = {
    'image': 'httpd:alpine',
  ...
  if composes_items:
    swarm_config['configs'][traefik_http_provider_name] = traefik_http_provider_config
    swarm_config['services']['compose-provider'] = traefik_http_provider_service
