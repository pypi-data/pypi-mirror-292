...
from typing import AsyncIterator, Dict, Deque, Tuple, cast
from urllib.parse import urlparse

import httpx
from aioquic.asyncio import QuicConnectionProtocol, connect
from aioquic.h3.connection import H3Connection, H3_ALPN
from aioquic.h3.events import H3Event, HeadersReceived, DataReceived, Headers
...
async def async_get_http3(url: str, timeout: int):
    parsed = urlparse(url)
    ...
    conf = QuicConfiguration(is_client=True, alpn_protocols=H3_ALPN, idle_timeout=timeout)
    async with connect(host, port, configuration=conf, create_protocol=H3Transport) as transport:
        async with httpx.AsyncClient(transport=cast(httpx.AsyncBaseTransport, transport), timeout=timeout, http1=False) as client:
            return await client.get(url, timeout=timeout, follow_redirects=False)


def get_http3(url: str, timeout: int):
    return asyncio.run(async_get_http3(url, timeout))
