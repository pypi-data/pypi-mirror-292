...
from scipy.linalg.lapack import dlamch


class AB13DDError(Exception):
    pass
...
#      $                   INFO )
def ab13dd(DICO, JOBE, EQUIL, JOBD, N, M, P, FPEAK,
           A, E, B, C, D, TOL):
    pass
# C
# C     PURPOSE
...
#          NODYN  = N.EQ.0 .OR. MIN( BNORM, CNORM ).EQ.ZERO
        NODYN = (N == 0) or (min(BNORM, CNORM) == 0)
...
#       END IF
    if M == 0 or P == 0:
        GPEAK = np.array([ZERO, ONE])
        FPEAK = np.array([ZERO, ONE])
        return GPEAK, FPEAK
...
#          END IF
        if VECT == 'A':
            D_SVD_U, D_SVD_S, D_SVD_VT = spla.svd(D, lapack_driver='gesvd')
        else:
            D_SVD_S = spla.svd(D, compute_uv=False, lapack_driver='gesvd')
...
#       END IF
    if NODYN:
        GPEAK = np.array([GAMMAL, ONE])
        FPEAK = np.array([ZERO, ONE])
        return GPEAK, FPEAK

# C
#       IF ( .NOT.USEPEN .AND. WITHD ) THEN
    if not USEPEN and WITHD:
# C
...
#       BIGNUM = ONE / SMLNUM
#       TOLER  = SQRT( EPS )
    EPS    = dlamch( b'Epsilon' )
    SAFMIN = dlamch( b'Safe minimum' )
    SAFMAX = ONE / SAFMIN
    # from netlib reference implementation:
    if np.log10(SAFMAX) > 2000e0:
       SAFMIN = np.sqrt(SAFMIN)
       SAFMAX = np.sqrt(SAFMAX)
    SMLNUM = np.sqrt( SAFMIN ) / dlamch(b'Precision')
    BIGNUM = ONE / SMLNUM
    TOLER  = np.sqrt(EPS)
...
#      $                N, IERR )
    if ILASCL:
        # This probably does not handle all cases correctly.
        # DLASCL is not available in scipy.
        assert np.isfinite(ANRMTO/ANRM)
        A_CPY *= ANRMTO/ANRM
...
#      $                   DWORK( IE ), N, IERR )
        if ILESCL:
            # This probably does not handle all cases correctly.
            # DLASCL is not available in scipy.
            assert np.isfinite(ENRMTO/ENRM)
            E_CPY *= ENRMTO/ENRM
