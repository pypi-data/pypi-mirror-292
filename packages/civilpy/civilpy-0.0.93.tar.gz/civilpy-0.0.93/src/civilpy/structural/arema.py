...
class CooperE80:
    ...
    """
    series: int = 80
    distances: list[int] = field(default_factory=lambda: [x * units.feet for x in [
        0, 8, 13, 18, 23, 32, 37, 43, 48, 56, 64, 69, 74, 79, 88, 93, 99, 104
    ]])
    magnitudes: list[int] = field(default_factory=lambda: [x * units.kips for x in [
        40, 80, 80, 80, 80, 52, 52, 52, 52, 40, 80, 80, 80, 80, 52, 52, 52, 52
    ]])
    distributed: dict[str] = field(default_factory=lambda: {
    ...
    def __repr__(self):
        return_string = ("\n".join("{}: {}".format(x, y) for x, y in zip(self.distances, self.magnitudes)) + '\n'
            + "{}: {}".format(self.distributed['distance'], self.distributed['magnitude']))

        return return_string

    def __post_init__(self):
        if self.series != 80:
            new_magnitudes = [x * units.kips for x in [
                40, 80, 80, 80, 80, 52, 52, 52, 52, 40, 80, 80, 80, 80, 52, 52, 52, 52
            ]]
