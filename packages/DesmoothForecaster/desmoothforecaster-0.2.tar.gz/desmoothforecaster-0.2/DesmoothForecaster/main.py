...
class DesmoothForecasterModel:
    ...
    def __init__(self, model='LSTM', custom_model=None, smoothing="exp_smoothing", smoothing_param=-1):
        """
    ...
    def predict(self, n_timesteps) -> pd.DataFrame:
        """
        ...
        """
        if self.trainX is None or self.df_for_training is None:
            raise ValueError("Model must be trained before predicting.")

        forecast_period_dates = self.df_for_training.index[-(n_timesteps):]
        forecast = self.model.predict(self.trainX[-n_timesteps:])

        forecast_copies = np.repeat(forecast, self.df_for_training.shape[1], axis=-1)
        ...
        original = self.df_for_training['smoothed'].to_frame()
        ...
        # Slice the original DataFrame to get the last 1% of rows
