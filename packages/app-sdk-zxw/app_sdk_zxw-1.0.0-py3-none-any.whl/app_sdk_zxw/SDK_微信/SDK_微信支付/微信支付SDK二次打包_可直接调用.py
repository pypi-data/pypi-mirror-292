...
"""
import secrets
from app_sdk_zxw.SDK_微信.SDK_微信支付 import minipay
from typing import Optional
import hashlib
import time
from app_sdk_zxw.SDK_微信.SDK_微信支付.返回值模型 import Return生成预支付单, Response查询订单


# 直接传给前端使用
async def 生成预支付单(openid,
                       支付金额=0,
                       商品编号='ZiDingYi',
                       商品描述='景募',
                       设备ID='aaa',
                       商户订单号='自动生成') -> tuple[bool, Optional[Return生成预支付单, str]]:
    """
    直接传给前端使用
    :param openid:
    :param 支付金额:
    ...
    if 商户订单号 == '自动生成':
        ...
        # 商户订单号 = date + '_' + str(random.randint(0, 99999))
        商户订单号 = secrets.token_hex(16)  # 生成16位随机字符串

    # 预下单
    uni = minipay.UnifiedOrder(out_trade_no=商户订单号, body=商品描述, openid=openid,
                               device_info=设备ID, total_fee=支付金额,
                               product_id=商品编号)
...
async def 查询订单(商户订单号: str = 'abv2010102333112') -> tuple[bool, Optional[Response查询订单, str]]:
...
async def 查询退款(商户订单号):
    ...
    if query.is_success:
        ...
        if response["refund_status_0"] == "CHANGE":
            return {"status": "CHANGE", "tip": "退款异常，退款到银行发现用户的卡作废或者冻结了，导致原路退款银行卡失败。"}
...
if __name__ == '__main__':
    # 申请退款(100, 200, "123123")
    xx = 查询退款("2020013006215459919015339")
    print(xx)
