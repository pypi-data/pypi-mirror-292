    ...
    * overrides the ``pop`` method to delete the appropriate key
    ...
    def check(self) -> None:
        ...
        * the ``tag_Value`` attribute must be set.
        ...
        """
        assert self.instance is not None, "The instance attribute must be set before setting tags."
        assert hasattr(self.instance, 'Tags'), \
            f"The {self.instance.__class__.__name__} does not have a Tags attribute."
        assert self.tag_class is not None, "The tag_class attribute must be set before setting tags."
        assert self.tag_Key is not None, "The tag_Key attribute must be set before setting tags."
        assert self.tag_Value is not None, "The tag_Value attribute must be set before setting tags."
    ...
    def __setitem__(self, key: str, value: str) -> None:
        ...
        for tag in instance.Tags:  # type: ignore
            if getattr(tag, cast(str, self.tag_Key)) == key:
                ...
                break
        if not found:
            tag = cast(Type, self.tag_class)(**{self.tag_Key: key, self.tag_Value: value})
            instance.Tags.append(tag)  # type: ignore
...
class TagsDictMixin:
    ...
    def __tag_Key(self) -> str:
        ...
        for candidate in candidates:
            if hasattr(instance, candidate):
                return candidate
        raise RuntimeError(f"Could not find a key attribute in {self.tag_class.__name__}")
    ...
    def __tag_Value(self) -> str:
        ...
        for candidate in candidates:
            if hasattr(instance, candidate):
                return candidate
        raise RuntimeError(f"Could not find a value attribute in {self.tag_class.__name__}")
