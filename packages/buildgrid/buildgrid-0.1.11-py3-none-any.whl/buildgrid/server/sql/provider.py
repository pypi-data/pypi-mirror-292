...
class SqlProvider:
    ...
    Args:
        automigrate (bool): Whether or not to ensure the database is fully
            migrated when starting up. Defaults to ``False``, meaning the
            database is assumed to be at the up to date already.
    ...
    def __init__(
        ...
        *,
        ...
        lock_timeout: int = 5,
        connect_args: Optional[Dict[Any, Any]] = None,
        ...
        if not connection_string:
            ...
            LOGGER.warning(
                "No connection string specified for the SQL provider, "
                f"will use SQLite with tempfile: [{self._database_tempfile.name}]"
            )
        ...
        connection = self._engine.url
        LOGGER.info(f"Created SQL provider with: {automigrate=}, {connection=}")
    ...
    def _create_or_migrate_db(self) -> None:
        ...
        This method runs the Alembic ``upgrade`` command to ensure that all of
        the database migrations have been run and the schema is up to date.

        .. warning::

            Do not run this method concurrently against a single database, at
        ...
        config.set_main_option("script_location", str(files("buildgrid.server.sql").joinpath("alembic")))
    ...
    def session(
        ...
        *,
        ...
        exceptions_to_not_raise_on: Optional[List[Type[Exception]]] = None,
        exceptions_to_not_rollback_on: Optional[List[Type[Exception]]] = None,
        ...
        factory: "Union[scoped_session, sessionmaker[Session]]" = self._session_factory
        if scoped:
