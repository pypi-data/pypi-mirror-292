...
from buildgrid._protos.build.bazel.remote.execution.v2.remote_execution_pb2 import CacheCapabilities, Digest
...
from ...threading import ContextThreadPoolExecutor
...
class WithCacheStorage(StorageABC):
    ...
    def get_blob(self, digest: Digest) -> Optional[IO[bytes]]:
        try:
            ...
            LOGGER.warning(f"Failed to read [{digest}] from cache storage", exc_info=True)
        ...
        try:
            ...
            LOGGER.warning(f"Failed to write [{digest}] to cache storage after reading blob", exc_info=True)
    ...
    def delete_blob(self, digest: Digest) -> None:
        ...
        try:
            ...
            LOGGER.warning(f"Failed to delete [{digest}] from cache storage", exc_info=True)
    ...
    def bulk_delete(self, digests: List[Digest]) -> List[str]:
        ...
        try:
            ...
            for failure in cache_failures:
                LOGGER.warning(f"Failed to delete [{failure}] from cache storage")
    ...
    def commit_write(self, digest: Digest, write_session: IO[bytes]) -> None:
        ...
        try:
            ...
            LOGGER.warning(f"Failed to commit write of [{digest}] to cache storage", exc_info=True)
