...
class MirroredCache(ActionCacheABC):
    ...
    def update_action_result(self, action_digest: Digest, action_result: ActionResult) -> None:
        ...
        LOGGER.info(f"Finished dual write to both action-caches: {text_format.MessageToString(action_digest)}")
    ...
    def get_action_result(self, action_digest: Digest) -> ActionResult:
        ...
        if first_result != second_result:
            publish_counter_metric(METRIC.ACTION_CACHE.MIRRORED_MISMATCH_COUNT, 1)
            LOGGER.warning(
                f"Different action results in mirrored caches, first: {text_format.MessageToString(first_result)}, "
                f"second: {text_format.MessageToString(second_result)}"
            )
            return first_result
...
def _try_get_action_result(cache: ActionCacheABC, digest: Digest) -> Optional[ActionResult]:
    try:
