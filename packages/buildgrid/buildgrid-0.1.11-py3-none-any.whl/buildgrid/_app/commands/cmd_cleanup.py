...
from buildgrid._app.settings.parser import load_config
...
def _create_cleanup_from_config(
    ...
    if cleanup := config.cleanup:
        instance_indexes = {}
        for cleanup_entry in cleanup:
            instance_indexes[cleanup_entry.name] = cleanup_entry.index
        ...
        for instance in config.instances:
            for storage in instance.storages:
                if isinstance(storage, IndexABC):
                    instance_indexes[instance.name] = storage
                    break
            else:
