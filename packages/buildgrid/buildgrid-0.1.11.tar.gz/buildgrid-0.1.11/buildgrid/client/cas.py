...
from typing import IO, BinaryIO, Dict, Iterator, List, Optional, Set, Tuple, TypeVar
...
from buildgrid.settings import BATCH_REQUEST_SIZE_THRESHOLD, HASH, MAX_REQUEST_COUNT, MAX_REQUEST_SIZE
...
class Downloader:
    ...
    def __init__(
        ...
        self.__bytestream_stub: Optional[ByteStreamStub] = ByteStreamStub(self.channel)
        self.__cas_stub: Optional[ContentAddressableStorageStub] = ContentAddressableStorageStub(self.channel)
    ...
    def get_blob(self, digest: Digest) -> Optional[bytearray]:
    ...
    def _fetch_blob_batch(
        self, digests: List[Digest], *, skip_unavailable: bool = False
    ) -> List[Tuple[bytes, Digest]]:
    ...
    def _write_directory(
        self, root_directory: Directory, root_path: str, directories: Optional[Dict[str, Directory]] = None
    ) -> None:
...
class Uploader:
    ...
    def put_blob(
        self, blob: IO[bytes], digest: Optional[Digest] = None, queue: bool = False, length: Optional[int] = None
    ) -> Digest:
    ...
    def put_message(self, message: Message, digest: Optional[Digest] = None, queue: bool = False) -> Digest:
    ...
    def _send_blob(self, blob: BinaryIO, digest: Optional[Digest] = None) -> Digest:
    ...
    def _queue_blob(self, blob: bytes, digest: Optional[Digest] = None) -> Digest:
