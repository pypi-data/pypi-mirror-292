...
class Table:
    ...
    def drop(self):
        drop_table_query = f"DROP TABLE IF EXISTS {self.name}"
        print(f"Running query: {drop_table_query}")
        self.cursor.execute(drop_table_query)
        self.connection.commit()
        self.table_created = False

    def add_column(self, name, column_type, **options):
        ...
        if name in existing_columns:
            print(f"Column {name} already exists in {self.name}. Skipping add column.")
            return
        ...
        add_column_query = f"ALTER TABLE {self.name} ADD COLUMN {' '.join(column_def)}"
        print(f"Running query: {add_column_query}")
        ...
        if self.unique_columns:
            ...
            if unique_check_conditions:
                ...
                if existing_record:
                    ...
                    return  # Skip insert if record already exists
    ...
    def select(self, columns='*', where=None, like=None, order_by=None, order_direction='ASC', limit=None):
        ...
        print(f"Running query: {query}")  # Debugging: megjeleníti a SQL lekérdezést
    ...
    def delete(self, **kwargs):
        if kwargs:
            ...
            delete_query = f"DELETE FROM {self.name} WHERE {' AND '.join(conditions)}"
            print(f"Running query: {delete_query}")  # Debugging: megjeleníti a SQL lekérdezést
            self.cursor.execute(delete_query, parameters)
        else:
    ...
    def update(self, where_clause, **new_values):
        ...
        self.cursor.execute(update_query, tuple(new_values.values()) + tuple(parameters))
        self.connection.commit()
