...
class MetricFinder:
    ...
    def find_metric(self, point1, point2):
        ...
        if isinstance(point1, str) and isinstance(point2, str):
            return self.find_string_metric(point1, point2)
            ...
            if all(isinstance(x, (int, float)) for x in point1) and all(isinstance(x, (int, float)) for x in point2):
                return self.find_numeric_metric(point1, point2)
            elif all(isinstance(x, int) for x in point1) and all(isinstance(x, int) for x in point2):
    ...
    def find_string_metric(self, str1, str2):
        ...
        return "Ratcliff Obershelp Distance"

    def find_numeric_metric(self, point1, point2):
        """
        Determines the appropriate numeric-based metric.

        Parameters:
        point1: The first numeric point (list or tuple of numbers).
        point2: The second numeric point (list or tuple of numbers).

        Returns:
        str: The name of the most appropriate numeric metric.
        """
        if len(point1) == len(point2):
            if len(point1) == 2 or len(point1) == 3:
                return "Euclidean Distance"
            elif len(point1) > 3:
                return "Mahalanobis Distance"
            elif len(point1) > 1:
                return "Manhattan Distance"
        return "Unknown Numeric Metric"
