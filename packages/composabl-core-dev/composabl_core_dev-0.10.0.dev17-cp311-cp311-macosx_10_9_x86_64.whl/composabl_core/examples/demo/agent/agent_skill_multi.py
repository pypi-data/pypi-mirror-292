...
agent_dictionary.add_selector_skill(
    target_skill_dictionary,
    [expert_skill_controller_dict, random_skill_controller_dict],
    fixed_order=True,
...
agent_box.add_selector_skill(
    target_skill_box,
    [expert_skill_controller_box, random_skill_controller_box],
    fixed_order=True,
    fixed_order_repeat=False
)
...
agent_discrete.add_selector_skill(
    target_skill_discrete,
    [expert_skill_controller_discrete, random_skill_controller_discrete],
    fixed_order=True,
    fixed_order_repeat=False
)
...
agent_multidiscrete.add_selector_skill(
    target_skill_multi_discrete,
    [expert_skill_controller_multi_discrete, random_skill_controller_multi_discrete],
    fixed_order=True,
    fixed_order_repeat=False
)
...
agent_tuple.add_selector_skill(
    target_skill_tuple,
    [expert_skill_controller_tuple, random_skill_controller_tuple],
    fixed_order=True,
    fixed_order_repeat=False
)
...
with SkillSelector("target_skill_dictionary", TeacherSpaceDictionary) as ss1:
    with Skill("expert-controller", ControllerExpertDict) as expert_skill_controller_dict:
        ss1.add_skill(expert_skill_controller_dict)

    with Skill("random-controller", ControllerRandomDict) as random_skill_controller_dict:
        ss1.add_skill(random_skill_controller_dict)

    agent_dictionary_api_context_manager.add_sensors(mapped_sensors_dict["dictionary"])
    agent_dictionary_api_context_manager.add_selector_skill(ss1, fixed_order=True, fixed_order_repeat=False)
