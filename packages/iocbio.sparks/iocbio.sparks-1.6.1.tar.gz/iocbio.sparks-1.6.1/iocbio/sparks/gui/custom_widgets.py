...
class SparksListView(QWidget):
    sigActiveSpark = Signal(str)
    sigSparkRemove = Signal(str)
...
class ImageIntensitySetter(QWidget):
    sigValueChanged = Signal(int, int)  # emits min and max value
...
class ImageAdjustments(QWidget):
    def __init__(self, label, vmin, vmax, levels_range, manualtextsize, lut_name):
        ...
        self.image_levels = ImageIntensitySetter(
        ...
        self.spark_levels = ImageIntensitySetter(
            label="Set spark image intesity levels",
            vmin=vmin,
            vmax=vmax,
            levels_range=levels_range,
            manualtextsize=manualtextsize,
...
class ParametersSetter(QWidget):
    sigParametersChanged = Signal(str, dict, dict, dict)
...
class StageField(QWidget):
    sigRemoveRequested = Signal(str)
    sigNameChanged = Signal(str, str)
    ...
    def set_data(self, data):
        ...
        for i, (name, d) in enumerate(data.items()):
            ...
            curve = pg.PlotCurveItem(
                d["x"], d["y"], name=name, pen={"color": colors[ci], "width": 2, "sytle": linetypes[li]}
            )
...
class XYPlotRange(XYPlot):
    sigAnalysisRegionChange = Signal(tuple)
    sigAnalysisRegionChangeFinished = Signal(tuple)
...
class EmptyContextMenuViewBox(pg.ViewBox):
    def __init__(
        self, parent=None, border=None, lockAspect=False, enableMouse=True, invertY=False, enableMenu=True, name=None
    ):
