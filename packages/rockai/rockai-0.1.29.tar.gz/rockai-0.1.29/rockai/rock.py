...
def build(
    ...
    file: Annotated[str, typer.Option(help="Path to predictor.py file")] = None,
    gpu: Annotated[bool, typer.Option(help="Is using gpu")] = True,
    platform: Annotated[
        str, typer.Option(help="docker image supported platform, `linux/amd64` by default, you can also change it to other platform like `linux/arm64` ")
    ] = "linux/amd64",
):
    """
    Build the image
    ...
    if name is not None and file is not None:
        ...
        build_docker_image_without_configuration(name, file, port, gpu, platform)
