...
def create_app(path: Path, auth: str = None) -> MyFastAPI:
    ...
    def predic_with_id(
        ...
        end_time = datetime.now()
        time_taken = (end_time - start_time).total_seconds()
        final_result = upload_files(result, upload_file=lambda fh: upload_file(fh, ""))
        logger.debug(result)
...
def start_server(port, auth=None):
    app = create_app(path=Path.cwd(), auth=auth)
