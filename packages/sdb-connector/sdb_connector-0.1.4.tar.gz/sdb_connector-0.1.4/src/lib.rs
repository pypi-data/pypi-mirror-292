...
//use polars::py_modules::DataFrame as PyPolarsDataFrame;
...
async fn select_data_with_db_connect_async_exe(ip: &str, port: &str,
     user: &str, pw:&str, namespace: &str, db_name: &str,
     table_name: &str, run_id: &str) -> Result<Vec<(u16, u64, u8, u16)>, Box<dyn Error>> {
    ...
    let result_query = format!("SELECT run_counter, len_trigger, channel, peak FROM {} WHERE run_id = {} ORDER BY run_counter ASC", table_name, run_id);
    let result = db.query(&result_query).await?;
    let mut ddata: surrealdb::Response = result;
    ...
    let exploded_data: Vec<(u16, u64, u8, u16)> = data
        ...
        .collect();
    //println!("exploded_data: {:?}", exploded_data);
    Ok(exploded_data)
}
