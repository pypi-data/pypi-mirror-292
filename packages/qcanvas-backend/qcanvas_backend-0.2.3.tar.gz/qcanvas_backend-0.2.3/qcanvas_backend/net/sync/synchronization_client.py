...
from qcanvas_backend.net.sync._canvas_data_bundle import CanvasDataBundle
from qcanvas_backend.net.sync._new_content_collector import NewContentCollector
from qcanvas_backend.net.sync._sync_meta import SyncMeta
...
class SynchronizationClient:
    ...
    async def _fetch_remote_sync_data(
        ...
        await asyncio.gather(get_mail(), get_announcements())
    ...
    async def _get_announcements_for_courses(
        ...
        with register_reporter(
            CompoundTaskReporter(SYNC_GOAL, "Fetch announcements", len(courses))
        ) as prog:
