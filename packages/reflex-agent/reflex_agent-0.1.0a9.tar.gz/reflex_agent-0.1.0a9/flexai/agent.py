...
class Agent:
    ...
    async def invoke_tool(self, message: ToolUseMessage) -> ToolResultMessage:
        ...
        """
        # Load the params.
        tool = self.toolbox[message.tool_name]

        # Invoke the tool, time it, and return the result or the exception.
        start_time = time.time()
        try:
            if asyncio.iscoroutinefunction(tool.fn):
                result = await tool.fn(**message.input)
            else:
                result = tool.fn(**message.input)
        except Exception as e:
            result = e
        end_time = time.time()

        return ToolResultMessage.from_result(
            message.id, result, execution_time=end_time - start_time
        )

    async def step(self, messages: list[Message]) -> list[AIMessage]:
        ...
        responses = await self.llm.get_chat_response(
            messages, system=system, tools=list(self.toolbox.values())
        )

        # Return the tool use message.
        for i in range(len(responses)):
            responses[i] = await self.modify_response(messages, responses[i])

        # Base case: no tool uses suggested at all
        if (
            isinstance(responses[0], ToolUseMessage)
            and responses[0].tool_name == "send_message"
        ):
            return [AIMessage(content=responses[0].input["message"])]
