...
def _customise_third_party_loggers() -> None:
    ...
    warning_handler.addFilter(is_not_pytorch_lightning_warning)  # type: ignore[arg-type] # Reason: see message # noqa: B950
    ...
    _configure_third_party_logger(
        ...
        log_level=LOG_LEVEL if LOG_HTTPCORE else logging.WARNING,
    )
    _configure_third_party_logger(
        module="httpx",
        logger_name="httpx",
        log_level=LOG_LEVEL if LOG_HTTPX else logging.WARNING,
    )
    _configure_third_party_logger(
        module="matplotlib",
        logger_name="matplotlib",
        log_level=LOG_LEVEL if LOG_MATPLOTLIB else logging.WARNING,
    )
    _configure_third_party_logger(
        module="urllib3.connectionpool",
        logger_name="urllib3.connectionpool",
        log_level=LOG_LEVEL if LOG_URLLIB3 else logging.WARNING,
    )
    _configure_third_party_logger(
        module="warnings",
...
def _get_bitfount_console_handler(
    ...
    console_log_formatter = logging.Formatter(LOG_FORMAT, datefmt=LOG_DATE_FORMAT)
...
SAMPLE_LOGGING_INTERVAL: Final = 100
...
class SampleFilter(logging.Filter):
    ...
    def __init__(
        ...
        self._sample_counter: Counter_T[Tuple[str, int]] = Counter()
