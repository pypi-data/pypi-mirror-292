...
class _Worker:
        ...
        if (optional_con := self._get_project_db_con_if_allowed(protocol)) is not None:
            with closing(optional_con) as project_db_con:
                try:
                    ...
                    cur.execute(
                        f"""CREATE TABLE IF NOT EXISTS "{self._task_hash}-v2" ({columns_and_types})"""  # noqa: B950
                    )
                    project_db_con.commit()
                except Exception as e:
                    logger.error(
                        f"Encountered error whilst creating task hash table in database: {e}"  # noqa: B950
                    )
    ...
    def _load_new_records_only_for_task(self, project_db_con: Connection) -> None:
        ...
        run_records = pd.read_sql(
            f'SELECT * FROM "{self._task_hash}-v2"',  # nosec hardcoded_sql_expressions # noqa: B950
            project_db_con,
        )
