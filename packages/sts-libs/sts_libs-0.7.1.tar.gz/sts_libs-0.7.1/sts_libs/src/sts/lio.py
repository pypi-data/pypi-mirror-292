...
class Loopback(Targetcli):
    def __init__(self, target_wwn: str | None = None) -> None:
        ...
        self.target_path = f'{self.loopback_path}{target_wwn}/'
        super().__init__(path=self.target_path)
...
def create_basic_iscsi_target(
    ...
    if not check_result(IscsiLUNs(target_wwn=target_wwn).create_lun(storage_object=backstore.path)):
        return False
