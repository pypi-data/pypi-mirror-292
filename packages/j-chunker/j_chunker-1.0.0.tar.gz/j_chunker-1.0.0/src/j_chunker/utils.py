...
# Local
from .llm import initialize_model, process_chunk_with_llm
...
def clean_chunk(text: str) -> str:
    ...
    def is_japanese(char):
        return any([
            'CJK UNIFIED IDEOGRAPH' in unicodedata.name(char, ''),  # Kanji
            'HIRAGANA' in unicodedata.name(char, ''),
            'KATAKANA' in unicodedata.name(char, ''),
            'HALFWIDTH KATAKANA' in unicodedata.name(char, ''),
            ord(u'\u3000') <= ord(char) <= ord(u'\u303f'),  # Japanese-style punctuation
        ])
...
def process_and_clean_chunks(chunks: List[Dict], pdf_name: str, output_file: Path, max_words: int, llm_summarize: bool = False, llm=None) -> List[Dict]:
    """
    ...
    Args:
        ...
        llm: Initialized LLM model (if llm_summarize is True).
    ...
    for i, chunk in enumerate(chunks, 1):
        if chunk['content'].strip():
            ...
            if llm_summarize:
                if llm is None:
                    raise ValueError("LLM model is not initialized but llm_summarize is True")
                processed_chunk = process_chunk_with_llm(chunk, llm, max_words)
                time.sleep(0.25)  # Add delay only when using LLM
            else:
                processed_chunk = {
                    'content': chunk['content'],
                    'cluster': int(chunk['cluster']),
            ...
            cleaned_text = clean_chunk(processed_chunk['content'])
            logging.info(f"Processed and cleaned chunk: {cleaned_text[:100]}...")
            processed_chunks.append({
                "chunk_id": i,
                "cluster": processed_chunk['cluster'],
                "content": cleaned_text,
                "pages": processed_chunk['pages'],
                "pdf_name": pdf_name
            })
        else:
