...
mumu_hwnd = win32gui.FindWindow(None, "MuMu模拟器12")
...
def 点击_后台版(坐标: pag.Point, 点击前延时: float = 0, 点击后延时: float = 0):
    mumu = win32gui.FindWindow("Qt5156QWindowIcon", "MuMu模拟器12")
    mumuplayer = win32gui.FindWindowEx(mumu, 0, "Qt5156QWindowIcon", "MuMuPlayer")
    print(mumu, mumuplayer)
    x, y = 坐标
    ...
    win32api.PostMessage(
        mumuplayer, win32con.WM_LBUTTONDOWN, win32con.MK_LBUTTON, position
    )  # 向窗口发送模拟鼠标点击
    win32api.PostMessage(
        mumuplayer, win32con.WM_LBUTTONUP, 0, position
    )  # 模拟释放鼠标左键

    pag.sleep(点击后延时)


def 点击链(坐标元组: tuple[pag.Point], 点击前延时: float = 0, 点击后延时: float = 0):
    for 坐标 in 坐标元组:
        点击_后台版(坐标, 点击前延时, 点击后延时)
...
def 获取图片坐标(
    ...
    try:
        ...
        坐标 = pag.locate(图片, mumu截图, confidence=图片相似度, region=识别区域)
...
def 点击图片(
    ...
    点击_后台版(xy, 点击前延时=点击前延时, 点击后延时=点击后延时)
