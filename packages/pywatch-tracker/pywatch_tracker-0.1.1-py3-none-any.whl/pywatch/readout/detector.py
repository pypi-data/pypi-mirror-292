...
class Detector:
    ...
    async def open(self) -> "Detector":
        """Open the serial port connection. Restarts the detector.
    ...
    def start_time(self) -> int:
        """Get the computer time when the detector was last opened."""
    ...
    def rate(self) -> float:
        """
        Get the number of hits per time interval of the detector. Calculates
        the number by using the data that was registered after the last time
        the detector was opened.

        :returns: :py:class:`float`
        """
