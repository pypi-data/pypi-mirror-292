...
def start(owner_key_id, host: str, port: int, target_key: str, target_enc_key: str, wait: bool) -> None:
    ...
    if wait:
        ...
        for _ in range(3):
            # First is some random genesis, 2nd is broadcast, 3rd data
...
@click.argument("target-key", type=str, required=False,
                default="Ah70wYetFbhu9X/nrFQsFXTW/3kE4GKeRfhQ+1os1rcL")  # "Target public signing key"
@click.argument("target-enc-key", type=str, required=False, default=None)  # "Target encryption key"
def test(owner_key_id, owner_key_enc_id, target_key: str, target_enc_key: str) -> None:
...
@click.argument("port", type=int)
@click.argument("target-key", type=str, required=False,
                default="Ah70wYetFbhu9X/nrFQsFXTW/3kE4GKeRfhQ+1os1rcL")
def chat(owner_key_id, host: str, port: int, target_key: str) -> None:
    beacon = Beacon(host, port, owner_key_id)
    client_str = f"Identity {beacon.pub_key}\nConnecting to:\nHost: {host}\nPort: {port}\nTargeting: {target_key}"
    rich.print(Panel(client_str, title="Client Info"))
    ...
    rich.print("Waiting for connection...")
    beam.fetch_message()

    def receive_still():
        while True:
            block, incomm_data, _ = beam.fetch_message()
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            rich.print(
                f"\n [yellow][{current_time}] [/yellow][red]{incomm_data['pub']} >>>[/red]: {block.data.decode('utf-8')}")

    Thread(target=receive_still, daemon=True).start()

    while True:
        # Read user input
        user_input = Prompt.ask(f"[bold magenta] Me@[gray]{beacon.pub_key}[/gray] >>>")
        if not user_input:
            continue
        ...
        # Send the message using the beam instance
        beam.send_communication_data(user_input.encode("ascii"))
