...
class TCPServer:
    ...
    @staticmethod
    def handle_client(pipe: multiprocessing.Pipe, beam_pipe_comm: dict):
        """
        ...
        :return:
        """
        share_bytes = pipe.recv()
        pipe.close()  # Release resources, nothing else will come from pipe TODO: send signal to kill through pipe
        client_socket = socket.fromshare(share_bytes)

        logger.debug(f"Socket reconstructed {client_socket}")
    ...
    def run(self):
        ...
        while True:
            ...
            client_process = multiprocessing.Process(target=TCPServer.handle_client,
                                                     ...
                                                     args=(
                                                         child_conn, beam_pipe_comm))
            client_process.start()

            share_bytes = client_socket.share(client_process.pid)
            parent_conn.send(share_bytes)
            parent_conn.close()
