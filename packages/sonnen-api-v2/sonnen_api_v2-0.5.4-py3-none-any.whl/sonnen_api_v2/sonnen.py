...
class Sonnen:
    ...
    def fetch_powermeter(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self._powermeter_consumption_grid = self._powermeter_data[1]
                self._powermeter_consumption_battery = self._powermeter_data[2]
                return True
        except requests.ConnectionError as conn_error:
            self._log_error(f'Connection error to battery system - {conn_error}')
        return False
    ...
    @get_item(float)
    def kwh_consumed_battery(self) -> float:
        """Consumed kWh"""
        return self._powermeter_consumption_battery[self.POWERMETER_KWH_CONSUMED]

    @get_item(float)
    def kwh_consumed_grid(self) -> float:
        """Consumed kWh"""
        return self._powermeter_consumption_grid[self.POWERMETER_KWH_CONSUMED]
