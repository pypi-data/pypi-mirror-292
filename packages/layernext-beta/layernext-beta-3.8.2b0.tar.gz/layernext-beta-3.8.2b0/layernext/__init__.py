...
class LayerNextClient:
    ...
    def get_file_details(self, unique_name: str, chat_id: str, fields_filter: dict = None):
        _datalake_client = datalake.DatalakeClient(
            self.encoded_key_secret, self.layernext_url
        )
    ...
    def __prepare_doc_elements_list(self, filtered_doc_list: list,  document_elem_dict: dict, chat_id:str):
        output_list = []
        ...
        for doc in filtered_doc_list:
            ...
            if "namedEntities" not in doc:
                metadata_obj = self.get_file_details(doc["uniqueName"], {'name': True, 'namedEntities': True},chat_id)
    ...
    def query_documents_with_structure(self, document_categories: List[str], document_keywords: List[str], 
                                          user_question: str, chat_id, page_index=0, page_size=10):
        """
        ...
        document_element_dict = self.find_elements(unique_name_list=unique_names, user_question=user_question, chat_id=chat_id)

        output_list = self.__prepare_doc_elements_list(filtered_doc_list, document_element_dict, chat_id)
        logger.info(
            f" sdk | query_documents_with_structure | request_id <<<< {request_id}, response: {output_list}"
        )
        return output_list
    
    def get_documents_with_structure(self, doc_unique_name_list: list, user_question: str, chat_id: str):
        """
        ...
        document_element_dict = self.find_elements(unique_name_list=doc_unique_name_list, user_question=user_question, chat_id=chat_id, is_target_documents_available=True)
    ...
    def extract_document_elements_keyInfo(self, document_elements_dict: dict, extraction_keys: List[dict], user_question: str, chat_id: str):
        ...
        response = _datalake_client.extract_doc_content(document_elements_dict, extraction_keys, user_question,chat_id)
    ...
    def find_elements(self, unique_name_list: List[str], user_question: str, chat_id: str, is_target_documents_available: bool = False):
        """
