...
from app_tools_zxw.SDK_支付宝.models import 退款查询返回结果pydantic, 订单查询pydantic, callback支付结果pydantic
...
class 支付服务:
    ...
    def 查询订单(self, 商户订单号: str) -> Union[str, bool]:
        model = AlipayTradeQueryModel()
        model.out_trade_no = 商户订单号

        request = AlipayTradeQueryRequest(biz_model=model)
        response = self.alipay_client.execute(request)
        print("订单查询结果 = ", response)
        result = 订单查询pydantic(**response.to_dict())
        if result.alipay_trade_query_response.msg == "Success":
    ...
    def 退款查询(self, 商户订单号: str) -> bool:
        ...
        response = self.alipay_client.execute(request)
        result = 退款查询返回结果pydantic(**response.to_dict())
    ...
    def 注册回调接口(self, app: FastAPI, async_func_支付成功):
        ...
        async def 回调(postBody: Request):
            ...
            if success and dataDict.get("trade_status") in ("TRADE_SUCCESS", "TRADE_FINISHED"):
                ...
                return await async_func_支付成功(dataPydantic)
            else:
