...
from yta_general_utils.file_processor import get_project_abspath, file_exists, is_folder, is_file, is_abspath, create_file_abspath, delete_file
...
class GoogleOauthAPI:
    ...
    def __get_credentials(self, scopes: list):
        return Credentials.from_authorized_user_file(self.client_secret_filename, scopes)
    ...
    def __write_credentials_to_json(self, credentials, api_name: str, api_version: str, scopes: str):
        with open(self.__get_token_filename(api_name, api_version, scopes), 'w') as token_file:
    ...
    def __refresh_oauth_token(self, api_name: str, api_version: str, scopes: list):
        ...
        the access.
        """
        creds = self.__get_credentials(scopes)

        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                creds.refresh(Request())
                # Credentials .token and .expiry are updated
                self.__write_credentials_to_json(creds, api_name, api_version, scopes)
            else:
    ...
    def create_service(self, api_name: str, api_version: str, scopes: list):
        ...
        try:
            service = build(api_name, api_version, credentials = self.__get_credentials(scopes), static_discovery = False)
