...
from gp_cross_factor._program import _Program
from gp_cross_factor.utils import _partition_estimators, check_random_state
from gp_cross_factor.fitness import _fitness_map, _Fitness, _extra_map
from gp_cross_factor.functions import _function_map, _Function, sig1 as sigmoid
from gp_cross_factor.more_functions import _extra_function_map
...
def _parallel_evolve_3D(n_programs, serialized_parents, X, y, sample_weight, seeds, params):
    """Private function used to build a batch of programs within a job."""
    n_dates, n_features, n_stocks = X.shape
    if serialized_parents is not None:
        parents = dill.loads(serialized_parents)
    else:
        parents = None
    ...
    def fit_3D(self, X, y, baseline, sample_weight=None, need_parallel = True):
        ...
        for gen in range(prior_generations, self.generations):
            ...
            if gen == 0:
                ...
                serialized_parents = dill.dumps(parents)

            # Parallel loop
            seeds = random_state.randint(MAX_INT, size=self.population_size)
            if need_parallel:
                # Use loky as the backend for joblib
                with parallel_backend('loky',n_jobs = n_jobs,verbose=int(self.verbose > 1)):
                    population = Parallel()(delayed(_parallel_evolve_3D)(n_programs[i],serialized_parents,X,y,sample_weight,seeds[starts[i]:starts[i + 1]],params) for i in range(n_jobs)) 
            else:
                population = []
                for i in range(n_jobs):
                    population.append(_parallel_evolve_3D(n_programs[i], serialized_parents, X, y, sample_weight, seeds[starts[i]:starts[i + 1]],params))
            ...
            def _process_program(program, X, baseline, satisfied_programs_last_day_df): # penalize highly correlated programs and save satisfied programs dfs
                last_day_series = pd.Series(program.execute_3D(X)[-1])
                program_corr_with_satisfied = 0
                if satisfied_programs_last_day_df is not None:
                    program_corr_with_satisfied = satisfied_programs_last_day_df.corrwith(last_day_series,method='spearman').abs().max()

                program.fitness_ = program.fitness(parsimony_coefficient)
                if program_corr_with_satisfied > 0.8:
                    program.fitness_ = (0.6 + 1 - program_corr_with_satisfied) * program.fitness_

                if program.raw_fitness_ > baseline: 
                    return program,last_day_series # Save satisfied programs' last day series if fitness > baseline
                else:
                    return program, None
            
            if need_parallel:
                with parallel_backend('loky',n_jobs = n_jobs,verbose=int(self.verbose > 1)):
                    results = Parallel()(delayed(_process_program)(program, X, baseline, self._satisfied_factor_last_day_dfs) for program in population)
            else:
                results = []
                for program in population:
                    results.append(_process_program(program, X, baseline, self._satisfied_factor_last_day_dfs))
                
            population = [result[0] for result in results]
            satisfied_programs_last_day_series_list = [result[1] for result in results if result[1] is not None]

            if self._satisfied_factor_last_day_dfs is None:
                if len(satisfied_programs_last_day_series_list) > 0:
                    self._satisfied_factor_last_day_dfs = pd.concat(satisfied_programs_last_day_series_list, axis=1)
            else:
                if len(satisfied_programs_last_day_series_list) > 0:
                    self._satisfied_factor_last_day_dfs = pd.concat([self._satisfied_factor_last_day_dfs] + satisfied_programs_last_day_series_list, axis=1)
            
            def _reduce_dataframe_by_correlation(df, threshold=0.95):
                '''Simply drop one of the highly correlated features'''
                corr_matrix = df.corr().abs()
                
                # initialize set to keep track of features to keep
                kept_features = set()
                to_drop = []
                ...
                for i in range(corr_matrix.shape[0]):
                    for j in range(i + 1, corr_matrix.shape[1]):
                        if corr_matrix.iloc[i, j] > threshold:
                            # 如果两个特征高度相关，选择保留其中一个
                            if corr_matrix.columns[i] not in kept_features:
                                kept_features.add(corr_matrix.columns[i])
                                to_drop.append(corr_matrix.columns[j])
                            else:
                                kept_features.add(corr_matrix.columns[j])
                                to_drop.append(corr_matrix.columns[i])

                to_drop = list(set(to_drop))
                return df.drop(columns=to_drop)
            
            self._satisfied_factor_last_day_dfs = _reduce_dataframe_by_correlation(self._satisfied_factor_last_day_dfs) if self._satisfied_factor_last_day_dfs is not None else self._satisfied_factor_last_day_dfs # Remove highly correlated factors in the satisfied factor list for next generation
