...
def binarize(data: tf.Tensor, token_dim: int, input_dim: int) -> tf.Tensor:
...
def _embedder_factory(token_dim: int, input_dim: int, binary: bool=True) -> callable:
    ...
    __binarize = functools.partial(binarize)
    # embed all
...
def postprocess(prediction: tf.Tensor, token_dim: int, input_dim: int=0x40000, binary: bool=True, random: bool=False) -> tf.Tensor:
    if binary: # values encoded as binary arrays
        __output = mlable.ops.divide(prediction, input_axis=-2, output_axis=-1, factor=token_dim, insert=True)
