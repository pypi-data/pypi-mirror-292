...
def api_call(url: str, method: str, return_type: BaseModel, schema_class: BaseModel):
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if method.lower() == "get":
                ...
                response = self.session.post(url, json=input_data.model_dump(
                    mode="json", exclude_unset=True, exclude_none=True))
            elif method.lower() == "put":
                response = self.session.put(url, json=input_data.model_dump(
                    mode="json", exclude_unset=True, exclude_none=True))
