...
def run(args):
    ...
    #     try:
    #         endpoint = f"http://api.thundercompute.com:8080/next_id"
...
def status():
    token = get_token()
    instances = utils.get_instances(token)
...
def create():
    token = get_token()
    if utils.create_instance(token):
        click.echo(
        ...
        click.echo(
            click.style(
                f"⛔ Failed to create thunder compute instance",
                fg="red",
            )
        )
...
def delete(instance_id):
    token = get_token()
    if utils.delete_instance(instance_id, token):
        click.echo(
        ...
        click.echo(
            click.style(
                f"⛔ Failed to delete thunder compute instance {instance_id}",
                fg="red",
            )
        )
...
def start(instance_id):
    token = get_token()
    if utils.start_instance(instance_id, token):
        click.echo(
        ...
        click.echo(
            click.style(
                f"⛔ Failed to start thunder compute instance {instance_id}",
                fg="red",
            )
        )
...
def stop(instance_id):
    token = get_token()
    if utils.stop_instance(instance_id, token):
        click.echo(
        ...
        click.echo(
            click.style(
                f"⛔ Failed to stop thunder compute instance {instance_id}",
                fg="red",
            )
        )
