...
class SQLiteS3Backend(BackendBase, IndexingBackendMixin, SnapshottingBackendMixin):
    ...
    async def _gather_paths_to_update(
        ...
        return paths_to_update[:max_paths]
    ...
    async def list_apps(
        ...
        for application in applications:
            print(application.updated_at, application.created_at)
            last_written = (
            ...
            out.append(
                schema.ApplicationSummary(
                    ...
                    num_steps=application.logfile_count if application.logfile_count is not None else 0,
                    tags={},
                )
            )
        return out
