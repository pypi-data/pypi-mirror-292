...
def query_list(queryrange: int, limit: int, start: int, end: int, explicit: bool, mode: str, cdxbackup: str, cdxinject: str):
    ...
    def query(queryrange, limit, start, end, explicit):
        ...
        try:
            with requests.get(cdxQuery, stream=True) as r:
                r.raise_for_status()
                cdxResult = []
                with tqdm(unit='B', unit_scale=True, desc='-----> Downloading CDX result', ascii="░▒█") as pbar:
                    for chunk in r.iter_content(chunk_size=8192):
                        if chunk:
                            cdxResult.append(chunk)
                            pbar.update(len(chunk))
                cdxResult = b''.join(cdxResult).decode('utf-8')
    ...
    if not cdxResult:
        cdxResult = query(queryrange, limit, start, end, explicit)
    try:
        cdxResult = json.loads(cdxResult)
    except json.JSONDecodeError:

        vb.write(message="\n!!!!! ERROR")
        vb.write(message="Could not parse CDX result - response arrived partially.\nNot all available snapshots have been received.")
        vb.write(message=f"\n-----> aprox. {count_cdxfile(cdxinject):,} snapshots received")
        vb.write(message="\nPress ANY key to continue...")

        countdown = threading.Thread(target=input_countdown)
        countdown.start() 
        ...
        try:
            cdxResult = cdxResult[:cdxResult.rfind("\"],")+1] + "]]"
            cdxResult = json.loads(cdxResult)
        except:
            vb.write(message="\nERROR: Could not parse partial data - exiting...\n")
            os._exit(1)

    snapshot_count = len(cdxResult)
