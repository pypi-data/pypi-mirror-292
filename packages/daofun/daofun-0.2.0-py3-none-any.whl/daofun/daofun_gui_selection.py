...
from daofun.misc_tools import check_file, read_lst
...
def refine_psf(in_fits, in_lst, out_lst, fits_suffix=""):
    with plt.style.context('dark_background'):
        ...
        star_list_column = [
            [
                sg.Listbox(
                    values=[f"[✓] {int(row.ID):5d} - mag: {row.MAG:2.1f}" for i, row in lst.sort_values("MAG").iterrows()],
        ...
        graph_viewer_column = [
            ...
            [sg.Button("Add", button_color="green"), sg.Button("Discard", button_color="red")],
        ]
        ...
        window.bind("<comma>", "key_comma")
        window.bind("<period>", "key_dot")
        ...
        def rotate_projection(invert=False):
            if not invert:
                graph_ax.azim += 5
                graph_canvas.draw()
            else:
        ...
        def update_list(dropped_stars_ids):
            rows = [f"[✓] {int(row.ID):5d} - mag: {row.MAG:2.1f}"
                    if row.ID not in dropped_stars_ids
                    else f"[x] {int(row.ID):5d} - mag: {row.MAG:2.1f}"
                    for i, row in lst.sort_values("MAG").iterrows()
                    ]
            window["-STAR LIST-"].update(rows)
            window["-TBOT-"].update(saving_text(lst.shape[0]-len(dropped_stars_ids)))
        ...
        while True:
            ...
            if event=="OK":
                ...
                if event.strip("-STAR LIST-") in ["up_arrow", "down_arrow"]:
                    ...
                    sel_star_id = int(opt_list[next_index].split()[1])
                else:
                    sel_star_id = int(values["-STAR LIST-"][0].split()[1])
                [fig.axes[0].collections.pop() for i in range(len(fig.axes[0].collections))]
                canvas_update_star(sel_star_id)

            elif event in ["left_arrow", "Add", 
                            "right_arrow", "Discard"] and len(values["-STAR LIST-"]):
                if not len(window["-STAR LIST-"].get_indexes()):
                        continue
                sel_star_id = int(values["-STAR LIST-"][0].split()[1])
                actual_index = window["-STAR LIST-"].get_indexes()[0]
                opt_list = window["-STAR LIST-"].get_list_values()
                next_index = (actual_index+1)%len(opt_list)
                if event in ["left_arrow", "Add"] and sel_star_id in dropped_stars_ids:
                    dropped_stars_ids.remove(sel_star_id)
                elif event in ["right_arrow", "Discard"] and sel_star_id not in dropped_stars_ids:
                    dropped_stars_ids.append(sel_star_id)
                update_list(dropped_stars_ids)
                window["-STAR LIST-"].update(set_to_index=[next_index], scroll_to_index=next_index)
                [fig.axes[0].collections.pop() for i in range(len(fig.axes[0].collections))]
                next_sel_star_id = int(opt_list[next_index].split()[1])
                canvas_update_star(next_sel_star_id)

            elif event == 'Save':
                selected_stars = lst[~lst.ID.isin(dropped_stars_ids)]
                lst.custom_save(f"{out_lst}", selected_stars)
                break

            elif event in ["key_comma", ","] and len(values["-STAR LIST-"]):
                if not len(window["-STAR LIST-"].get_indexes()):
                    continue
                rotate_projection(invert=True)

            elif event in ["key_dot"] and len(values["-STAR LIST-"]):
                if not len(window["-STAR LIST-"].get_indexes()):
                    continue
                rotate_projection()
