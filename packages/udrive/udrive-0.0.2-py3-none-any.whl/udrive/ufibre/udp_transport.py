...
from ufibre.utils import wait_any

def noprint(x):
  pass

class UDPTransport(ufibre.protocol.PacketSource, ufibre.protocol.PacketSink):
...
def discover_channels(path, serial_number, callback, cancellation_token, channel_termination_token, logger):
  ...
  while not cancellation_token.is_set():
    try:
      udp_transport = ufibre.udp_transport.UDPTransport(dest_addr, dest_port, logger)
