#!/usr/bin/env python
"""
Tool for flashing .hex files to the uDrive via the STM built-in USB DFU mode.
"""
...
def put_into_dfu_mode(device, cancellation_token):
    ...
    if not hasattr(device, "enter_dfu_mode"):
        print("The firmware on device {} cannot soft enter DFU mode.\n"
              ...
              "dfu-util, see the udrive documentation.\n"
    ...
    except ufibre.ChannelBrokenException:
...
def update_device(device, firmware, logger, cancellation_token):
    ...
    if hw_version < (3, 5, 0):
        ...
        print("Warning: DFU mode is not supported on uDrives earlier than v3.5 unless you perform a hardware mod.")
...
def launch_dfu(args, logger, cancellation_token):
    ...
    logger.info("Waiting for uDrive...")

    devices = [None, None]

    # Start background thread to scan for uDrives in DFU mode
