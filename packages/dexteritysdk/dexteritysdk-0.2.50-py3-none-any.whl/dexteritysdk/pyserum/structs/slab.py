...
from dexteritysdk.pyserum.layouts.event_queue import CALLBACK
from dexteritysdk.pyserum.layouts.slab import SLAB_LAYOUT, NodeType
    ...
    def __build(nodes: ListContainer, buffer: bytes) -> List[SlabNode]:
        ...
        for construct_node in nodes:
            ...
            if node_type == NodeType.UNINITIALIZED:
                ...
                callback_info_buffer = buffer[node.callback:node.callback + 64]
                callback = CALLBACK.parse(callback_info_buffer)
                res.append(
                    SlabLeafNode(
