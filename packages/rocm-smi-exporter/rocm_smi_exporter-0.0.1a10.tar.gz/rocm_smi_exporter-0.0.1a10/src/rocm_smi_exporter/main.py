...
def get_k8s_dev_alloc(endpoint: str):
    # Make the GET request to the specified URL
    try:
        response = requests.get(endpoint)
    except Exception as error:
        logger.warn(f"Failed to get k8s dev alloc, {error=}")
        return {}
    else:
...
class GPUMetrics:
    ...
    def __init__(self, config: Config):
        ...
        per_gpu_metric_labels = LABELS.copy()
        
        if self.config.resolve_k8s_dev_alloc:
            per_gpu_metric_labels.append(LABEL_POD_NAME)

        self.gpu_util = Gauge(METRIC_GPU_UTIL, "GPU utilization (in %).", per_gpu_metric_labels)
        self.gpu_mem_used = Gauge(METRIC_GPU_MEM_USED, "GPU memory used (in Byte).", per_gpu_metric_labels)
        self.gpu_mem_total = Gauge(METRIC_GPU_MEM_TOTAL, "GPU memory total (in Byte).", per_gpu_metric_labels)
        self.gpu_mem_util = Gauge(METRIC_GPU_MEM_UTIL, "GPU memory utilization (in %).", per_gpu_metric_labels)
        self.gpu_cu_occupancy = Gauge(METRIC_GPU_CU_OCCUPANCY, "GPU CU occupancy (in %).", per_gpu_metric_labels)
        self.gpu_temp = Gauge(METRIC_GPU_TEMP, "GPU temperature (in c).", per_gpu_metric_labels)
        self.gpu_power = Gauge(METRIC_GPU_POWER, "GPU power, (in W).", per_gpu_metric_labels)
    ...
    def fetch(self):
        ...
        k8s_dev_alloc = None
        if self.config.resolve_k8s_dev_alloc:
            k8s_dev_alloc = get_k8s_dev_alloc(self.config.k8s_dev_alloc_endpoint)
        ...
        for dev in range(ngpus):
            ...
            if self.config.resolve_k8s_dev_alloc:
                k8s_info = k8s_dev_alloc.get(pci_id, {})
                pod_name = k8s_info.get("Pod", "")
                labels[LABEL_POD_NAME] = pod_name
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='ROCm SMI Exporter command line flags.')
    ...
    parser.add_argument('--resolve-k8s-dev-alloc', type=bool, default=False, action=argparse.BooleanOptionalAction, help='If true, assume running inside k8s and resolve k8s info for GPU metrics')
    parser.add_argument('--k8s-dev-alloc-endpoint', type=str, default='http://localhost:9002/v1/alloc', help='If --resolve-k8s-dev-alloc is true, specify the endpoint to get k8s info')
...
def main():
    ...
    app_metrics = GPUMetrics(
        GPUMetrics.Config(
            ...
            k8s_dev_alloc_endpoint=args.k8s_dev_alloc_endpoint
        )
    )
