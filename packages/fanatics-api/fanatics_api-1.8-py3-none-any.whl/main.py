...
def fetch_single_data(serialNumber, url, headers, failed_file, time_sleep, password, workerNum):
    ...
    while proxy:
        ...
        try:
            ...
            print(f"SSL Error with proxy {proxy}: {e}, will retry")
            i += 1
            print(f"Retry {i} times, sleep 1 second")
            time.sleep(1)
            proxy = get_proxy(password, password, force=(i > 10), proxyNum=workerNum * 2)
            continue
        except RemoteDisconnected as e:
            print(f"RemoteDisconnected Error with proxy {proxy}: {e}, will remove this proxy")
            remove_proxy(proxy)
            i += 1
            print(f"Retry {i} times, sleep 1 second")
            time.sleep(1)
            ...
            print(f"Proxy Error with proxy {proxy}: {e}, will remove this proxy")
            remove_proxy(proxy)
            i+=1
            print(f"Retry {i} times, sleep 1 second")
            time.sleep(1)
