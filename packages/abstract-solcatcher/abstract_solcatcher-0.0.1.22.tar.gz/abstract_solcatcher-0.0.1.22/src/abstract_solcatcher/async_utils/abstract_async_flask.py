...
async def asyncCallRequest(endpoint,*args,**kwargs):
  url = getEndpointUrl(endpoint)
  return await asyncPostRequest(url,kwargs,endpoint=endpoint)

def solcatcherPostRequest(endpoint,signature):
    return asyncio.run(asyncCallRequest(endpoint,signature=signature))
