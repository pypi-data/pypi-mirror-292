...
def get_genesis_signature(account):
  SignaturesFromAddress = getSignaturesForAddress(account)
  before = SignaturesFromAddress[-1].get('signature')
  limit=1000
  while True:
    SignaturesFromAddress = getSignaturesForAddress(account,limit=limit,before=before)
    if SignaturesFromAddress:
      before = SignaturesFromAddress[-1].get('signature')
    if len(SignaturesFromAddress)<limit:
      return before
