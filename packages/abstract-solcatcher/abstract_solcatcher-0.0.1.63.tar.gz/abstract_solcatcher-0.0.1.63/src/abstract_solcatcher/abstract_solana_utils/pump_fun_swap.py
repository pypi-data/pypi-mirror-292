...
def buildTxn(mint,amount,slippage,token_account,buy=True):
    keys = getKeys(get_coin_data(mint),token_account,owner)
    if buy:
...
def pump_fun_buy(mint_str: str, sol_in: float = 0.001, slippage: int = 25) -> bool:
        ...
        buildTxn(mint,amount,slippage,token_account,buy=True)
