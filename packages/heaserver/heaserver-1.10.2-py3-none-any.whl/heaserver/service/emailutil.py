...
async def send_email(message: str, sender: str, receivers: list[str], subject: str, host: str, host_port: int):
    ...
    :param sender: a string with the address of the sender.
    :param receivers: list of strings, one for each recipient.
    ...
    try:
        ...
        logger.debug("Error: Sender Email is not Valid")
        raise ValueError(email_exception)

    try:
        for x in receivers:
            validate_email(x, check_deliverability=False)
    except EmailNotValidError as email_exception:
        logger.debug("Error: One of Receiver Email is not Valid")
        raise ValueError(email_exception)
