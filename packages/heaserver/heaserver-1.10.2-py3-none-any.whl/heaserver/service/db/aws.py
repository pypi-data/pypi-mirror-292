...
class S3(Database):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__admin_creds = TTLCache(maxsize=128, ttl=self.MIN_DURATION_SECONDS)
    ...
    async def generate_cloud_credentials(self, request: Request, arn: str, session_name: str,
        ...
        try:
            ...
            raise ValueError(f'User {sub} does not have role {arn}')
        except ClientResponseError as cre:
            raise ValueError('Token cannot be obtained: {cre}')
    ...
    async def elevate_privileges(self, request: web.Request, credentials: AWSCredentials,
        ...
        """
        Creates an ephemeral credentials object with admin-level privileges for the same account as the given user
        credentials object. It relies on the registry service's AWS_ADMIN_ROLE property being set to the AWS admin
        role, otherwise this method will raise a ValueError.
        ...
        admin_cred = self.__admin_creds.get(sub)
        if admin_cred is None or admin_cred.has_expired(60):
            admin_cred = await self.generate_cloud_credentials(request, admin_role, sub, duration=lifespan)
            self.__admin_creds[sub] = admin_cred
...
class S3ClientContext(DatabaseContextManager[S3Client, AWSCredentials]):  # Go into db package?
    ...
    def __init__(self, request: web.Request, volume_id: str | None = None, credentials: AWSCredentials | None = None):
        """
        Creates an object for getting an S3 client. Either a volume_id or a credentials object must be provided.
...
class IAMClientContext(DatabaseContextManager[IAMClient, AWSCredentials]):  # Go into db package?
    ...
    def __init__(self, request: web.Request, volume_id: str | None = None, credentials: AWSCredentials | None = None):
        """
        Creates an object for getting an IAM client. Either a volume_id or a credentials object must be provided.
...
class STSClientContext(DatabaseContextManager[STSClient, AWSCredentials]):  # Go into db package?
    ...
    def __init__(self, request: web.Request, volume_id: str | None = None, credentials: AWSCredentials | None = None):
        """
        Creates an object for getting an STS client. Either a volume_id or a credentials object must be provided.
...
class AWSPermissionContext(PermissionContext, ABC, Generic[AWSDesktopObjectTypeVar]):
    ...
    async def get_permissions(self, obj: AWSDesktopObjectTypeVar) -> list[Permission]:
        ...
        if cached_value is not None:
            ...
            # Need a check for whether privilege elevation is configured.
    ...
    async def _simulate_perms(self, obj: AWSDesktopObjectTypeVar, actions: Sequence[str]) -> list[Permission]:
        ...
        try:
            credentials = await self.__request.app[HEA_DB].elevate_privileges(self.__request, self.__credentials)
        except ValueError:
            logger.exception("Error getting elevated privileges, will fall back to the user's privileges")
            credentials = self.__credentials
        perms: list[Permission] = []
        async with IAMClientContext(request=self.__request, credentials=credentials) as iam_client:
            loop = asyncio.get_running_loop()
            try:
                ...
                response_ = await loop.run_in_executor(None, partial(iam_client.simulate_principal_policy,
                                                                     PolicySourceArn=self.__credentials.role,
                                                                     ActionNames=actions,
