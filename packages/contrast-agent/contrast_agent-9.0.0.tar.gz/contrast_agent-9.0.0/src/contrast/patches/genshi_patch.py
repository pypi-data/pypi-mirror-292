...
from contrast_rewriter import populate_operator_module
...
def build_genshi_eval_globals_patch(orig_func, _, rule_applicator):
    ...
    def genshi_eval_globals_patch(wrapped, instance, args, kwargs):
        if len(args) > 0 and isinstance(args[0], dict):
            populate_operator_module(args[0])
...
def register_patches():
    register_post_import_hook(patch_genshi_globals, GENSHI_TEMPALTE_ENGINE_EVAL_MODULE)
