...
def apply_ng_app_settings(
    ...
    app_settings = NGApplicationSettings(response_body)
    ui_config = app_settings.common_config()
    logger.debug("Received updated application settings", config=ui_config)
    if _changed := config.update_ui_values(ui_config):
        config.log_config()
...
class Settings(Singleton):
    ...
    def get_app_name(self, app_name):
        if config_name := self.config.get("application.name"):
            return config_name
    ...
    def establish_server_settings_poll(self, reporting_client):
        """
        ...
        if self.server_settings_poll is None:
            ...
            self.server_settings_poll = ServerSettingsPoll(reporting_client)
    ...
    def apply_application_settings(self, application_settings_json: dict):
        apply_application_settings(self.config, application_settings_json, self)
    ...
    def apply_ng_app_settings(self, response_json):
        apply_ng_app_settings(self.config, response_json, self)
    ...
    def response_scanning_enabled(self):
        return (
            self.config
            and self.config.get("assess.enable")
            and self.config.get("assess.enable_scan_response")
        )
