...
def handle_route_discovery(
    framework: str, discovery_func: Callable[..., Set[Route]], args
) -> None:
    """
...
def _do_route_discovery(
    framework: str,
    discovery_func: Callable[..., Set[Route]],
    args: tuple,
) -> None:
    """
    ...
    agent_state.module.reporting_client.add_message(msg)
    log_discovered_routes(framework, discovered_routes)


def log_discovered_routes(framework: str, routes: Set[Route]) -> None:
    logger.debug(
        "Discovered routes",
        framework=framework,
