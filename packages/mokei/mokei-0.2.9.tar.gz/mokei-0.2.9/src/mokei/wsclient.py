...
class MokeiWebSocketClient:
    ...
    async def _onbinary_handler(self, ws, msg: bytes):
        await asyncio.gather(*[handler(ws, msg) for handler in self._onbinary_handlers])

    async def _onerror_handler(self, ws, data):
        await asyncio.gather(*[handler(ws, data) for handler in self._onerror_handlers])
                    ...
                    async with session.ws_connect(self.url) as ws:
                        ...
                        async for msg in ws:
                            ...
                            if msg.type == aiohttp.WSMsgType.TEXT:
                                ...
                                await self._onbinary_handler(ws, msg.data)
                            elif msg.type == aiohttp.WSMsgType.ERROR:
                                await self._onerror_handler(ws, msg.data)
