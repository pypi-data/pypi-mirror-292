...
class CollFilter(Generic[U, T]):
    ...
    def __init__(self, data: Iterable[Tuple[U, T, float]], parallel_num=2 * os.cpu_count(), row_unique=False,
                 similar_fn=default_similar_func, cache_similar=False):
        if parallel_num > 1:
            from cf.pooling import PoolCollFilter
            self._coll_filter = PoolCollFilter(data, parallel_num, row_unique, similar_fn, cache_similar)
        else:
            from cf.base import BaseCollFilter
            self._coll_filter = BaseCollFilter(data, row_unique, similar_fn, cache_similar)
