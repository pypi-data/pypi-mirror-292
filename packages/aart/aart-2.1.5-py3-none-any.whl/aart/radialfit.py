...
def visamp_func_1(u, aL, aR, dphi):
    return np.sqrt(aL**2+aR**2+2*aL*aR*np.sin(2*np.pi*dphi*u/unitfact))/np.sqrt(u*1e9)

def g_fit_1(aL, aR, d, x, y):
    deviation= np.sum((visamp_func_1(x, aL, aR, d)-np.abs(y))**2)
    avg_v_fit = np.sum(visamp_func_1(x, aL, aR, d))
    return np.exp(-np.sqrt(deviation/len(x))/(avg_v_fit/len(x)))

def compute_visamp_fit_1(freqs, visamp, p0_ratio):
    ...
    params, pcov_v = curve_fit(visamp_func_1, freqs, visamp,p0=p0, bounds=bounds, maxfev=2000)
    return params  
...
# functional form of the visibility with aL and aR as functions of u using the upper and lower envelopes
def visamp_exp_func(u, a, b, aL, aR, dphi):
    return np.exp(-a * (u ** b)) * np.sqrt(aL**2 + aR**2 + 2*aL*aR*np.sin(2*np.pi*dphi*u*muas_to_rad*1e9)) / np.sqrt(u)

## goodness of fit
def g_fit_exp(a, b, aL, aR, d, x, y):
    deviation= np.sum((visamp_exp_func(x, a, b, aL, aR, d)-np.abs(y))**2)
    avg_v_fit = np.sum(visamp_exp_func(x, a, b, aL, aR, d))
    return np.exp(-np.sqrt(deviation/len(x))/(avg_v_fit/len(x)))

def compute_visamp_fit_exp(freqs, visamp, a_max=20, b_max=3):
    vmax=np.max(visamp)
    umax=freqs[np.argmax(visamp)]
    vmin=np.min(visamp)
    umin=freqs[np.argmin(visamp)]
...
def log_model(u, a, b, aL, aR, dphi):
    return -a * (u ** b) + 0.5 * np.log(aL**2 + aR**2 + 2*aL*aR*np.sin(2*np.pi*dphi*u*muas_to_rad*1e9)) - 0.5 * np.log(u)

def compute_exp_fit_from_log(freqs, log_visamp, a_max=20, b_max=3):
    visamp = np.exp(log_visamp)
    vmax=np.max(visamp)
    umax=freqs[np.argmax(visamp)]
    vmin=np.min(visamp)
    umin=freqs[np.argmin(visamp)]
    ...
    # a0 = np.abs((np.log(vmax) - np.log(vmin)) / (umin ** b0 - umax ** b0) / 50)   # after tests, 50 is a good factor to divide by for the solver to converge
    a0=0.0001
    ...
    # finding the best fit d_\varphi with prior that d\in[35, 45]
    params, pcov_v = curve_fit(log_model, freqs, log_visamp,p0 = p0, bounds=bounds, maxfev=6000)
    ...
    try:
        for i in range(len(radonangles)):
            ...
            if method=="GLM":
                ...
                params = compute_visamp_fit_1(freqfit, data, p0_ratio)
                aL = params[0]
                aR = params[1]
                dphi = params[2]
            ...
            #     gvals = [g_fit_2(aL, aR, d, exponent, freqfit, data) for d in drange]
                ...
                ## corresponding goodness of fit for each d \in drange
                gvals = [g_fit_exp(a, b, aL, aR, d, freqfit, data) for d in drange]
                
            elif method=="Log-Exponential":
                params = compute_exp_fit_from_log(freqfit, np.log(data))
                a = params[0]
                b = params[1]
                aL = params[2]
                aR = params[3]
