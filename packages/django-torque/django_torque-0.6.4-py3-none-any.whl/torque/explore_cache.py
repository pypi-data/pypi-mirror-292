...
def convert_searchcachedocumentquery_to_memcacheids(query):
    return [
        ...
        for scd in query.values("collection__name", "document__key", "group")
    ]
...
class ExploreCache:
    ...
    def build_memcache_result_cache(self, ids):
        ...
        # There were some cache misses, so let the caller know
        return False
    ...
    def get_results(self, ids):
        ...
        memcache_ids = convert_searchcachedocumentquery_to_memcacheids(
            models.SearchCacheDocument.objects.filter(id__in=ids)
        )
        result = self.build_memcache_result_cache(memcache_ids)
        # If build_memcache_result_cache returned nothing, then we had some cache misses, and we need to reload
        if not result:
            result = cache.get_many([id[0] for id in memcache_ids])
