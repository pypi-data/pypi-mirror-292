...
def update_create_table_query(
    ...
    if engine_full_to_replace or "" != "":
        create_table_query = create_table_query.replace(engine_full_to_replace[0], engine_full_to_replace[1])
    ...
    if cluster_name and not re.search(r"MergeTree\([^)]*\)", create_table_query):
        create_table_query = create_table_query.replace('MergeTree', 'ReplicatedMergeTree()')
