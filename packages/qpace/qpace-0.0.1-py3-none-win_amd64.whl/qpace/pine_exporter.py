...
from .backtest_iterator import BacktestIterator
...
class PineExporter:
    ...
    @staticmethod
    def from_pace_backtest_iter(bt: BacktestIterator) -> str:
        all_trades: list[PineExporter.__Trade] = []
        ...
        return PineExporter.__create_pinescript_strategy_from_trades_with_qty(
            all_trades
        )
    ...
    def __create_pinescript_strategy_from_trades_with_qty(
        ...
        ps = "//@version=5"
        ps += '\nstrategy("Strategy export", overlay=true, initial_capital=1000, default_qty_type = strategy.percent_of_equity, default_qty_value = 100)'
    ...
    def trend_bar_color(df: pd.DataFrame, trend_column: str = "trend", strategy=False):
        ...
        up_trend_times = (df[df[trend_column] == 1]["open_time_ms"] / 1000.0).astype(
            int
        )
        down_trend_times = (df[df[trend_column] == -1]["open_time_ms"] / 1000.0).astype(
            int
        )
        ...
        ps += f"""
up_trend_times = {PineExporter.__as_array_items(up_trend_times)}
down_trend_times = {PineExporter.__as_array_items(down_trend_times)}
