...
class Element:
    def __init__(
        ...
        self._block = block
        self._leaf = leaf
        self._content = _process_element_content_input(content)
    ...
    @property
    def content(self) -> _ElementContentContainer:
    ...
    def add_content(self, content: _ElementContentType, key: str | int = None):
        if key is None:
            key = max(key for key in self._content.keys() is isinstance(key, int)) + 1
    ...
    def display(self) -> None:
        ...
        _render.display(str(self), ipython=True, as_md=True)
        return


class ThematicBreak(Element):
    def __init__(self, char: _Literal["-", "_", "*"] = "-"):
...
class ATXHeading(Element):
    def __init__(self, level: _Literal[1, 2, 3, 4, 5, 6], content: _ElementContentInputType = ""):
        super().__init__(block=True, leaf=True, content=content)
        self._level = level
        return
...
class FieldListElement(Element):
    def __init__(self, name: _ElementContentType, body: _ElementContentInputType = "", indent_size: int = 4):
        super().__init__(block=True, leaf=False, content=body)
        self.name = name
...
class HTMLBlock(Element):
    ...
    def __init__(self, content: _ElementContentInputType = None):
        super().__init__(block=True, leaf=True, content=content, newlines_after=2)
        return
...
class CodeFence(Element):
    ...
    def __init__(
        ...
        super().__init__(block=True, leaf=False, content=content)
        self._info = info
        self.fence = fence
        return
...
class Directive(CodeFence):
    def __init__(
        ...
        super().__init__(content=content, fence=fence)
        self.name = name
        self.arg = arg
...
def heading(level: _Literal[1, 2, 3, 4, 5, 6], content: _ElementContentInputType = "") -> ATXHeading:
    ...
    content : str
        Heading content.
    """
    return ATXHeading(level=level, content=content)
...
def code_fence(
    ...
    fence: {'`', '~', ':'}, default: '`'
        Fence character.
    """
    return CodeFence(content=content, info=info, fence=fence)
...
def directive(
    ...
    fence: {'`', '~', ':'}, default: '`'
        Fence character.
    """
    return Directive(name=name, content=content, arg=arg, options=options, fence=fence)
...
def admonition(
    ...
    return Directive(name="admonition", content=content, arg=title, options=options, fence=fence)
...
def code_block(
    ...
    return Directive(name="code-block", content=content, arg=language, options=options, fence=fence)
...
def tab_item(
    ...
    return Directive(name="tab-item", content=content, arg=title, options=options, fence=fence)
...
def tab_set(
    ...
    return Directive(name="tab-set", content=content, options=options, fence=fence)
...
def card(
    ...
    return Directive(name="card", content=content, arg=body_title, options=options, fence=fence)
