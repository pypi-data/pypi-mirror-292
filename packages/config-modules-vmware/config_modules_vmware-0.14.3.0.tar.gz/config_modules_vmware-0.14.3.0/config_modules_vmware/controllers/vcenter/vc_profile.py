...
class VcProfile(BaseController):
    ...
    def get(self, context: VcenterContext, template: dict = None) -> Tuple[dict, List[str]]:
        ...
        if utils.is_newer_or_same_version(context.product_version, "9.0"):
            return self._get_9_0(context, template)
        elif utils.is_newer_or_same_version(context.product_version, "8.0.3"):
            return self._get_8_0_3(context, template)
        else:
            return {}, [
                consts.UNSUPPORTED_VERSION_MESSAGE_FORMAT.format(context.product_version, context.product_category)
            ]
    ...
    def _get_9_0(self, context: VcenterContext, template: dict = None) -> Tuple[dict, List[str]]:
        logger.info(f"Getting VC profile configuration for version {context.product_version}")
        current_vc_profile = {}
        errors = []
        try:
            vc_rest_client = context.vc_rest_client()
            url = vc_rest_client.get_base_url() + vc_consts.VC_PROFILE_DOMAIN_CONFIGURATION_URL
            current_vc_profile = vc_rest_client.get_helper(url)
        except Exception as e:
            logger.error(f"An error occurred for 'get' vc_profile: {e}")
            errors.append(str(e))
    ...
    def check_compliance(self, context: VcenterContext, desired_values: Any) -> Dict:
        ...
        try:
            if utils.is_newer_or_same_version(context.product_version, "9.0"):
                task_id, task_response, errors = self._check_compliance_9_0(context, desired_values)
            elif utils.is_newer_or_same_version(context.product_version, "8.0.3"):
                task_id, task_response, errors = self._check_compliance_8_0_3(context, desired_values)
            else:
                errors.append(
                    vc_profile_utils.create_error(
                        vc_profile_utils.source_type_config_module,
                        consts.UNSUPPORTED_VERSION_MESSAGE_FORMAT.format(
                            context.product_version, context.product_category
                        ),
                    )
                )
        except Exception as e:
            logger.error(f"An error occurred for 'check_compliance' vc_profile: {str(e)}")
            errors.append(
                vc_profile_utils.create_error(
                    vc_profile_utils.source_type_config_module,
                    str(e),
    ...
    def get_schema(self, context: VcenterContext) -> Dict:
        """Get VC profile configuration schema.
        ...
        logger.info("Getting VC profile configuration schema.")

        if utils.is_newer_or_same_version(context.product_version, "9.0"):
            try:
                vc_rest_client = context.vc_rest_client()
                url = vc_rest_client.get_base_url() + vc_consts.VC_PROFILE_DOMAIN_CONFIGURATION_SCHEMA_URL
                schema = vc_rest_client.get_helper(url)
                return {consts.RESULT: schema, consts.STATUS: GetSchemaStatus.SUCCESS}
            except Exception as e:
                logger.error(f"An error occurred for 'get_schema' vc_profile: {e}")
                return {consts.STATUS: GetSchemaStatus.FAILED, consts.MESSAGE: str(e)}
        else:
            return {
                consts.STATUS: GetSchemaStatus.FAILED,
    ...
    def _validate_desired_values_8_0_3(self, desired_values: dict):
        """
        Validate if components in desired values are supported for version 8.0.3.
        """
    ...
    def _validate_desired_values_9_0(self, desired_values: dict):
        """
        Validate if components in desired values are supported for version 9.0.
        """
        if not desired_values or not desired_values.get("config", {}).get("vcenter", {}):
            raise ValueError("Desired values is invalid")

    def _check_compliance_8_0_3(self, context: VcenterContext, desired_values: Any) -> Tuple[str, dict, List[Error]]:
        """
        Invoke VC check compliance API with given desired_values.
        :param context: Product context instance.
        :type context: VcenterContext
        :param desired_values: Desired values for the specified configuration.
        :type desired_values: Any
        :type: str
        :return: Tuple of the task_id, task_response, and list of errors
        :rtype: tuple
        """
        self._validate_desired_values_8_0_3(desired_values)
        ...
        merged_desired_values, errors = self._get_merged_desired_values(context, desired_values, get_template)
        if errors:
            return "", {}, errors

        desired_state = {"desired_state": merged_desired_values}
        return self._invoke_vc_profile_check_compliance(context, desired_state, vc_consts.DESIRED_STATE_SCAN_URL)

    def _check_compliance_9_0(self, context: VcenterContext, desired_values: Any) -> Tuple[str, dict, List[Error]]:
        """
        Invoke VC check compliance API with given desired_values.
        :param context: Product context instance.
        :type context: VcenterContext
        ...
        :rtype: tuple
        """
        self._validate_desired_values_9_0(desired_values)

        # Check compliance API does not support partial drift, only at component level
        # So fetch only requested components
        get_template = {
            "config": {"vcenter": {key: {} for key in desired_values.get("config", {}).get("vcenter", {}).keys()}}
        }
        ...
        return self._invoke_vc_profile_check_compliance(
            context, merged_desired_values, vc_consts.VC_PROFILE_DOMAIN_CONFIGURATION_CHECK_COMPLIANCE_URL
        )

    def _get_merged_desired_values(
        self,
        ...
        """
        Get the current config merged with the provided desired values
        :param context: Product context instance.
        :type context: VcenterContext
        :param desired_values: Desired values for the specified configuration.
        :type desired_values: Any
        :param get_template: The get config template that will be merged with desired_values
        :type get_template: dict
        :return: Tuple of the merged desired_values and list of errors
        :rtype: tuple
        """
        errors = []
        merged_desired_values = {}
        ...
        if get_errors:
            ...
            merged_desired_values = self._merge_config(current_config, desired_values)
        return merged_desired_values, errors

    def _invoke_vc_profile_check_compliance(
        self, context: VcenterContext, desired_values: Any, check_compliance_url: str
    ) -> Tuple[str, dict, List[Error]]:
        """
        Invoke VC check compliance API with given desired_values.
        :param context: Product context instance.
        :type context: VcenterContext
        :param desired_values: Desired values for the specified configuration.
        :type desired_values: Any
        :param check_compliance_url: The VC check compliance url
        :type check_compliance_url: str
        :return: Tuple of the task_id, task_response, and list of errors
        :rtype: tuple
        """
        errors = []
        ...
        # Invoke check compliance VCProfile API
        try:
            task_id = context.vc_rest_client().post_helper(
                url=context.vc_rest_client().get_base_url() + check_compliance_url,
                body=desired_values,
            )
        except Exception as e:
            ...
            errors.append(
                vc_profile_utils.create_error(
                    ...
                    str(e),
                    context.hostname,
                    context.vc_rest_client().get_base_url() + check_compliance_url,
        ...
        if task_id:
            try:
                ...
                logger.error(f"An error occurred in polling vc_profile 'check_compliance' task: {e}")
                errors.append(
                    vc_profile_utils.create_error(
                        context.product_category,
                        str(e),
        ...
        return task_id, task_response, errors
