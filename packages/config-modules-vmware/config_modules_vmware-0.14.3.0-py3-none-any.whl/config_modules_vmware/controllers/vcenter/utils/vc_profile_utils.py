...
def _parse_unavailable_failed_status(context, task_id, task_response, errors):
        ...
        if "status" in task_response and "progress" in task_response and "result" in task_response:
            if task_response["status"] == vc_consts.CIS_TASK_SUCCEEDED:
                ...
                if result_status.upper() == "COMPLIANT":
                    ...
                    added_configs, deleted_configs, modified_configs = _parse_compliance_changes(context, task_result)
                    ...
                    _parse_unavailable_failed_status(context, task_id, task_response, errors)
                else:
                    logger.error(
                        f"unknown result status[{result_status}] for task[{task_id}] in 'check_compliance' vc_profile"
                    )
...
def _parse_compliance_changes(
    context: VcenterContext, task_result: dict
) -> Tuple[List[ConfigAddition], List[ConfigDeletion], List[ConfigModification]]:
    """
    Parse the compliance changes and return the tuple of additions, deletions, and modifications
    :param context: Product context instance.
    :type context: VcenterContext
    :param task_result: The compliance task result
    :type task_result: dict
    :return: Tuple of additions, deletions, and modifications
    :rtype: tuple
    """
    if utils.is_newer_or_same_version(context.product_version, "9.0"):
        return _parse_compliance_changes_9_0(task_result)
    else:
        return _parse_compliance_changes_8_0_3(task_result)


def _parse_compliance_changes_8_0_3(
    task_result: dict,
) -> Tuple[List[ConfigAddition], List[ConfigDeletion], List[ConfigModification]]:
    added_configs = []
    deleted_configs = []
        ...
        for diff in diffs:
            ...
            if "Added" in diff_value["description"]:
                ...
                added_configs.append(
                    ConfigAddition(diff_value["path"], diff_value["category"], diff_value["desired_value"])
                )
            elif "Removed" in diff_value["description"]:
                # Removed Config
                deleted_configs.append(
                    ConfigDeletion(diff_value["path"], diff_value["category"], diff_value["current_value"])
                )
            else:
                # Modified config
                modified_config = ConfigModification(diff_value["path"], diff_value["category"])
                modified_config.current_value = diff_value.get("current_value", "")
                modified_config.desired_value = diff_value.get("desired_value", "")
                modified_configs.append(modified_config)
    return added_configs, deleted_configs, modified_configs


def _parse_compliance_changes_9_0(
    task_result: dict,
) -> Tuple[List[ConfigAddition], List[ConfigDeletion], List[ConfigModification]]:
    added_configs = []
    deleted_configs = []
    modified_configs = []
    compliance_info = task_result["compliance_info"]
    if "adds" in compliance_info:
        adds = compliance_info["adds"]
        for add in adds:
            added_configs.append(ConfigAddition(add["path"], _get_category_from_path(add["path"]), add["target"]))
    if "deletes" in compliance_info:
        deletes = compliance_info["deletes"]
        for delete in deletes:
            deleted_configs.append(
                ConfigDeletion(delete["path"], _get_category_from_path(delete["path"]), delete["current"])
            )
    if "sets" in compliance_info:
        sets = compliance_info["sets"]
        for modification in sets:
            modified_config = ConfigModification(modification["path"], _get_category_from_path(modification["path"]))
            modified_config.current_value = modification.get("current", "")
            modified_config.desired_value = modification.get("target", "")
            modified_configs.append(modified_config)
    return added_configs, deleted_configs, modified_configs
...
def _get_category_from_path(path: str) -> str:
    """
    Get category from compliance info path.

    Example:
    /vcenter/appliance/user_account_settings/local_accounts_policy/warn_days -> user_account_settings

    :param path: The compliance info path
    :type path: str
    :return: The category
    :rtype: str
    """
    keys = path.split("/")
    if len(keys) < 4:
        logger.warning(f"Cannot retrieve category from path {path}")
        return ""
    return keys[3]
