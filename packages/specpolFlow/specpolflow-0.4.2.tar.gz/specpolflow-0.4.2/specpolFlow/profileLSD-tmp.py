...
class LSD:
    ...
    * npix - the number of pixels (points in velocity) in the LSD profile
    ...
    def calc_bis(self, Ic=1.,  cog='I', biswidth=None, plotFit=False, fullOutput=False):
        ...
        _blu_use = (lsd.vel < cog_val - remove_center_vel) & indVelUse
        _red_use = (lsd.vel > cog_val + remove_center_vel) & indVelUse
        _red_extra = (lsd.vel > cog_val) & (lsd.vel <= velrange[1]*2.)

        if not emission:
            _blu_use &= lsd.specI < Ic
            _red_use &= lsd.specI < Ic
            # get the portion of the extended red side of the line where I is
            # monotonically increasing (for interpolating between later)
            ...
            _red_use &= lsd.specI > Ic
            # get the portion of the extended red side of the line where I is
            # monotonically decreasing (for interpolating between later)
            ind_red_extra = np.nonzero(_red_extra)[0]
            for ind in range(ind_red_extra[1], ind_red_extra[-2]):
                if lsd.specI[ind] <= lsd.specI[ind+1]: break
            _red_extra[ind+1:] = False
        ...
        #deltaI0 = specImax - specImin
        if not emission:
            deltaI = Ic - specImin
        else:
            deltaI = specImax - Ic
        #print('deltaI', deltaI0, deltaI)
        ...
        if not emission:
            ...
            #rvel_levels = np.interp(specI_levels, lsd.specI[_red_use],
            #                        (lsd.vel - cog_val)[_red_use])
            ...
            #rvel_levels = np.interp(specI_levels, lsd.specI[_red_use][::-1],
            #                        (lsd.vel - cog_val)[_red_use][::-1])
        ...
        # use some error trapping in case there are no points in a depth range
        ...
        if plotFit == True:
            ...
            plt.errorbar(lsd.vel, lsd.specI, yerr=lsd.specSigI,
                         c='k', zorder=1)
            scale_bisector = 10. # arbitrary scale for plotting
            plt.plot(bisector*scale_bisector, specI_levels, 'r.-', lw=0.5, zorder=2,
