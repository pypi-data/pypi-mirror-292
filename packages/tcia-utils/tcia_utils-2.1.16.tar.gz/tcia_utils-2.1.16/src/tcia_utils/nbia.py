...
def setApiUrl(endpoint, api_url):
    ...
    if not endpoint in searchEndpoints and not endpoint in advancedEndpoints:
        ...
        if api_url == "":
            # log in with guest on v2 restricted API if no user credentials are provided
            ...
            if endpoint in searchEndpoints:
                # all requests go to restricted URL, but use guest account token if no user-specific login token existed previously
                base_url = "https://services.cancerimagingarchive.net/nbia-api/services/v2/"
            if endpoint in advancedEndpoints:
                # Using "Advanced" API (with guest if no login provided): https://wiki.cancerimagingarchive.net/x/YoATBg
...
def queryData(endpoint, options, api_url, format):
    ...
    try:
        ...
        # REMOVING if statement since everything needs headers now!!
