...
class NodeManager:
    ...
    @staticmethod
    async def get_nodes(node_ids: List[int]) -> List[NodeModel]:
        if not node_ids:
            return []
        return await CRUDNode().get_by_ids(list_ids=node_ids)


class EdgeManager:
    @staticmethod
    async def get_edges(edge_ids: List[int]) -> List[Edge]:
        if not edge_ids:
            return []
        return await CRUDEdge().get_by_ids(list_ids=edge_ids)
...
class TemplateManager:
    ...
    def get_templates(template_ids: List[int]) -> List[Template]:
        if not template_ids:
            return []
        return CRUDTemplate().get_by_ids(list_ids=template_ids)
...
class RouterManager:
    @classmethod
    async def get_router(cls, router_id: int) -> Router:
        return await CRUDRouter().get_by_id(id=router_id)
    ...
    async def get_graph(self, chat_id: str, llm, router_id: int, condition_cls: Condition = Condition,
        ...
        router = await RouterManager.get_router(router_id)
        ...
        nodes = await NodeManager.get_nodes(int_node_ids)
        edges = await EdgeManager.get_edges(int_edge_ids)

        template_ids = self.get_template_ids(nodes)
        templates = await TemplateManager.get_templates(template_ids)
