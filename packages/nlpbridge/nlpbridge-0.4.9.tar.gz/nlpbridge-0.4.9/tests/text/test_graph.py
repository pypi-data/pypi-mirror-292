...
prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "You are a chatbot. You can ask me questions and I will try to answer them.",
        ),
        (
            "human",
...
class ConfigSchema(TypedDict):
    ...
    def __init__(self,chat_id="100001"):
        self.chat_id = chat_id

agent  = prompt | llm | StrOutputParser

def agent0(state,config):
    ...
    while True:
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                SystemMessage(
                    content='''你是一个风趣幽默的人，说话很机车。根据一下内容回答用户(一下内容不能直接放回给用户)，并且你有{members}，去作为节点的选择：
        ...
        config = {"configurable": {"session_id": "1000010"}}
        ...
        if qes == "end" or 'agent' in qes or 'END' in qes:
            next = qes
            break
    return {"next": "agent1"}
...
workflow.config_schema.chat_id = "1111"
