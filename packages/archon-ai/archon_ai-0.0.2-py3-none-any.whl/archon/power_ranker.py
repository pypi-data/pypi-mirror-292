...
class PowerRanker:
    def __init__(self, benchmark_name: str, judge_path: str, baseline_path: str, model_list_paths: list[str], output_dir: str = 'outputs/'):
        if benchmark_name not in QUESTION_MAP:
            raise ValueError(f'{benchmark_name} is not a valid benchmark name')
        self.benchmark = {'name': benchmark_name, 'question_file': QUESTION_MAP[benchmark_name]}
        self.output_dir = output_dir
        self.judge = self.base_initializer(judge_path)
        self.baseline = self.base_initializer(baseline_path)
        self.eval_models = [self.eval_initializer(model_path) for model_path in model_list_paths]
        self.results = None
    ...
    def base_initializer(self, model_path: str):
        ...
        new_model['answer_path'] = f"{self.output_dir}/{self.benchmark['name']}/model_answer/{new_model['name']}.json"
    ...
    def eval_initializer(self, model_path: str):
        ...
        if self.benchmark['name'] == 'arena_hard_auto':
            new_model['judgment_path'] = f"{self.output_dir}/{self.benchmark['name']}/model_judgment/{self.judge['name']}-judge/{self.baseline['name']}-baseline/{new_model['name']}.json"
        
        # TODO: Update mt_bench save file
        if self.benchmark['name'] == 'mt_bench':
            new_model['judgment_path'] = f"{self.output_dir}/{self.benchmark['name']}/model_judgment/{self.judge['name']}_pair.jsonl"
        return new_model
    ...
    def compare_against_arena(self):
        ...
        judge_config = generate_pairwise_config(baseline=self.baseline['name'], judge=self.judge['config_path'], model_list=model_name_list, temperature=0.0)
        generate_judgments(configs=judge_config, save_directory=self.output_dir, question_file=self.benchmark['question_file'])
        self.results, self.model_to_score_dict = rank_model_performance(save_directory=self.output_dir, judge_name=self.judge['name'], baseline=self.baseline['name'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--benchmark_name', type=str, default='arena_hard_auto', help='Name of the benchmark')
    parser.add_argument('--judge_path', type=str, required=True, help='Path to the judge archon configuration')
    parser.add_argument('--output_dir', type=str, default='outputs', help='Directory for output files (defaults to outputs)')
    parser.add_argument('--baseline_path', type=str, required=True, help='Path to the baseline model configuration')
    parser.add_argument('--model_list_paths', type=str, nargs='+', required=True, help='List of model configuration paths')
    parser.add_argument('--dataset_sample', type=float, default=1.0, help='Sample size of the dataset to use')
    args = parser.parse_args()

    test = PowerRanker(args.benchmark_name, args.judge_path, args.baseline_path, args.model_list_paths, args.output_dir)
