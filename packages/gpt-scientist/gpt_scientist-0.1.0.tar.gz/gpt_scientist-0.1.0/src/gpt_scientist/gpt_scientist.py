...
class Scientist:
    ...
    # Move global functions into the class:
    ...
    def get_response(self, prompt: str) -> dict:
        ...
        for attempt in range(self.num_reties):
            if attempt > 0:
                logging.error(f"Attempt {attempt + 1}")
            ...
            for i in range(self.num_results):
                ...
                if response is None:
                    ...
                    logging.error(f"Bad response: {completions.choices[i].message.content.strip()}")
                    continue
                return response
    ...
    def analyze_data(self,
        ...
        '''
            ...
            create a prompt by concatenating prompt_prefix and the value and a suffix explaining the expected format of the response;
        ...
        if self.checkpoint_file and os.path.exists(self.checkpoint_file):
            ...
            if not set(data.columns).issubset(checkpoint_data.columns) or not set(output_fields).issubset(checkpoint_data.columns):
                logging.warning(f"Checkpoint file {self.checkpoint_file} does not have the same columns as the dataframe and will be overwritten.")
                os.remove(self.checkpoint_file)
            else:
                # Merge the checkpoint data with the dataframe and start from the last row
                logging.info(f"Found {len(checkpoint_data)}/{len(data)} rows in the checkpoint file.")
        ...
        for i, row in data.iterrows():
            ...
            if response is None:
                logging.error(f"No valid response for input: {input}")
                continue
    ...
    def analyze_csv(self,
        ...
        data = self.analyze_data(data, prompt_prefix, input_field, output_fields)
        if output_path:
    ...
    def analyze_google_sheet(self,
        ...
        '''
        if not IN_COLAB:
            logging.error("This method is only available in Google Colab.")
            return
        creds, _ = default()
