...
class Scientist:
    ...
    def get_response(self, prompt: str, output_fields: list[str] = []) -> dict:
        ...
        for attempt in range(self.num_reties):
            ...
            for i in range(self.num_results):
                ...
                if missing_fields:
                    # Log the bad respons
                    self.logger.error(f"Response is missing fields {missing_fields}: {response}")
                    continue
    ...
    def analyze_data(self,
        ...
        data = data.copy()
        # Initialize the output fields with None
        data[output_fields] = None
    ...
    def _convert_value_for_gsheet(val):
