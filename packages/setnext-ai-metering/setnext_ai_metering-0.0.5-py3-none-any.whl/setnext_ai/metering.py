...
async def log(input_prompt="", output_completion="", transaction_uuid="", **kwargs):
    try:
        ...
        payment_cost = kwargs.get('PAYMENT_COST') or os.environ.get('PAYMENT_COST')
        ...
        setnext_ai_metering_api_token = kwargs.get('SETNEXT_METERING_API_TOKEN') or os.environ.get('SETNEXT_METERING_API_TOKEN')

        if not credit_id:
            raise ValueError("SETNEXT-METERING: Validation Error. CREDIT_ID is mandatory.")
        if not payment_cost:
            raise ValueError("SETNEXT-METERING: Validation Error. PAYMENT_COST is not found.")
        if not client_id:
...
# async def log_image_to_text(input_image_path, input_text,  output_text, transaction_uuid="", **kwargs):
#     try:
...
#         async with httpx.AsyncClient() as client:
#             response = await client.post(
#                 'https://t7joqdff3l.execute-api.us-east-1.amazonaws.com/dev/logs',
#                 json=message_body,
#                 headers={"Content-Type": "application/json"}
#             )
#             response.raise_for_status()
...
async def main():
    try:
        result = await log(
            input_image_path=r"C:\Users\dhara\OneDrive\Pictures\Saved Pictures\download.jpeg",
