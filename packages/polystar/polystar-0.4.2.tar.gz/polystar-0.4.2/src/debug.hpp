...
#ifndef POLYSTAR_DEBUG_HPP_
#define POLYSTAR_DEBUG_HPP_
/*! \file
    \author Greg Tucker
...
std::string my_to_string(const T x, const size_t width=0){
  ...
  if constexpr (std::is_base_of<char,T>::value || (std::is_integral<T>::value && std::is_unsigned<T>::value) ){
    ...
    streamobj << (polystar::is_negative(x) ? "-" : " ") << polystar::abs(x);
  }
...
std::string my_to_string(const std::complex<T> x, const size_t width=0){
  ...
  if (!std::is_integral<T>::value || std::is_signed<T>::value){
    ...
    streamobj << (polystar::is_negative(r) ? "-" : " ") << polystar::abs(r);
    streamobj << (polystar::is_negative(i) ? "-i" : "+i") << polystar::abs(i);
  } else {
...
#define info_update(...) polystar::printer.println(PREPEND_LOG, __VA_ARGS__)
//! A macro for conditional informational logging to standard output
#define info_update_if(condition, ...) if (condition) polystar::printer.println(PREPEND_LOG, __VA_ARGS__)
  ...
  #define debug_update(...) polystar::printer.println(PREPEND_LOG, __VA_ARGS__)
  //! A macro for conditional debug logging to standard output
  #define debug_update_if(condition, ...) if (condition) polystar::printer.println(PREPEND_LOG, __VA_ARGS__)
#else
  ...
  #define verbose_update(...) polystar::printer.println(PREPEND_LOG, __VA_ARGS__)
  //! A macro for conditional verbose debug logging to standard output
  #define verbose_update_if(condition, ...) if (condition) polystar::printer.println(PREPEND_LOG, __VA_ARGS__)
#else
  ...
  #define profile_update(...) polystar::printer.println(PREPEND_LOG, __VA_ARGS__)
  //! A macro for conditionally printing profiling messages to standard output
  #define profile_update_if(condition, ...) if (condition) polystar::printer.println(PREPEND_LOG, __VA_ARGS__)
#else
