...
namespace SVG {
    ...
    public:
        ...
        Element& operator<<(T&& node) {
            ...
            SVG_TYPE_CHECK;
            this->children.push_back(std::make_unique<T>(std::forward<T>(node)));
            return *this;
        }
    ...
    public:
      ...
      template<typename T> inline void add_to(const std::string & type, T x, T y){
        if (attr.find("d") == attr.end()) {
          start(x, y);
        } else {
          attr["d"] += type + " " + std::to_string(x) + " " + std::to_string(y);
          check_x_y(x, y);
        }
      }

      template<typename T>
      inline void move_to(T x, T y){ add_to("M", x, y); }


      template<typename T>
        inline void line_to(T x, T y){ add_to("L", x, y); }
    ...
    private:
      ...
      template<class T>
      inline std::enable_if_t<std::is_convertible_v<T,double>, void> check_x_y(T x, T y){
        auto a = static_cast<double>(x);
        auto b = static_cast<double>(y);
        if (a < x_min) x_min = a;
        if (a > x_max) x_max = a;
      ...
      template<class T> inline std::enable_if_t<!std::is_convertible_v<T,double>, void> check_x_y(T, T) {}
    };
