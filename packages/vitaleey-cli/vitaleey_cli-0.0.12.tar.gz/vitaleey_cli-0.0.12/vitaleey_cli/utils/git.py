...
class Git:
    ...
    @property
    def branch(self):
        return getattr(gitlab_config(), "branch", os.environ.get("CI_DEFAULT_BRANCH"))
    ...
    def repo(self):
        if self._repo is None:
            self._repo = self.gl.projects.get(getattr(gitlab_config(), "project_id", os.environ["CI_PROJECT_ID"]))
        return self._repo
