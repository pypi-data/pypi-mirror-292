...
def build(**options):
    ...
    docker = Docker(
        registry=getattr(app_config, "docker_registry", os.environ.get("DOCKER_REGISTRY")),
        image_name=getattr(app_config, "docker_image_name", os.environ.get("DOCKER_IMAGE_NAME")),
    ...
    try:
        docker.build(
            build_image_name, target=options["target"], cache_from=latest_image
        )
        image = docker.get_image_name(
            tag=version,
        )
        docker.image_tag(build_image_name, version)
        if options["push"]:
            docker.push(image)
