...
import logging
from cyclarity_sdk.platform_api import ClarityLoggerFactory, LogHandlerType
from abc import abstractmethod
from cyclarity_sdk.platform_api import PlatformApi
...
class ParsableModel(BaseModelExtraAllow):
    @classmethod
    def generate_params_schema(cls, *args, **kwargs) -> str:
    ...
    def model_post_init(self, *args, **kwargs):
        ...
        self.logger = ClarityLoggerFactory.get_logger(
            ...
            level=logging.DEBUG,
            handler_type=LogHandlerType.CLI  # DEV
        )
...
class ContextParsable(ParsableModel):
    ...
    def setup(self):
        ...
        raise NotImplementedError(
            f'Setup sequence was not definded for class {self.__class__.__name__}')
    ...
    def run(self, *args, **kwargs):
        ...
        """
        raise NotImplementedError(
            f'Excecute sequence was not definded for class {self.__class__.__name__}')
    ...
    def teardown(self, exception_type=None, exception_value=None, traceback=None):
        ...
        """
        raise NotImplementedError(
            f'Teardown sequence was not defined for class {self.__class__.__name__}')
...
class ResultsModel(BaseModel):
    results_type: ClassVar[type]

    @classmethod
    @validate_call  # force passing result_type which inherit from base model
    def __init_subclass__(cls, results_type: Type[BaseModel], **kwargs: Any):
        # initialize result_type passed from the subclasses
        cls.results_type = results_type

    @classmethod
    def generate_results_schema(cls, *args, **kwargs) -> str:
        return cls.results_type.model_json_schema()


class Runnable(ContextParsable, ResultsModel, results_type=BaseResultsModel):  # noqa
    def model_post_init(self, *args, **kwargs):
        super().model_post_init(*args, **kwargs)
        self.platform_api = PlatformApi()

    def __call__(self, *args, **kwargs) -> BaseResultsModel:
        """Executable phase (python built-in method)
            Calling an instance of a Runnable class will excecute the run() method and return the results.
        Returns:
            Results: a result object of the executable
    ...
    def set_dev_api(self, api_env: IPlatformConnectorApi):
        self.platform_api.set_api(api_env)
