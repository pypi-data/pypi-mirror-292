...
def def_prefs(prm):
    ...
    #Figure preferences
    prm['pref']['grid'] = True
    prm['pref']['marker_size'] = 20
    prm['pref']['tracker_size'] = 15
    prm['pref']['dpi'] = 80
    #range is 0--255
    prm['pref']['lineColor1'] = (0,0,0)
    prm['pref']['line_width'] = 1
    prm['pref']['backgroundColor'] = (250,250,250)
    prm['pref']['canvasColor'] = (85, 170, 255)#(200, 200, 200)
...
def get_prefs(prm):
    prm = def_prefs(prm)
    prm['prefFile'] = os.path.expanduser("~") +'/.config/audiometry_trainer/preferences'+prefFileSuffix

    if os.path.exists(os.path.expanduser("~") +'/.config/') == False:
        os.mkdir(os.path.expanduser("~") +'/.config/')
    if os.path.exists(os.path.expanduser("~") +'/.config/audiometry_trainer/') == False:
        os.mkdir(os.path.expanduser("~") +'/.config/audiometry_trainer/')
