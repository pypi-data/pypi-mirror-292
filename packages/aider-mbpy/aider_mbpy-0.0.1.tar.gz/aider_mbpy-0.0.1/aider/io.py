...
class InputOutput:
    ...
    def __init__(
        ...
        log_chat_history_jsonl="chat_history.jsonl",
    ):
        ...
        if log_chat_history_jsonl is not  None:
            self.chat_history_jsonl = Path("chat_history.jsonl")
        ...
        self.log_chat_history_jsonl("system", f"aider chat started at {current_time}")
...
{prefix} {hist}"""
        ...
        self.log_chat_history_jsonl("user", inp)
    # OUTPUT

    def ai_output(self, content):
        hist = "\n" + content.strip() + "\n\n"
        self.append_chat_history(hist)
        self.log_chat_history_jsonl("assistant", content)
    ...
    def tool_output(self, *messages, log_only=False, bold=False):
        if messages:
            ...
            hist = f"{hist.strip()}"
            self.append_chat_history(hist, linebreak=True, blockquote=True)
            self.log_chat_history_jsonl("tool", " ".join(messages))
        if not log_only:
    ...
    def log_chat_history_jsonl(self, role, content):
        message = {
            "role": role,
            "content": content,
            "timestamp": datetime.now().isoformat(),
            "num_error_outputs": self.num_error_outputs,
        }

        with open(self.chat_history_jsonl, "a", encoding=self.encoding) as f:
            json.dump(message, f)
            f.write("\n")
