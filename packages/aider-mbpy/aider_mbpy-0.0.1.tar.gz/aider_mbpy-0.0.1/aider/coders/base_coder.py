...
class Coder:
                ...
                while new_user_message:
                    ...
                    if self.reflected_message:
                        if self.num_reflections < self.max_reflections:
                            ...
                            error = Path(self.io.chat_history_file).read_text().splitlines[-10:]
                            self.io.tool_error(
                                f"Only {self.max_reflections} reflections allowed, stopping and searching the web for {error}"
                                
                            )
                            self.commands.run("web " +  error)
                if with_message:
    ...
    def send_new_user_message(self, inp):
        ...
        if self.show_pretty() and self.stream:
            mdargs = {"style": self.assistant_output_color, "code_theme": self.code_theme}
            ...
            while True:
                try:
                    ...
                    if messages[-1]["role"] == "assistant":
                        ...
                        messages.append({"role": "assistant", "content": self.multi_response_content})
        ...
        if self.partial_response_function_call:
            ...
            content = args["explanation"] if args else ""
        ...
        try:
            ...
            if self.partial_response_content:
                ...
                if args:
                    ...
                    self.save_function_call_to_jsonl(args)

        if interrupted:
            raise KeyboardInterrupt

    def save_function_call_to_jsonl(self, args):
        """Save the function call details to a JSONL file for later analysis or processing.

        This method is called when there's a function call in the AI's response.
        """
        if self.io.jsonl_history_file:
            entry = {
                "timestamp": datetime.now().isoformat(),
                "function_call": args,
                "type": "function_call"
            }
            with Path(self.io.jsonl_history_file).open("a", encoding=self.io.encoding) as f:
                json.dump(entry, f)
                f.write("\n")
