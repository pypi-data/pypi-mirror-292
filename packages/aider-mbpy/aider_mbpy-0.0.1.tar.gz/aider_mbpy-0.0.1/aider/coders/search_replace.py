...
class RelativeIndenter:
    ...
    def make_relative(self, text) -> str:
...
def proc(dname):
    ...
    try:
        search_text = Path(dname / "search").read_text()
        replace_text = Path(dname / "replace").read_text()
        original_text = Path(dname / "original").read_text()
    except FileNotFoundError:
        print(f"Directory {dname} does not contain search, replace, and original files.")
