...
class Settings(object):
    ...
    def Config(cls, config_file="config.in"):
        ...
        if len(cls.BlueStackdir) > 0:
            ...
            # MuMu提供了开启关闭的console, 不用像BlueStackdir检索PID关闭
            for i in range(5):
                cls.win_Instance[i] = f"{i}"
            emulator = 'MuMu'
        if len(emulator) > 0:
...
class deviceOB:
    ...
    def 启动设备(self):
        ...
        # 开始运行
        # 控制不同的节点在不同的时间启动
        hour, minu = DQWheel.time_getHM()
        while minu % self.totalnode != self.mynode:
            sleep(10)
            hour, minu = DQWheel.time_getHM()
        ...
        if self.客户端 == "win_LD":  # 让LDPlayer在后台运行
            ...
            win32api.keybd_event(17, 0, win32con.KEYEVENTF_KEYUP, 0)
        if self.客户端 == "win_MuMu":  # 让MuMu在后台运行
            TimeECHO(f"MuMu: alt+Q")
            import win32api
            import win32con
