...
class Rule_EasyQL_LT02(BaseRule):
    ...
    **Anti-pattern**
    ...
    The ``•`` character represents a space and the ``→`` character represents
    a tab. In this example, the third line contains five spaces instead of four
    and the second line contains two spaces and one tab.
    ...
    **Best practice**
    ...
    def _eval(self, context: RuleContext) -> List[LintResult]:
        ...
        """
        reflows: List[LintResult] = list()

        for sub_segment in context.segment.segments:
            if len(sub_segment.segments) > 0 and \
                    "create" in sub_segment.segments[0].type:
                reflows.extend(self.handle_create_layout())
            else:
                reflows.extend(
