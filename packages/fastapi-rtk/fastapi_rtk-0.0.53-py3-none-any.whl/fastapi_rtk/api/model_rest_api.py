...
class ModelRestApi(BaseApi):
    ...
    async def info_headless(
        ...
        db: AsyncSession | Session = SelfDepends().get_api_db,
    ):
        """
        ...
        Args:
            ...
            db (AsyncSession | Session): An asynchronous database session.
        ...
        """
        info = (
            (await smart_run(self._generate_info_schema, permissions, db))
            if self._default_info_schema
            else self.info_return_schema()
        )
        await smart_run(self.pre_info, info, permissions, db)
        return info

    async def pre_info(
        self, info: InfoResponse, permissions: List[str], db: AsyncSession | Session
    ):
        """
    ...
    async def _generate_info_schema(
        self, permissions: List[str], db: AsyncSession | Session
    ):
        """
        ...
        Args:
            ...
            db (AsyncSession | Session): The database session.
    ...
    async def _get_column_info(
        ...
        Args:
            ...
            db (AsyncSession | Session): The async db object for the database connection.
