...
from fastapi_rtk import g, metadata, session_manager
...
async def run_async_migrations() -> None:
    ...
    try:
        engine_url = session_manager._engine.url.render_as_string(
            hide_password=False
        ).replace("%", "%%")
    except AttributeError:
        engine_url = str(session_manager._engine.url).replace("%", "%%")
