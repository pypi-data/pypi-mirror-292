...
def open_ui_from_ui_list(ui, ui_view, swipe_point, first_ui, final_ui, fun_name="air"):
    """
...
def open_ui(ui, ui_view, fun_name="air"):
    @check_func(f'正在打开{ui}界面')
    def _open_ui():
        if operate(fun_name).wait_next_element(ui, ui_view) is True:
            air.sleep(1)
...
def close_ui(ui_view, close_pos, fun_name='air'):
    """
...
def click_ui(ui, ocrPlus=None, fun_name='air'):
    @check_func(f"点击{ui}")
    def _click_ui():
        if ocrPlus:
...
def click_ui_from_ui_list(ui, swipe_point, first_ui, final_ui, fun_name='air'):
...
def open_list(list_pos, next_pos, fun_name="air"):
...
def check_scenes(scenes_pos, wait_pos, fun_name="air"):
...
def swip_ui(swipe_pos, fun_name="air"):
...
def Plot(dialogue_ui_list, stop_ui, choice_ui=None, jump_ui=None, start_ui=None, log_path=None,
         fun_name="air"):
    """
...
class FuncListCheckPage(TaskCaseTemplate):
    ...
    def scenes_jump(self):
        open_list("Scene", "场景跳转")
        check_scenes("斗罗场景", "WaitingBar")
        open_list("Scene", "场景跳转")
        check_scenes("TestShader", "WaitingBar")
        open_list("Scene", "场景跳转")
        check_scenes("斗罗场景 (流式加载)", "WaitingBar")

    @put_task
    def she_zhi_view(self):
        open_list("功能", "功能列表")
        open_ui_from_ui_list("设置", "SettingsPage", "FuncListSwipe", "LanguageRoom", "Multilingual")
        close_ui("SettingsPage", "CloseSet")

    @put_task
    def hua_zhi_view(self):
        open_list("Func", "功能列表")
        open_ui_from_ui_list("画质", "PictureQualityPage", "FuncListSwipe", "LanguageRoom", "Multilingual")
        click_ui("veryLow")
        click_ui("Application")
        click_ui("Low")
        click_ui("Application")
        click_ui("Mid")
        click_ui("Application")
        click_ui("其它")
        click_ui("画质")
        close_ui("PictureQualityPage", "BackButton")

    def reloadall_check(self):
        open_list("Func", "功能列表")
        click_ui_from_ui_list("reloadall", "FuncListSwipe", "LanguageRoom", "Multilingual")

    @put_task
    def ji_neng_view(self):
        open_list("Func", "功能列表")
        open_ui_from_ui_list("技能装配UI", "skillView", "FuncListSwipe", "LanguageRoom", "Multilingual")
        click_ui("skillViewAttackKey")
        close_ui("skillView", "BackButton")

    def test_load_rawdata(self):
        open_list("Func", "功能列表")
        click_ui_from_ui_list("testloadrawdata", "FuncListSwipe", "LanguageRoom", "Multilingual")

    @put_task
    def demo_ui_view(self):
        open_list("Func", "功能列表")
        open_ui_from_ui_list("DemoUI", "demoUIView", "FuncListSwipe", "LanguageRoom", "Multilingual")
        click_ui("点击发事件")
        click_ui("列表组件")
        click_ui("基础组件")
        close_ui("demoUIView", "BackButton")

    @put_task
    def sound_setting_view(self):
        open_list("Func", "功能列表")
        open_ui_from_ui_list("soudsetingview", "MusicSettingInterface", "FuncListSwipe", "LanguageRoom", "Multilingual")
        click_ui("使用Wwise", ocrPlus=True)
        click_ui("Destroy Audio")
        click_ui("使用 UnityAudio")
        click_ui("创建机关")
        close_ui("MusicSettingInterface", "CloseMusicView")

    @put_task
    def DemoRTShowHighQualityShadowView(self):
        open_list("Func", "功能列表")
        open_ui_from_ui_list("DemoRTShowHighQualityShadowView", "HDshadowModelInterface", "FuncListSwipe",
                             "LanguageRoom",
                             "Multilingual")
        close_ui("HDshadowModelInterface", "BackButton")

    @put_task
    def DemoRTShowOrthographicView(self):
        open_list("Func", "功能列表")
        open_ui_from_ui_list("DemoRTShowOrthographicView", "DemoRTShowOrthographicViewIn", "FuncListSwipe",
                             "LanguageRoom",
                             "Multilingual")
        close_ui("DemoRTShowOrthographicViewIn", "BackButton")

    @put_task
    def DemoRTShowPlanarShadowView(self):
        open_list("Func", "功能列表")
        open_ui_from_ui_list("DemoRTShowPlanarShadowView", "PlanarShadow", "FuncListSwipe", "LanguageRoom",
                             "Multilingual")
        close_ui("PlanarShadow", "BackButton")

    @put_task
    def SRPBatcherProfiler(self):
        open_list("Func", "功能列表")
        open_ui_from_ui_list("SRPBatcherProfiler", "关闭SRP Batcher", "FuncListSwipe", "LanguageRoom",
                             "Multilingual")
        click_ui("关闭SRP Batcher")
        click_ui("开启SRP Batcher")
        open_list("Func", "功能列表")
        close_ui("关闭SRP Batcher", "SRPBatcherProfiler")

    @put_task
    def TestUIMoldeClipView(self):
        open_list("Func", "功能列表")
        open_ui_from_ui_list("TestUIMoldeClipView", "TestMoldeView", "FuncListSwipe", "LanguageRoom",
                             "Multilingual")
        swip_ui("TestMoldeSwipe")
        click_ui("打开新")
        click_ui("TestUIMoldeClipView")
        close_ui("TestMoldeView", "TestMoldeCloseBtn")

    def check_for_updates(self):
        open_list("Func", "功能列表")
        click_ui_from_ui_list("checkForUpdates", "FuncListSwipe", "LanguageRoom", "Multilingual")

    def shot_cut(self):
        open_list("Func", "功能列表")
        click_ui_from_ui_list("镜头切换", "FuncListSwipe", "LanguageRoom", "Multilingual")
        click_ui_from_ui_list("镜头切换", "FuncListSwipe", "LanguageRoom", "Multilingual")
        click_ui_from_ui_list("镜头切换", "FuncListSwipe", "LanguageRoom", "Multilingual")
    ...
    def xie_yi_view(self):
        open_list("Func", "功能列表")
        open_ui_from_ui_list("Proto", "测试发送", "FuncListSwipe", "LanguageRoom", "Multilingual")
        click_ui("恢复监听发送")
        click_ui("恢复监听接收")
        click_ui("测试发送")
        click_ui("清空")
        close_ui("测试发送", "ProtoCloseBtn")

    @put_task
    def shi_pin(self):
        open_list("Func", "功能列表")
        open_ui_from_ui_list("AVPro视频", "VideoView", "FuncListSwipe", "LanguageRoom", "Multilingual")
        close_ui("VideoView", "VideoInterfaceCloseButton")

    @put_task(is_recording=True)
    def ce_shi_ju_qin1(self, log_path):
        open_list("Func", "功能列表")
        open_ui_from_ui_list("测试剧情1", "TestPlotBlackCurtain1", "FuncListSwipe", "LanguageRoom", "Multilingual")
        Plot(["最近怪物横行，请少侠前往击杀"], "功能", ["挑战Boss"], start_ui="TestPlotStart1", log_path=log_path)

    @put_task(is_recording=True)
    def ce_shi_ju_qin2(self, log_path):
        open_list("Func", "功能列表")
        open_ui_from_ui_list("测试剧情2", "TestPlotBlackCurtain2", "FuncListSwipe", "LanguageRoom", "Multilingual")
        Plot(["最近怪物横行，请少侠前往击杀", "这个小怪有点强，你要不要和我一起去？", "不是吧，小怪还要我帮忙啊！",
