...
class iTracer(object):
    ...
    def getTracerId(self):
        return _flumy.iTracer_getTracerId(self)
...
# Remove all deposition units having null thickness (age hiatus)
  ...
  nclass = MD_NB_CLASS_GR
  bins = np.arange(nclass+1)
...
# Show VPC from a 3D block facies and global proportions
def showVPC(simu, dz, zmin=None, zmax=None, keys=None, title="VPC", filepng=None):
...
# Get facies 3D regular block
  nz = int(round((zmax-zmin)/dz, 0) + 1)
  fac_arr,g,a = simu.getBlock(dz, zb=zmin, nz=nz)

# Retrieve facies labels
  if (keys is None):
    props = getProps(fac_arr)
    keys  = props.keys()
  ...
  left = np.zeros(nz)
  width = 1.0
  for lab, pr in vpc.items():
    p = ax.barh(levels, pr, width, label=lab, left=left, color=fac_col[fac_lab.index(lab)])
    left += pr
...
# Default title
  ...
  for i in range(gr.shape[0]):
    ax.bar(np.arange(nclass), gr[i], bottom = np.sum(gr[:i], axis = 0), color=fac_col[i])
  if ylim is not None:
    plt.ylim(0, ylim)
...
# Show topography
def showTopo(topo, size=5, title="", legend=0, filepng=None):
  """
...
# Show facies section
def showSectionFac(fac, size=5, title="", legend=0, full_legend=False, filepng=None):
  """
...
# Plot the slice
  ...
  fig, ax = plt.subplots(figsize=(size,size))
  im = ax.imshow(newfac, cmap=cmap, norm=norm, origin='lower')
  plt.title(title)
  if (legend > 0):
...
def showSectionGrain(grain, size=5, title="", legend=0, filepng=None):
  """
  legend: 0: no legend, 1: right, 2: bottom
  """
...
# Default title
  ...
  fig, ax = plt.subplots(figsize=(size,size))
  im = ax.imshow(grain, origin='lower')
  plt.title(title)
...
def showSectionAge(age, size=5, title="", legend=0, filepng=None):
  """
  legend: 0: no legend, 1: right, 2: bottom
  """
...
def showSection(fac=None, grain=None, age=None, size=5, title="",
                legend=0, full_legend=False, filepng=None):
  """
  ...
  if (np.any(fac)):
    showSectionFac(fac, size, title, legend, full_legend, filepng)
  elif (np.any(grain)):
    showSectionGrain(grain, size, title, legend, filepng)
  elif (np.any(age)):
    showSectionAge(age, size, title, legend, filepng)



def showProbaSection(proba, size=5, title="", legend=0, filepng=None):
  """
  legend: 0: no legend, 1: right, 2: bottom
  """
...
# Default title
  ...
  fig, ax = plt.subplots(figsize=(size,size))
  im = ax.imshow(proba, origin='lower', vmin=0, vmax=1)
  plt.title(title)
