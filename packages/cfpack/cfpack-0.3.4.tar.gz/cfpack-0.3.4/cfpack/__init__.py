...
def round(x, nfigs=3, str_ret=False):
    if x == 0: # special case where input is 0
        rounded_x = x
    else:
        ...
        rounded_x = int(np.round(scaled_x * 10**nfigs))/10**(nfigs-order_of_magnitude)
    # in case the user requested a string to be returned
    if not str_ret:
        return rounded_x # return rounded value
    else:
        parts = str(rounded_x).split('e') # split into scale part and exp part
        scale_part = parts[0]
        negative_x = False
        ...
        rounded_x_str = scale_part
        if len(parts) == 2:
            rounded_x_str += "e"+parts[1] # re-attach the exp part
