...
class Occurrence(QGraphicsRectItem):
    ...
    def __init__(
        ...
        begin_time: int,
        end_time: int,
        # FIXME: [CREATION] are those really needed?
        lower_bound: int = None,
        upper_bound: int = None,
    ):
        """Initializes the Occurrence widget"""
        super().__init__(timeline)
        self.brush_color = self.DEFAULT_BG_COLOR
        self.pen_color = self.DEFAULT_PEN_COLOR
        self.pen_width = self.PEN_WIDTH_OFF_CURSOR
        self.event = None
        self.name = None
        ...
        self.setRect(
            QRectF(
                ...
                self.timeline.FIXED_HEIGHT - self.timeline.title.FIXED_HEIGHT,
            )
        )

        self.setX(begin_x_position)
        self.setY(self.timeline.title.FIXED_HEIGHT)
        self.lower_bound = lower_bound
        self.upper_bound = upper_bound
        self.comment: str = ""

    # FIXME: [CREATION] move into Timeline
    def can_be_initiated(occurrences, time):
            ...
            if not lower_bound:
                ...
                if a.end_time < time:
                    if lower_bound < a.end_time:
                        lower_bound = a.end_time + int(a.mfps / 2)
            if not upper_bound:
                if a.begin_time > time:
                    upper_bound = a.begin_time - int(a.mfps / 2)
            else:
                if a.begin_time > time:
                    if upper_bound > a.begin_time:
                        upper_bound = a.begin_time - int(a.mfps / 2)
