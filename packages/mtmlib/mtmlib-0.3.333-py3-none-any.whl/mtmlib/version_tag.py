...
def patch_git_tag_version():
    ...
    _, tag_push_err = exec_shell_command("git", "push", "origin", new_patch_version)
    if tag_push_err:
        msg = f"推送标签失败: {tag_push_err}"
        raise TagVersionError(msg)

    logger.info("新版本标签已创建并推送: {new_patch_version}")
...
def read_tag_version_next():
    ...
    if current_tag_rev != current_tag:
        version["patch"] += 1
    logger.info("当前tag %s, next tag: %s", current_tag, version)
    return version
