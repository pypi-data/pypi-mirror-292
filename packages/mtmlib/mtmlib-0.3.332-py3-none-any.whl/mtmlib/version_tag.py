...
def patch_git_tag_version():
    ...
    # 创建并推送新标签
    tag_creation, tag_creation_err = exec_shell_command("git", "tag", new_patch_version)
    if tag_creation_err:
        raise TagVersionError(f"创建标签失败: {tag_creation_err}")

    tag_push, tag_push_err = exec_shell_command(
        "git", "push", "origin", new_patch_version
    )
    if tag_push_err:
        raise TagVersionError(f"推送标签失败: {tag_push_err}")

    print(f"新版本标签已创建并推送: {new_patch_version}")
...
def read_tag_version_next():
    """读取下一个版本号"""
    current_tag, err = exec_shell_command("git", "describe", "--tags")
    print("当前版本号", current_tag)
    if err:
        raise TagVersionError(f"无法读取当前标签: {err}")
