...
from .bullet import colors, ScrollBar
...
def update_package():
    ...
    if LooseVersion(newest_version) > LooseVersion(this_version):
        print(f'A newer version of slurm-jupyter exists ({newest_version})')
        cmd = f"conda install -y -q -c mbg-exercises -c conda-forge mbg-exercises={newest_version}"
        os.system(cmd)
...
def get_week_mapping():
    pass


def check_no_other_exercise_container_running():
    pass


def check_no_other_local_jupyter_running():
    pass


def purge_docker_images_and_containers(image_name_list):
    pass
...
def launch_exercise():
    ...
    # update_package() # enable once uploaded to anaconda
    ...
    registry = 'https://gitlab.au.dk/api/v4/projects/au81667%2Fmbg-docker-exercises/registry/repositories'
    ...
    with subprocess.Popen(shlex.split(cmd), stdout=DEVNULL, stderr=DEVNULL) as docker_run_p:
        ...
        try:
            ...
            with subprocess.Popen(shlex.split(cmd), stdout=PIPE, stderr=STDOUT, bufsize=1, universal_newlines=True) as docker_log_p:
                # try block to allow normal exceptions in subprocess with-block
                try:
                    while True:
                        time.sleep(0.1)
