...
class AllOptionsSDK:
    ...
    async def query(self, conditions: str):
        """Query the database using SQL and return a spreadsheet."""
        await db.connect()


        # Define the base query
        base_query = "SELECT distinct ticker, strike, call_put, expiry, bid, ask FROM master_all_two"
        ...
        # Columns expected from the query
        columns = ['sym', 'strk', 'cp', 'exp', 'bid', 'ask']

        # Debugging: print the number of columns and data returned
        print("Number of columns expected:", len(columns))
        if results:
        ...
        if results and len(columns) != len(results[0]):
            raise ValueError(f"Mismatch between extracted columns ({len(columns)}) and data columns ({len(results[0])}).")

        # Create a DataFrame from the results
        df = pd.DataFrame(results, columns=columns)
        # Convert DataFrame to a table string
