...
class CMADaasAccess():
    ...
    def read_data_from_cmadaas(fullurl):
        try:
            ...
            if 'returnCode' not in jsonobj:
                LogLib.error(
                    'CMADaasAccess read_data_from_cmadaas json no returnCode %s %s' % (rstcontent, str(fullurl)))
                return None
            ...
            if jsonobj['returnCode'] != '0':
                if jsonobj['returnCode'] == '-1':
                    ...
                    LogLib.error('CMADaasAccess read_data_from_cmadaas json no returnCode or returnCode != 0 %s %s' % (
                    rstcontent, str(fullurl)))
                    return None
    ...
    def dataframe_from_dict_json(j_dict, default=np.nan):
        import pandas as pd
        try:
            LogLib.info('CMADaasAccess Convert dataFrame from JSON_dict {0}'.format(
            ...
            LogLib.error('CMADaasAccess Convert dataFrame from JSON_dict {0}'.format(
                datetime.datetime.now().strftime("%Y%m%d%H%M")))
            return (None)
    ...
    @staticmethod
    def get_obs_micaps3_from_cmadaas(qparams, time="202011110200", save_path=None, filename=None, defaultNan=9999,
                                     show=False,
                                     **kargs):
        """
    ...
    @staticmethod
    def get_files_from_cmadaas(qparams, time='202011110200', save_path=None, filename=None, show_files=True,
                               is_new=True,
                               **kargs):
        """
        time： 可以为字符串str，如“202011110200”；也可以为datetime类时间数据
        ...
        try:
            ...
            print("ERROR:", data)
            LogLib.error(
                '{0} CMADaasAccess Files output ERROR!: {1}'.format(datetime.datetime.now().strftime("%Y%m%d%H%M"),
                                                                    data))
