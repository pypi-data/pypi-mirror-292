...
def set_customized_shpfile_list(shpfile_list = None):
    if shpfile_list is None:
        meteva_base.customized_basemap_list = None
    else:
        ...
        for shpfile_filename in shpfile_list:
            if shpfile_filename.find("/") <0 and shpfile_filename.find("\\") <0:
                ...
                meteva_base.customized_basemap_list.append(shpfile_filename)
    #print(meteva_base.customized_basemap_list)
...
def add_china_map_2basemap(ax,name ="province", facecolor='none',
    ...
    if meteva_base.customized_basemap_list is None:
        ...
        for shpfile in meteva_base.customized_basemap_list:
            #print(shpfile)
...
def contourf_2d_grid(grd,save_path = None,title = None,clevs= None,cmap ="rainbow",add_county_line = False,add_worldmap =False,show = False,dpi = 300,
    ...
    if isinstance(grd,list):
        ...
        if subplot is not None:
            ...
            for s in subplot:
                split.remove(s)
        grd_list = meteva_base.split_grd(grd, used_coords=split)
    ...
    for i in range(len(grd_list)):
        ...
        if sup_title is None:
            ...
            time_str = meteva_base.tool.time_tools.all_type_time_to_str(grd1["time"].values[0])
        ...
        if subplot is None:
            ...
            grd_list1 = meteva_base.split_grd(grd1, used_coords=subplot)
        ...
        if title is not None:
            ...
            for kk in range(len(grd_list1)):
                ...
                time_str = meteva_base.tool.time_tools.all_type_time_to_str(grd2["time"].values[0])
                dati_str = time_str[0:4] + "年" + time_str[4:6] + "月" + time_str[6:8] + "日" + time_str[8:10] + "时"
    ...
    for p in range(nplot):
        ...
        if title is None:
            try:
                ...
                time_str = meteva_base.tool.time_tools.time_to_str(grd_list[p]["time"].values[0])
                dati_str = time_str[0:4] + "年" + time_str[4:6] + "月" + time_str[6:8] + "日" + time_str[8:10] + "时"
...
def pcolormesh_2d_grid(grd,save_path = None,title = None,clevs= None,cmap = "rainbow",add_county_line = False,add_worldmap=False,show = False,dpi = 300,
    ...
    if title is None:
        time_str = meteva_base.tool.time_tools.time_to_str(grid0.gtime[0])
        dati_str = time_str[0:4] + "年" + time_str[4:6] + "月" + time_str[6:8] + "日" + time_str[8:10] + "时"
...
def scatter_sta(sta0,value_column=None,
    ...
    if isinstance(map_extend,list):
        ...
        rlon = elon - slon
        rlat = elat - slat
    elif isinstance(map_extend,meteva_base.grid):
    ...
    #clevs1, cmap1 = meteva_base.tool.color_tools.def_cmap_clevs(clevs=clevs, cmap=cmap, vmin = None, vmax=None)
    #meteva_base.tool.color_tools.show_cmap_clev(cmap1,clevs1)
    ...
    if subplot is None:
        ...
        sta_list = meteva_base.split(sta0,used_coords=split)
        nplot = len(sta_list)
        ...
        for p in range(nplot):
            ...
            #sta_one_member = meteva_base.sele_by_para(sta,member=[data_name],drop_IV=True)
            sta_one_member = sta_list[p]
            data_name = meteva_base.get_stadata_names(sta_one_member)[0]
            ...
            if title is None:
                try:
                    time_str = meteva_base.tool.time_tools.time_to_str(sta_one_member.iloc[0, 1])
                    dati_str = time_str[0:4] + "年" + time_str[4:6] + "月" + time_str[6:8] + "日" + time_str[8:10] + "时"
            ...
            if save_path1 is not None:
                meteva_base.tool.path_tools.creat_path(save_path1)
        ...
        if sup_title is None:
            ...
            for i in range(ng):
                sta0 = sta_list[i]
                if subplot==["level"]:
                    data_names = meteva_base.get_stadata_names(sta0)
                    time_str = meteva_base.tool.time_tools.time_to_str(sta0.iloc[0, 1])
                    dati_str = time_str[0:4] + "年" + time_str[4:6] + "月" + time_str[6:8] + "日" + time_str[8:10] + "时"
        ...
        for n in range(ng):
            sta1 = sta_list[n]
            sta_g1  = meteva_base.split(sta1,used_coords = subplot)
            ng1 = len(sta_g1)
            if isinstance(title,list):
...
def scatter_sta_list(sta0_list,map_extend = None,add_county_line = False,add_worldmap = False,
    ...
    title1 = None
    for p in range(nplot):
        sta_one_member = meteva_base.sele_by_para(sta0_list[p],drop_IV=True)
        data_name = meteva_base.get_stadata_names(sta_one_member)
...
def plot_bar(plot_type,array,name_list_dict = None,legend = None,axis = None,ylabel = "Value",vmin = None,vmax = None,ncol = None,grid = None,tag = -1,save_path = None,show = False
    ...
    if len(array[array!=meteva_base.IV]) ==0:
        print("所有的值都为缺失值")
        return
    ...
    if len(shape) ==1:
        ...
        if isinstance(x_one,datetime.datetime):
            xticks_labels = meteva_base.get_time_str_list(name_list_dict[axis],3)
        else:
            xticks_labels = []
        ...
        width_axis = meteva_base.plot_tools.caculate_axis_width(xticks_labels, sup_fontsize,legend_num= 1)
        ...
        xticks = x[::spasify]
        if isinstance(x_one,datetime.datetime):
            xticks_labels = meteva_base.get_time_str_list(name_list_dict[axis][::spasify],3)
        else:
        ...
        y_plot = array[array != meteva_base.IV]
        x_plot = x[array!=meteva_base.IV]

        if log_y:
            array[array == 0] = meteva_base.IV
        if vmin is None:
            vmin1 = np.min(array[array != meteva_base.IV])
        else:
            vmin1 = vmin
        if vmax is None:
            vmax1 = np.max(array[array != meteva_base.IV])
        else:
            vmax1 = vmax
        ...
        if plot_type == "bar":
            ...
            dat0 = array
            index_iv = np.where(dat0 == meteva_base.IV)
            if len(index_iv) == 0:
                if color_list is None:
                    plt.plot(x, dat0)
                else:
                ...
                dat0_notiv[dat0_notiv == meteva_base.IV] = np.nan
                if color_list is None:
            ...
            if tag >= 0:
                for ii in range(len(dat0)):
                    a = x[ii]
                    b = dat0[ii]
                    if np.isnan(b) or b == meteva_base.IV: continue
                    va = "center"
        ...
        width_axis = meteva_base.plot_tools.caculate_axis_width(xticks_labels, sup_fontsize,legend_num)
        width_axis_labels = meteva_base.plot_tools.caculate_axis_width(xticks_labels, sup_fontsize, 1)
        ...
        if plot_type == "bar":
            ...
            for i in range(legend_num):
                ...
                dat0 = dat[i,:]
                y_plot = dat0[dat0 != meteva_base.IV]
                x_plot = x1[dat0 != meteva_base.IV]
                if color_list is None:
                    if meteva_base.plot_color_dict is not None and legend_list[i] in meteva_base.plot_color_dict.keys():
                        color_set1 = meteva_base.plot_color_dict[legend_list[i]]
                ...
                if len(dat0[dat0 == meteva_base.IV]) > 0:
                    x_iv = x1[dat0 == meteva_base.IV]
                    y_iv = np.zeros(x_iv.size)
                    plt.plot(x_iv, y_iv, "^", color='k')
                if tag >= 0:
                    # add data tag
        ...
        if isinstance(x_one,datetime.datetime):
            ...
            xticks_labels = meteva_base.get_dayofyear_str_list(name_list_dict[axis])
        else:
            xticks_labels = []
            for local in name_list_dict[axis]:
                if isinstance(local,float):
        ...
        if isinstance(x_one,datetime.datetime):
            ...
            xticks_labels = meteva_base.get_dayofyear_str_list(name_list_dict[axis][::spasify])
        else:
            xticks_labels = xticks_labels[::spasify]
        ...
        for k in range(subplot_num):
            ...
            data_k = data[k,:,:]
            if log_y:
                data_k[data_k == 0] = meteva_base.IV
            if vmin is None:
                dat_k0 = data_k[data_k != meteva_base.IV]
                if dat_k0.size>0:
                    vmin1 = np.min(dat_k0)
                else:
                    vmin1 = 0
            ...
            for i in range(legend_num):
                if plot_type == "bar":
                    ...
                    x_plot = x1[dat0 != meteva_base.IV]
                    if color_list is None:
                        if meteva_base.plot_color_dict is not None and name_list_dict[legend][i] in meteva_base.plot_color_dict.keys():
                            color_set1 = meteva_base.plot_color_dict[name_list_dict[legend][i]]
                            if k == 0:
    ...
    if save_path is None:
        ...
        meteva_base.creat_path(save_path)
...
def bar_line(array,type_list,name_list_dict = None,legend = None,axis = None,vmin_bar = None,vmax_bar = None,
    ...
    if len(shape)==2:
        ...
        if vmin_bar is None:
            vmin1_bar = np.min(dat_bar[dat_bar != meteva_base.IV])
        else:
            vmin1_bar = vmin_bar
        if vmax_bar is None:
            vmax1_bar = np.max(dat_bar[dat_bar != meteva_base.IV])
        else:
            vmax1_bar = vmax_bar
        ...
        if vmin_line is None:
            vmin1_line = np.min(dat_line[dat_line != meteva_base.IV])
        else:
            vmin1_line = vmin_line
        if vmax_line is None:
            vmax1_line = np.max(dat_line[dat_line != meteva_base.IV])
        else:
        ...
        for k in range(subplot_num):
            ...
            for i in range(legend_num):
                if type_list[i] == "bar":
                    if log_y_bar:
                        data_k[i, data_k[i, :] == 0] = meteva_base.IV
            ...
            if vmin_bar is None:
                dat_k0 = dat_bar[dat_bar != meteva_base.IV]
                if dat_k0.size > 0:
            ...
            if vmin_line is None:
                dat_k0 = dat_line[dat_line != meteva_base.IV]
                if dat_k0.size > 0:
            ...
            for i in range(legend_num):
                if type_list[i] == "bar":
                    ...
                    x_plot = x1[dat0 != meteva_base.IV]
                    if color_list is None:
                        if meteva_base.plot_color_dict is not None and name_list_dict[legend][
                            i] in meteva_base.plot_color_dict.keys():
                            color_set1 = meteva_base.plot_color_dict[name_list_dict[legend][i]]
                            if k == 0:
                                plt.bar(x_plot, y_plot, width=width * 0.95, label=name_list_dict[legend][i],
                                        color=color_set1)
...
def myheatmap(ax_one,data_0,cmap,clevs,annot=1,fontsize=10):
    ...
    data_k[data_k==meteva_base.IV] = np.nan
...
def mesh(array,name_list_dict = None,axis_x = None,axis_y = None,cmap = "rainbow",clevs = None,ncol = None,annot =None,save_path = None,show = False,dpi = 300,
    ...
    if len(shape) == 3 or len(shape) == 2:
        ...
        if isinstance(x_one, datetime.datetime):
            xticks_labels = meteva_base.get_time_str_list(name_list_dict[axis_x], 3)
        else:
            xticks_labels = []
        ...
        xticks = x[::spasify]
        if isinstance(x_one, datetime.datetime):
            xticks_labels = meteva_base.get_time_str_list(name_list_dict[axis_x][::spasify], 3)
        else:
            xticks_labels = xticks_labels[::spasify]
        ...
        data[data == meteva_base.IV] = np.nan
        if not isinstance(cmap,list):
            data_copy = copy.deepcopy(data)
            data_copy[np.isnan(data_copy)] = meteva_base.IV
            vmin = np.min(data_copy[data_copy != meteva_base.IV])
            vmax = np.max(data_copy[data_copy != meteva_base.IV])
            cmap,clevs = meteva_base.color_tools.def_cmap_clevs(cmap=cmap, clevs=clevs, vmin=vmin, vmax=vmax)
        ...
        for k in range(subplot_num):
            ...
            if isinstance(cmap, list):
                ...
                data_k_copy[np.isnan(data_k_copy)] = meteva_base.IV
                vmin = np.min(data_k_copy[data_k_copy != meteva_base.IV])
                vmax = np.max(data_k_copy[data_k_copy != meteva_base.IV])
            else:
                cmap0 = cmap
                clevs0 = clevs
            cmap1,clevs1= meteva_base.color_tools.def_cmap_clevs(cmap = cmap0,clevs=clevs0,vmin=vmin,vmax = vmax)
            ax_one = plt.subplot(nrow, ncol, k + 1)
...
def mesh_obtime_time(sta0,save_dir = None,save_path = None,
    ...
    dh_x = meteva_base.tool.math_tools.greatest_common_divisor(dhs_ob_not0)
    #print(dh_x)
    np.sum(dhs_fo_not0)
    ...
    t_ob = []
    for t in times_ob:
        t_ob.append(meteva_base.all_type_time_to_datetime(t))

    y_ticks = []
    t_fo0= meteva_base.all_type_time_to_datetime(times_fo[0])
    ...
    width0 = col * 0.1 + 2
    height0 = row * 0.1 + 2
    x_plot,x_ticks = meteva_base.get_x_ticks(times_ob,width0-2,row=3)
    #sup_fontsize = 10
    ...
    x_plot /= dh_x
    #y_plot, y_ticks = meteva_base.get_y_ticks(times_fo, height)
    if xtimetype == "right":
        x_plot  = x_plot+0.5
    elif xtimetype == "left":
        x_plot = x_plot -0.5
    ...
    dat[np.isnan(dat)] = meteva_base.IV
    vmin = np.min(dat[dat != meteva_base.IV])
    vmax = np.max(dat[dat != meteva_base.IV])


    if cmap is None:
        cmap = "bwr"
    cmap_part,clevs_part = meteva_base.tool.color_tools.def_cmap_clevs(cmap=cmap,clevs=clevs,vmin=vmin,vmax = vmax)

    #if cmap is None:
    #    cmap = "bwr"
    #    cmap_part = cmap
    #if clevs is not None:
    #    clev_part, cmap_part = meteva_base.tool.color_tools.get_part_clev_and_cmap(clevs, cmap, vmax, vmin)
    vmax = clevs_part[-1]
    ...
    for d in range(nfo):
        ...
        sta_one_member = meteva_base.in_member_list(sta, [data_name])
        #meteva_base.set_stadata_names(sta_ob_part2, [data_name])
        #sta_one_member = meteva_base.combine_join(sta_ob_part2, sta_fo_all2)
        #以最近的预报作为窗口中间的时刻
        for id in ids:
            sta_one_id = meteva_base.in_id_list(sta_one_member,id)
            dat = np.ones((col, row)) * meteva_base.IV

            for j in range(row):
                jr = row - j - 1
                time_fo = times_fo[0] + np.timedelta64(1, 'h') * dh_y * jr
                sta_on_row = meteva_base.in_time_list(sta_one_id,time_fo)
            ...
            mask[dat.T == meteva_base.IV] = True
...
def mesh_obtime_dtime(sta0,save_dir = None,save_path = None,
    ...
    for d in range(nfo):
        ...
        for id in ids:
            ...
            dat = np.ones((col, row)) * meteva_base.IV
            for j in range(row):
                sta_on_row = meteva_base.in_dtime_list(sta_one_id,dhs_fo[j])
                #print(sta_on_row)
...
def mesh_time_dtime(sta0,save_dir = None,save_path = None,
    ...
    dh_x = meteva_base.tool.math_tools.greatest_common_divisor(dhs_x_not0)
    ...
    x_plot, x_ticks = meteva_base.get_x_ticks(times_fo, width - 2,row = 3)
    y_plot, y_ticks = meteva_base.get_y_ticks(dhs_fo, height,sup_fontsize * 0.8)
...
def barbs_grid_wind(grid_wind,width = None,height = None,sup_fontsize = 12,dpi = 300,skip = None,save_path = None,show = False):
    grid1 = meteva_base.get_grid_of_data(grid_wind)
    axs = meteva_base.creat_axs(1, map_extend=grid1, add_minmap=False, ncol=2, width=width,height=height,sup_fontsize=sup_fontsize,dpi=dpi)
    meteva_base.add_barbs(axs[0], grid_wind,skip=skip)
