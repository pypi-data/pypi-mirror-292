...
def generate_bcr_sequences(model_path, num_sequences=100, chain_type="light", starting_residue=""):
...
def save_best_sequences(filename, sequences):
    with open(filename, 'w') as file:
        for seq, ppl in sequences:
            cleaned_seq = seq.replace("<| text|>", "").strip()
...
def generate_bcr_library(args):
    ...
    for starting_residue in residues:
        sequences = generate_bcr_sequences(
            model_path=args.model_path,
            num_sequences=num_sequences_each_starting_residue,
            chain_type=args.chain_type,
