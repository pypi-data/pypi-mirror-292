...
class ScheduleUpdater(Updater):
    def __init__(
        self, name: str, qs: QuerySet, update_func: Callable, period: timedelta
    ) -> None:
        self.name = name
        self.queue_name = f"{ScheduleUpdater.redis_prefix()}:queue:{name}"
        self.qs = qs
    ...
    def redis_prefix() -> str:
        return f"{settings.ENV.service_name}:wise:schedule_updater:"
