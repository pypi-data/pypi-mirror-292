...
class Logger:
	...
	# 생성됨.
	#--------------------------------------------------------------------------------
	def __init__(self, name : str, path : str, level : Level) -> None:

		# 생성.
		self.__ansicode = ANSICODE()
		self.__internalLogger = logging.getLogger(name)

		# 로그 수준 설정.
		self.__internalLogger.setLevel(level.value)
		...
		fileHandler : StreamHandler = FileHandler(path, encoding = UTF8)
	...
	# 로그 출력.
	#--------------------------------------------------------------------------------
	def PrintWithColor(self, text : str, level : Level) -> None:
