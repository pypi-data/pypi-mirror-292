...
class ExchangeTable:
    ...
    def to_msg(self) -> bytearray:
        ...
        for attribute in self.attributes:
            ...
            if byte_nb == 4: # DWord data need alignement
                # 32-bit/ 1DWord alignement
                if len(byte_array)%4 == 2:
                    # add reserved WORD
                    reserved = int(0)
                    byte_array.extend(reserved.to_bytes(2, "little"))
                elif len(byte_array)%4 != 0 :
                    print(f"EXCHANGE TABLE WARN, padding of {len(byte_array)%4} bytes")
                    reserved = int(0)
                    byte_array.extend(reserved.to_bytes(len(byte_array)%4, "little"))
...
class I40Table(ExchangeTable):
    ...
    def set_exception(self, exception_code: bool): #FIXME: is it int ?
