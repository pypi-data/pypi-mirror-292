...
def bypass(link: str, session: Optional[requests.Session] = None) -> str:
	"""Bypasses a Linkvertise link and returns the final URL.

	A new session is created if none is provided.
	"""
	session = session or requests.Session()
	return get_url(parse_link(link), session=session)
