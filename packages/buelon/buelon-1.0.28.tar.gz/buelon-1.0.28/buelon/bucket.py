...
class Client:
    ...
    def __init__(self):
        ...
        with self.db.connect() as conn:
            conn.execute('CREATE TABLE IF NOT EXISTS bucket (key TEXT PRIMARY KEY, data BYTEA, epoch REAL);')
    ...
    def set(self, key: str, data: bytes, timeout: float | None = 60 * 5.) -> None:
        ...
        """
        with self.db.connect() as conn:
            conn.execute('INSERT INTO bucket (key, data, epoch) VALUES (%s, %s, %s) '
    ...
    def get(self, key: str, timeout: float | None = 60 * 5.) -> bytes | None:
        ...
        try:
            with self.db.connect() as conn:
                data = conn.execute('SELECT data FROM bucket WHERE key = %s', (key,)).fetchone()[0]
                return data
        except: pass
        return
        # raise RuntimeError('Not using redis.')
    ...
    def delete(self, key: str, timeout: float | None = 60 * 5.) -> None:
        ...
        """
        with self.db.connect() as conn:
            conn.execute('DELETE FROM bucket WHERE key = %s', (key, ))
        return
        # raise RuntimeError('Not using redis.')
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
