...
from textx import get_children_of_type, get_parent_of_type, get_metamodel
from virtmat.language.utilities.amml import get_params_units, get_calculator_class
from virtmat.language.utilities.amml import get_params_magnitudes
...
from virtmat.language.utilities.types import typemap, dtypemap, DType, is_array_type
from virtmat.language.utilities.types import is_numeric_type, is_numeric_scalar_type
from virtmat.language.utilities.types import is_numeric_array_type
from virtmat.language.utilities.types import is_scalar_type, scalar_type, scalar_realtype
from virtmat.language.utilities.types import is_numeric_scalar, scalar_numtype
from virtmat.language.utilities.types import get_dtype
...
def get_numeric_type(datatypes):
    ...
    if None in datatypes:
        ...
        if any(issubclass(t, complex) for t in datatypes):
            datatype = complex
        else:
            datatype = None
    elif all(issubclass(t, int) for t in datatypes):
        datatype = int
    elif all(issubclass(t, scalar_realtype) for t in datatypes):
        datatype = float
    else:
        assert any(issubclass(t, complex) for t in datatypes)
        datatype = complex
    return datatype
...
def term_type(self):
    ...
    if '/' in self.operators and None not in datatypes:
        if all(issubclass(t, scalar_realtype) for t in datatypes):
            datatype = float
        else:
            assert any(issubclass(t, complex) for t in datatypes)
            datatype = complex
...
def dict_type(self):
    ...
    return DType('Dict', (typemap['Dict'],), {'datatype': dtypes, 'datalen': 1})
    ...
    if accessor.id is not None:
        if issubclass(type_, typemap['Table']):
            if hasattr(var, 'parameter') and isinstance_r(var, ['Table']):
                ...
                series = get_reference(var.parameter).get_column(accessor.id)
                if series is None:
                    msg = f'column {accessor.id} not found in Table {var.name}'
                    raise PropertyError(msg)
                rettype = series.type_
            else:
...
def sum_type(self):
    ...
    if self.parameter is not None:
        check_series_type(self.parameter, scalar_numtype)
...
def get_par_datatype(obj, datatype):
    ...
    if datatype is not None and issubclass(datatype, scalar_type):
        ...
        if issubclass(datatype, bool):
            ...
            assert issubclass(datatype, scalar_numtype)
            param = meta['Quantity']()
...
def map_type(self):
    ...
    if not all(p.type_.datalen == dsize or p.type_.datalen is None for p in self.params):
        raise StaticValueError('Map input series must have equal size')
    if not isinstance_m(func, ['ObjectImport']):
        if len(func.args) != len(self.params):
            msg = ('Number of map function arguments and number of input '
        ...
        if par_types:
            ...
            if new_type is None or issubclass(new_type, (bool, str)):
                ...
                new_datatype = new_type.datatype
            typespec = {'datatype': new_datatype, 'datalen': dsize}
        else:
            typespec = {}
    else:
...
def reduce_type(self):
    ...
    if not isinstance_m(func, ['ObjectImport']):
        ...
        if (isinstance_m(func.expr, ['FunctionCall'])
            ...
            ret_type = subst(self, func, [param]*2, [type_]*2).type_
    else:
        check_function_import(func)
        ret_type = None
    return ret_type
...
def amml_structure_type(self):
    ...
    if self.filename or self.url:
        return rtype
    tab_typs = {'atoms': 'Table', 'cell': 'FloatArray', 'pbc': 'BoolArray'}
    ...
    for col in self.tab.columns:
        if isinstance_m(col, ['Series']):
            for elem in col.elements:
                if not hasattr(elem, '_tx_metamodel'):
                    msg = (f'element in \'{col.name}\' must have type '
                           f'{tab_typs[col.name]} but has type {type(elem)}')
                    raise_exception(col, StaticTypeError, msg)
                obj = get_reference(elem)
                if not isinstance_m(obj, [tab_typs[col.name]]):
                    msg = (f'element in \'{col.name}\' must have type '
                           f'{tab_typs[col.name]} but has type {obj.__class__.__name__}')
...
def amml_calculator_type(self):
    ...
    rtype = DType('AMMLCalculator', (typemap['AMMLCalculator'],), {})
    if self.parameters is None:
        return rtype
    literals = ['Quantity', 'Number', 'Bool', 'String', 'FloatArray',
                'IntArray', 'BoolArray', 'StrArray']
    if isinstance_m(self.parameters, ['Table']):
        params_series = []
        for col in self.parameters.columns:
            if isinstance_m(col, ['Series']):
                if all(isinstance_m(e, literals) for e in col.elements):
                    params_series.append(col.value)
        if len(params_series) > 0:
            recs = pandas.concat(params_series, axis=1).to_dict(orient='records')
            for calc_params in recs:
                units = get_params_units(self.name, calc_params)
                calc_class = get_calculator_class(self.name)
                calc_class(**get_params_magnitudes(calc_params, units))
    return rtype


def amml_algorithm_type(_):
    ...
    if (isinstance_m(self.struct.ref.parameter, ['AMMLStructure']) and
       self.struct.ref.parameter.tab is not None):
        ...
        if atoms is not None:
            symbs = [e.get_column('symbols').elements for e in atoms.elements]
