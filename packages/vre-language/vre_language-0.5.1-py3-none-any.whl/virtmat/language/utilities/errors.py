...
CONNECTION_EXCEPTION_IMPORTS = {'pymongo.errors': 'ServerSelectionTimeoutError'}
CONNECTION_EXCEPTIONS = []
for mod, exc in CONNECTION_EXCEPTION_IMPORTS.items():
    try:
        module = importlib.import_module(mod)
        class_ = getattr(module, exc)
    except ModuleNotFoundError:
        ...
        CONNECTION_EXCEPTIONS.append(class_)
...
def error_handler(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            return None
        except tuple([*CONNECTION_EXCEPTIONS, *FILE_READ_EXCEPTIONS]) as err:
