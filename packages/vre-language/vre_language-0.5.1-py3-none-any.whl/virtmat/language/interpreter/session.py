...
from virtmat.language.utilities.errors import ModelNotFoundError
...
from virtmat.language.utilities.types import get_dfr_types, get_units
...
class Session:
    ...
    def __init__(self, lpad, uuid=None, grammar_str=None, grammar_path=None,
                 ...
                 on_demand=False, async_run=False, textx_debug=False,
                 config_dir=None, detect_duplicates=False, **wfe_kwargs):
    ...
    def _get_var_values(self, var_names):
        ...
        for uuid in self.uuids:
            ...
            for var_n in var_names:
                dct[var_n] = next(v for v in var_l if v.name == var_n).value
            dfr = pandas.concat([dfr, pandas.DataFrame([dct])])
    ...
    def _update_models_old_vars(self, vary, df_old, model_str):
        ...
        for col in df_norm.columns:
            ...
            if upd_units != old_units:
                ...
                raise VaryError(msg)
        for col in df_norm.columns:
            if isinstance(df_norm[col].dtype, PintType):
