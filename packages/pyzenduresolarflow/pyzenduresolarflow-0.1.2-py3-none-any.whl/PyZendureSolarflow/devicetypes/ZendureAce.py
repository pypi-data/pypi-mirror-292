...
class ZendureAce(SolarflowDeviceBase):
    """The Solarflow Hub Device class."""
    ...
    def _update(self, data):
        ...
        if "acSwitch" in data:
            self.acSwitch = data["acSwitch"]

        if "dcSwitch" in data:
            self.dcSwitch = data["dcSwitch"]
    ...
    def setAcSwitch(self, value: bool):
        acSwitch = {"properties": {"acSwitch": 0 if not value else 1}}
        self._mqttClient.publish(
            f"iot/{self.productKey}/{self.deviceKey}/properties/write",
            json.dumps(acSwitch),
        )

    def setDcSwitch(self, value: bool):
        dcSwitch = {"properties": {"dcSwitch": 0 if not value else 1}}
        self._mqttClient.publish(
            f"iot/{self.productKey}/{self.deviceKey}/properties/write",
            json.dumps(dcSwitch),
        )

    def setInputlimit(self, inputlimit: int):
        if self.productKey and self.deviceKey and inputlimit:
            inputlimitdata = {"properties": {"outputLimit": inputlimit}}
            self._mqttClient.publish(
                f"iot/{self.productKey}/{self.deviceKey}/properties/write",
                json.dumps(inputlimitdata),
            )
