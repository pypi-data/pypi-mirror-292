...
async def async_get_json_response(response, response_result=None, load_nested_json=True):
    ...
    if not isinstance(response_json, dict):
        try:
            ...
            # If JSON decoding fails, log the error and return the raw response
            print(f"Failed to decode JSON response: {e}")
            response_json = await response.text()  # Correct usage with parentheses
...
async def postAsyncRequest(url, data=None, headers=None, endpoint=None, status_code=False):
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(**values) as response:
            if status_code:
                status_code = response.status
                json_response = await async_get_json_response(response)
                return {"result": json_response, "status_code": status_code}
            else:
                json_response = await async_get_json_response(response)
                return json_response
