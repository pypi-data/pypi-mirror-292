...
from typing import Any, Sequence, Tuple, Iterable, Literal
from datetime import datetime
...
class OpsReporter(Reporter):
    ...
    def processor(self, data: list, args: dict) -> dict:
        return {}
    ...
    def collect_and_process(
        ...
        # A slice delta of smaller than an hour leads to weird artifacts
        # in the counts. This is because of the hourly refresh of models
        # that exist but don't have updates in a given hour. They will
        # get emitted at the boundary of the hour. This will lead to
        # undercounts in the other slices, and what looks like a peak
        # at the end. It is best to avoid this confusion by having
        # a slice delta of minimum an hour.
        ...
        query_delta = 2 * slice_delta   # 2 hours
        # We keep the query delta at 2 hours to keep memory usage down
        # in case of very large clusters.
        # It would probably be better to be smarter about the
        # query window and make it bigger if it's a small cluster
        # to go faster.
        ...
        for st in range(start_time, end_time, query_delta):
        ...
        # Reporting period
        self.context["st"] = datetime.fromtimestamp(args["st"]).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        self.context["et"] = datetime.fromtimestamp(args["et"]).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    ...
    def update_model_data(
        self, data: Iterable[dict], st: float, et: float, slice_delta: int
    ):
        data_sorted = sorted(data, key=lambda x: x["time"])
        len_data = len(data_sorted)
        logger.info(
            f"Update model data from {datetime.fromtimestamp(st)} "
            f"to {datetime.fromtimestamp(et)} ({et-st} sec): "
            f"{len_data} records"
        )
    ...
    def renderer(
        ...
        if fmt == "mdx":
            ...
            # Cluster stats grid
            i_last = self.cluster_stats.index[-1][0]
            i_first = self.cluster_stats.index[0][0]
            # i_day = i_last - pd.Timedelta(days=1)
