...
from resource_graph.altimeter.aws.auth.cache import AWSCredentials, AWSCredentialsCache
...
class MultiHopAccessor(BaseImmutableModel):
    ...
    def get_session(
        ...
        """
        logger = Logger()
        cws = localstack_client.session.Session(region_name=region_name)
        for access_step in self.access_steps:
            access_account_id = (
                access_step.account_id if access_step.account_id else account_id
            )
