...
class QueryResultSet:
    ...
    def to_list(self) -> List[Dict[str, Any]]:
        ...
        for value in self.values:
            result_list.append(
                {field: value.get(field, {}).get("value") for field in self.fields}
            )
        return result_list
    ...
    def to_csv(self) -> str:
        ...
        with io.StringIO() as csv_buf:
            ...
            writer = csv.DictWriter(
                csv_buf, fieldnames=self.fields, lineterminator="\n"
            )
            writer.writeheader()
    ...
    @classmethod
    def from_dict(
        cls: Type["QueryResultSet"], data: Dict[str, Any]
    ) -> "QueryResultSet":
...
class QueryResult:
    ...
    def __init__(
        self, graph_uris_load_times: Dict[str, int], query_result_set: QueryResultSet
    ):
