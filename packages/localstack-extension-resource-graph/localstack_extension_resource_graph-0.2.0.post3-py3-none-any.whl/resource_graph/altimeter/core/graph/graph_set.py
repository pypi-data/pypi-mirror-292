...
from resource_graph.altimeter.core.graph.node_cache import NodeCache
    ...
    def from_graph_sets(cls, graph_sets: List["GraphSet"]) -> "GraphSet":
        ...
        resources = list(
            itertools.chain.from_iterable(
                [graph_set.resources for graph_set in graph_sets]
            )
        )
        errors = list(
            itertools.chain.from_iterable(
                [graph_set.errors for graph_set in graph_sets]
            )
        )
        return cls(
            name=name,
...
class ValidatedGraphSet(GraphSet):
    ...
    @validator("resources")
    def dedupe_and_validate_resources(
        cls, resources: Tuple[Resource, ...]
    ) -> Tuple[Resource, ...]:
