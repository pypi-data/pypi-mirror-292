...
from resource_graph.altimeter.aws.resource.s3 import S3ResourceSpec
            ...
            ListField(
                ...
                EmbeddedDictField(
                    ...
                    AnonymousDictField(
                        "ApplyServerSideEncryptionByDefault",
                        ResourceLinkField(
                            "KMSMasterKeyID", KMSKeyResourceSpec, optional=True
                        ),
                    ),
                ),
...
def get_s3_bucket_encryption(
    ...
    try:
        ...
        if error_code == "AccessDenied":
            raise S3BucketAccessDeniedException(
                f"Error getting encryption configuration for {bucket_name}: {response_error}",
                c_e,
            ) from c_e
        if error_code == "NoSuchBucket":
            raise S3BucketDoesNotExistException(
