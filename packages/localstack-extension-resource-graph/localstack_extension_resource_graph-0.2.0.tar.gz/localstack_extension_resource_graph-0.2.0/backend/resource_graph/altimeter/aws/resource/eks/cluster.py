...
from resource_graph.altimeter.aws.resource.eks import EKSResourceSpec
...
class EKSClusterResourceSpec(EKSResourceSpec):
    ...
    @classmethod
    def list_from_aws(
        cls: Type["EKSClusterResourceSpec"],
        client: BaseClient,
        account_id: str,
        region: str,
    ) -> ListFromAWSResult:
        ...
        try:
            ...
            if (
                error_msg
                != f"Account {account_id} is not authorized to use this service"
            ):
                raise c_e
