...
from resource_graph.altimeter.aws.scan.scan_plan import ScanPlan
from resource_graph.altimeter.aws.scan.muxer import AWSScanMuxer
from resource_graph.altimeter.aws.scan.scan_manifest import ScanManifest
...
from resource_graph.altimeter.core.graph.graph_set import GraphSet, ValidatedGraphSet
from resource_graph.altimeter.core.log import Logger


def get_sub_account_ids(
    account_ids: Tuple[str, ...], accessor: Accessor
) -> Tuple[str, ...]:
    logger = Logger()
    ...
    for account_scan_manifest in muxer.scan(scan_plan=scan_plan):
        ...
        if account_scan_manifest.artifacts:
            for account_scan_artifact in account_scan_manifest.artifacts:
                ...
                graph_sets.append(GraphSet.model_validate(artifact_graph_set_dict))
            scanned_accounts.append(account_id)
        else:
            unscanned_accounts.add(account_id)
    if not graph_sets:
        raise Exception("BUG: No graph_sets generated.")
    validated_graph_set = ValidatedGraphSet.from_graph_set(
        GraphSet.from_graph_sets(graph_sets)
    )
    master_artifact_path: Optional[str] = None
    if config.write_master_json:
        master_artifact_path = artifact_writer.write_json(
            name="master", data=validated_graph_set
        )
