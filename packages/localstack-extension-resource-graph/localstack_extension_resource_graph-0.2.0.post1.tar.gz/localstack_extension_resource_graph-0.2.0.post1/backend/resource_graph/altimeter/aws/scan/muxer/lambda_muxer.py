...
from resource_graph.altimeter.aws.scan.account_scanner import AccountScanResult
...
def invoke_lambda(
    lambda_name: str,
    lambda_timeout: int,
    account_scan_lambda_event: AccountScanLambdaEvent,
) -> AccountScanResult:
    ...
    with logger.bind(
        lambda_name=lambda_name, lambda_timeout=lambda_timeout, account_id=account_id
    ):
        ...
        try:
            resp = lambda_client.invoke(
                ...
                Payload=account_scan_lambda_event.json().encode("utf-8"),
            )
        ...
        if resp.get("FunctionError", None):
            ...
            logger.info(
                event=AWSLogEvents.RunAccountScanLambdaError, error=function_error
            )
            raise Exception(
