    ...
    try:
        ...
        if src.client_data['operacao'].lower().__contains__('b2b'):
            ...
            headers = {'Content-type': 'application/json',
                       ...
                       'Authorization-token': 'basic ' + src.client_data.get('token_api_integracao')
                       }
        else:
...
def put_prices_mplace(body: List[Preco_Mplace], db_config, job_config_dict) -> List[PriceResponse]:
    try:
        ...
        if src.client_data['operacao'].lower().__contains__('b2b'):
            headers = {'Content-type': 'application/json',
                       'Accept': 'application/json',
                       'Authorization': f'basic {src.client_data.get("token_api_integracao")}'
                       }
...
def send_log_mplace(site_order_code, log):
    logger.info('Dormindo 1.2s Zzz...')
    time.sleep(1.2)
    url = f"/api/Order/{site_order_code}/warning"
    token = src.client_data.get('token_api_integracao')
    header = {'Accept': 'application/json', 'Authorization': f'basic {token}'}
    try:
