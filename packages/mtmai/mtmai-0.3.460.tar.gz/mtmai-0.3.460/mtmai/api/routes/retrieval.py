...
async def add_content(db: SessionDep, current_user: CurrentUser, req: RagAddContentReq):
    # await doc_add(db=db, content=req.content)
    result = await embedding_hf(inputs=[req.content])
    item = Document(document=req.content, embedding=result[0], owner=current_user.id)
    db.add(item)
    db.commit()
    db.refresh(item)
    return item

    # doc = Document(document=req.content, embedding=result[0], owner=current_user.id)
    # db.add(doc)
    # db.commit()
