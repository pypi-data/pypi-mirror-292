...
py_projects = [".", "mtmdb", "mtmlib", "mtmtrain"]
...
def release_py():
    ...
    for project in py_projects:
        release_py_dir(project)
    git_commit_push()


def release_py_dir(project_dir: str):
    testing_dir = Path(f"{project_dir}/tests")
    if testing_dir.exists():
        logger.info("🚀 testing")
        bash(f"cd {project_dir} && poetry run poe test")
        logger.info("✅ testing ok!")

    # bash("poetry run poe test")
    bash(f"cd {project_dir} && poetry build && poetry publish")
    mtutils.pyproject_patch_version(project_dir)
