...
def ensureIntor():
    ...
    if mtutils.is_tcp_listen(controll_port):
        ...
        if is_tor_connected:
            ...
            print(term.format("TOR hidden dir: %s" % tor_proc.hidden_dir, term.Color.GREEN), flush=True)

            if tor_proc.onion_name:
                os.environ["MAIN_ONION"] = tor_proc.onion_name
            print(term.format("onion:  %s" % tor_proc.onion_name, term.Color.GREEN), flush=True)
            print(term.format("private key:  %s" % tor_proc.private_key, term.Color.GREEN), flush=True)
...
class TorProc():
    ...
    def start(self,
        ...
        if EXTERNAL_PROXY_HOST and EXTERNAL_PROXY_PORT:
            torcc_lines.append(f"Socks5Proxy {EXTERNAL_PROXY_HOST}:{EXTERNAL_PROXY_PORT}")
        # tor 使用外部代理
        ...
        if self.hiddenservice_ports:
            ...
            logger.info(f"hidden_dir: {self.hidden_dir}, {toruser.pw_uid}, {toruser.pw_gid}")
            os.chown(self.hidden_dir, toruser.pw_uid, toruser.pw_gid)
            torcc_lines.append("HiddenServiceDir \"{}\"".format(self.hidden_dir))

            for port in self.hiddenservice_ports:
                torcc_lines.append("HiddenServicePort \"{}\"".format(port))
