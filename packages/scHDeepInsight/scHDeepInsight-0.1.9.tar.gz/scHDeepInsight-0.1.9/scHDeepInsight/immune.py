...
class Immune:
    ...
    def predict(self, barcode_path: str, image_path: str, batch_size: int = 128):
        ...
        # Process the prediction results and generate the DataFrame
        ...
        pred_label_base = pred_label_base.rename(columns={0:"predicted_base_type"})
        ...
        pred_label_detailed = pred_label_detailed.rename(columns={0:"predicted_detailed_type"})

        # Create DataFrame with probabilities
        prob_analysis = pd.DataFrame({
            "predicted_base_type": pr_base,
            "predicted_detailed_type": pr_detailed,
            "predicted_base_type_prob": pred_base_probs,
            "predicted_detailed_type_prob": pred_detail_probs
        })

        # Determine potential rare cells
        prob_analysis['is_potential_rare'] = prob_analysis.groupby('predicted_base_type', group_keys=False).apply(self._is_potential_rare)
        barcode = pd.read_csv(barcode_path)
        pred_label = pd.concat([barcode, pred_label_base, pred_label_detailed, prob_analysis[['predicted_base_type_prob', 'predicted_detailed_type_prob', 'is_potential_rare']]], axis=1)
    ...
    def _create_pred_label(self, barcode_path, out_base, out_detailed, out_base_probs, out_detailed_probs):
        ...
        prob_analysis = pd.DataFrame({
            "predicted_base_type": pred_base,
            "predicted_detailed_type": pred_detailed,
            "predicted_base_type_prob": pred_base_probs,
            "predicted_detailed_type_prob": pred_detail_probs
        })
