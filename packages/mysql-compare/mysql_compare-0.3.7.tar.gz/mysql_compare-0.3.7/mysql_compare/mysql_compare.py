...
class MysqlTableCompare:
    ...
    def processing_result(self, batch_id: int, cts: list[ComparisonTask]):
        ...
        while cts and cts[0].batch_id == batch_id:
            _, source_rows, diff_rows = astuple(cts.pop(0))
            ...
            if self.processed_rows_number % 10000 == 0 or diff_rows:
                ...
                self.checkpoint.row = extract_keyvals(source_rows[-1], self.source_table_keys)
    ...
    def get_full_table_keys_order(self, ckpt_row: dict = None):
        _keyval = ckpt_row
        ...
        with connect(**self.source_dsn) as source_con:
            ...
            while True:
                ...
                with source_con.cursor(dictionary=True, buffered=True) as cur:
                    cur.execute(statement, params=_params)

                    for row in cur.fetchall():
                        _rows_queue.append(row)

                _rows_cnt = len(_rows_queue)

                if _rows_cnt >= 1:
                    self.logger.debug(f"full table query - fetch once query: {get_elapsed_time(_start_time_1, 2)}s, fetch rows count: {_rows_cnt}.")

                    while _rows_queue:
                        rows = []
                        for _ in range(min(self.fetch_size, len(_rows_queue))):
                            rows.append(_rows_queue.popleft())

                        last_row = rows[-1]
                        _keyval = extract_keyvals(last_row, self.source_table_keys)
                        yield rows
                else:
                    self.logger.debug(f"full table query - fetch all rows: {get_elapsed_time(_start_time_0, 2)}s.")
                    return
    ...
    def compare_rows_by_keys(self, ct: ComparisonTask):
        ...
        keyval_list = []
        for row in ct.source_rows:
            keyval_list.append(extract_keyvals(row, self.source_table_keys))

        target_rows = self.get_table_rows_by_keys(self.target_conpool, self.dst_database, self.dst_table, keyval_list)

        ct.different += list(itertools.filterfalse(lambda x: x in target_rows, ct.source_rows))

        self.logger.debug(
            f"compare rows success - batch[{ct.batch_id}] - elapsed time {get_elapsed_time(_start_time,2)}s, rows: {len(ct.source_rows)}, different: {len(ct.different)}."
        )
    ...
    def compare_full_table(self, checkpoint_row):
        ...
        fulltasks = enumerate(self.get_full_table_keys_order(checkpoint_row), start=1)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.parallel) as executor:
            ...
            def generator_futures(n):
                try:
                    return [
                        executor.submit(self.compare_rows_by_keys, ComparisonTask(batch_id, source_row, []))
                        for batch_id, source_row in itertools.islice(fulltasks, n)
                    ]
