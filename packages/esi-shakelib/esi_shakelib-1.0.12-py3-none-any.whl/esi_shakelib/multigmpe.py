...
from esi_shakelib.conversions.imt.abrahamson_bhasin_2020 import AbrahamsonBhasin2020
...
class MultiGMPE(GMPE):
    ...
    def __from_config__(cls, conf, filter_imt=None):
        ...
        if selected_gmpe in conf["gmpe_sets"].keys():
            ...
            out = MultiGMPE.__from_list__(
                [get_gmpe_from_name(modinfo[0], conf)], [1.0], imc=imc, imt=filter_imt
            )
            if out is None:
                return None
        else:
    ...
    def __from_list__(
        ...
        if len(weights) != len(gmpes):
            raise ValueError("Length of weights must match length of GMPE list.")
