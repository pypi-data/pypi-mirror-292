...
class App(WSGIProtocol):
    ...
    __slots__ = ("_id", "_logger", "_routing_table", "_start_response", "_start_time", "config")

    def __init__(self, config: Optional[CommonDictT] = None) -> None:
        """Initialize an WSGI application object.

        Args:
            config: The dictionary with settings/constants.
        """
        self.config: CommonDictT = DEFAULT_CONFIG | config if config else DEFAULT_CONFIG
