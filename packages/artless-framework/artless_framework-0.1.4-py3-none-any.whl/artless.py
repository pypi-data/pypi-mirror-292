...
# Constants
HTTP_PREFIX = "HTTP_"
UNPREFIXED_HEADERS = frozenset(["CONTENT_TYPE", "CONTENT_LENGTH"])
CTYPE_HEADER_NAME = "Content-Type"
...
class Request:
    ...
    def __init__(self, environ: EnvironT) -> None:
        ...
        self.query_string: Optional[str] = environ.get("QUERY_STRING")
        ...
        if decoder := self._get_body_decoder(self.headers["Content-Type"].partition(";")[0]):
    ...
    def _get_body_decoder(content_type: str) -> Optional[Type[BodyDecoder]]:
        ...
        available_ctype_decoders: Mapping[str, Type[BodyDecoder]] = {
...
class Response:
    ...
    __slots__ = ("_body", "_headers", "_status")
    ...
    def __init__(self, *, status: HTTPStatus = HTTPStatus.OK) -> None:
        ...
        self._headers: MutableMapping[str, str] = {CTYPE_HEADER_NAME: DEFAULT_CTYPE}
        self._body: bytes = b""

    def add_header(self, name: str, value: str) -> None:
        """Add (or replace if existing) response header."""
    ...
    def content_type(self) -> str:
        ...
        return self._headers[CTYPE_HEADER_NAME]

    @content_type.setter
    def content_type(self, value: str) -> None:
        """Set content_type response header value."""
        self._headers[CTYPE_HEADER_NAME] = value
...
class ResponseFactory:
    ...
    def redirect(
        ...
        *,
        ...
        response = Response(status=status)
        response.add_header("Location", quote(redirect_url))
        return response
...
class App(WSGIProtocol):
    ...
    def _wsgi_response(self, request: Request, response: Response) -> Sequence[bytes]:
        ...
        deltatime_ms: float = (time() - self._start_time) * 1000
    ...
    def _add_route(self, method: str, path: str, handler: Callable) -> None:
        ...
        if _method not in self._routing_table:
            self._routing_table[_method] = {}

        if compiled_re_path in self._routing_table[_method]:
            raise ValueError(f'Route for "{_method} {path}" already exists!')

        self._routing_table[_method][compiled_re_path] = handler
