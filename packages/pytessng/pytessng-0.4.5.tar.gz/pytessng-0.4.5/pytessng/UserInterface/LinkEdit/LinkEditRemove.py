...
class LinkEditRemove(BaseClass):
    ...
    def monitor_check_state(self, checked):
        if checked:
            ...
            mouse_remove = MouseRemove(netiface, self.apply_remove_links, self.utils)
...
class MouseRemove(BaseMouse):
    def __init__(self, netiface, apply_remove_links_func: Callable, utils):
        # 获取画布
        self.netiface = netiface
        self.scene = self.netiface.graphicsScene()

        # 执行函数
        self.apply_remove_links_func = apply_remove_links_func
        # 工具包
        self.utils = utils
    ...
    def handle_mouse_release_event(self, event) -> None:
        ...
        if event.button() == Qt.LeftButton:
            ...
            self.apply_remove_links_func(params)
    ...
    def handle_mouse_move_event(self, event) -> None:
        ...
        self.transparent_box_item.setPen(QColor(255, 0, 0))  # 设置边框颜色
    ...
    def show_confirm_dialog(self, link_count: int, mode: int):
        ...
        messages = {
            "title": "删除框选路段",
            "content": f"有{link_count}条路段被{text}选中，是否删除",
            "yes": "删除",
        }
