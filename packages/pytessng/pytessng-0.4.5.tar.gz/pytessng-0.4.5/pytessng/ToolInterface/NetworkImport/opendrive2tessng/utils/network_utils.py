...
class Network:
    ...
    def create_network(self, tess_lane_types, netiface=None):
        ...
        # 处理过短连接段
        from pytessng.ToolInterface.LinkEdit.connectorLengthLimiter.ConnectorLengthLimiter import ConnectorLengthLimiter
        ConnectorLengthLimiter(netiface).edit(min_connector_length=2)
            ...
            for section_id, section_info in road_info.get('sections', {}).items():
                ...
                for direction in ['left', 'right']:
                    ...
                    for index in range(len(childs)):
                        ...
                        link_name = f"{road_info['name']}_{road_id}_{section_id}_{index}_{direction}"
