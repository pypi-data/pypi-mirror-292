    ...
    def get_signal_light_data(simuiface, netiface) -> dict:
        ...
        for signal_group in signal_group_list:
            ...
            for signal_phase in signal_phase_list:
                ...
                # 所有灯头所在的车道ID
                lane_id_list: list = sorted([
                    signal_lamp.laneObject().id() if signal_lamp.laneObject().isLane() else signal_lamp.laneObject().fromLane().id()
                    for signal_lamp in signal_lamp_list
                ])

                # 信号相位数据
                signal_phase_data = dict(
                    curColor=current_color,
                    countDown=count_down,
                    lampColor=lamp_color,
                    duration=duration,
                    laneIdList=lane_id_list,
                )
