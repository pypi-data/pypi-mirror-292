...
class ViewXodrRoad(BaseUserInterface):
    name: str = "查看road"
    width: int = 100
    height: int = 600
...
class MyTable(QTableWidget):
    ...
    def _init_table(self):
        ...
        # 设置表格内容
        for index, junction_id in enumerate(list(self.road_data.keys())):
    ...
    def _move_to_center(self, row: int, col: int):
        ...
        # 获取中心点
        x, y = self.road_data[junction_id]["center_point"]
        ...
        # 高亮路段
        link_id_list = self.road_data[junction_id]["link_id_list"]
    ...
    def _highlighted_links(self, link_id_list: list):
        ...
        for link_id in link_id_list:
            link = self._netiface.findLink(link_id)
            # 边界线
    ...
    def _read_road_data(self):
        ...
        # 遍历路段
        for link in self._netiface.links():
            link_name = link.name()
            try:
                road_id = int(link_name.split("_")[-4])
            except:
                continue
        ...
        # 按照road_id排序
        self.road_data = dict(sorted(self.road_data.items(), key=lambda v: v[0]))
