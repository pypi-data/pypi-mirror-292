...
class AMYG4(ChefsHatPlayer):
    ...
    def get_reward(self, info):
        ...
        matchFinished = thisPlayer in info["Finished_Players"]

        maxScore = numpy.argmax(info["Game_Score"], axis=0)
        ...
        if matchFinished:
            if thisPlayer == 0:
                ...
                if beforeScore is not None and info["Game_Score"][0] > beforeScore:
                    if maxScore == 0:
                        return 1
                    else:
                        return 0.001
    ...
    def update_my_action(self, info):
        if self.training:
            thisPlayer = info["Author_Index"]
            done = thisPlayer in info["Finished_Players"]
