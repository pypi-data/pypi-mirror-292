...
class RewriteQuestion:
    ...
    def run(self, documents:List[Document]):
        ...
        def process_question(question):
            if self.service == "default":
                ...
                response_json = self.llm.invoke(prompt,json_schema, "RewriteQuestion")
                try:
    ...
    def to_dict(self) -> dict:
        return default_to_dict(self, service=self.service)
    ...
    def run(self, personas:List[str]):
        ...
        for persona in personas:
            if self.service == "default":
                ...
                response_json = self.llm.invoke(prompt,json_schema, "PersonaToWritingStyle") 
                try:
                    raw_answer = json.loads(response_json["reply"])
                except Exception as e:
        ...
        def process_question(question):
            ...
            for style in writing_styles:
                if self.service == "default":
                    ...
                    response_json = self.llm.invoke(prompt,json_schema, "RewriteQuestionByPersona")
                    try:
                        raw_answer = json.loads(response_json["reply"])
                    except Exception as e:
