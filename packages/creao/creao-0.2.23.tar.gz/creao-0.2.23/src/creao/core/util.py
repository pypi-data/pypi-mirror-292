...
def submit_pipeline_job(pipeline_id:str, pipe_yaml:str, run_config_template:str, dataset:Dataset, username:str):
    # generate a random file name for the dataset based on uuid
    file_uuid = str(uuid.uuid4())
    # save the dataset to a temporary file
    # Use tempfile to create a temporary file
    with tempfile.NamedTemporaryFile(suffix=f"{file_uuid}.parquet", delete=False) as tmp_file:
    ...
    presigned_url = get_presigned_url(temp_file_path)
