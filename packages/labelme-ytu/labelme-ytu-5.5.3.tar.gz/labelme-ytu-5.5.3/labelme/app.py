...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        _runYoloTrackButton = QtWidgets.QPushButton(self.tr("Track Detection"))
        _runYoloTrackButton.clicked.connect(self.runYoloTrack)

        runYolo.defaultWidget().layout().addWidget(_runYoloButton) 
        runYolo.defaultWidget().layout().addWidget(_runYoloVidButton) 
        runYolo.defaultWidget().layout().addWidget(_runYoloTrackButton)
    ...
    def runYolo(self, track=False):
        ...
        model = Yolo(self.model_path, track=track)
    ...
    def runYoloWithModel(self, model, track=False):
        if(self.imagePath is None or not self.imagePath.lower().endswith(tuple([
                ".{}".format(fmt.data().decode())
                for fmt in QtGui.QImageReader.supportedImageFormats()
            ]))):
    ...
    def runYoloVid(self, track = False):
        ...
        for i in range(self.fileListWidget.count()):
            ...
            self.runYoloWithModel(model, track=track)
            self.saveFileAuto()
            #time.sleep(2)
