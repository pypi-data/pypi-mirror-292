            ...
            try:
                ...
                if execute_flag is None:
                    ...
                    if workflow_type == "databricks_sql":
                        ...
                        # Find the root of the project, ensuring it begins with "cdh-lava-core"
                        project_root = None
                        current_dir = current_file_dir

                        while current_dir != os.path.dirname(current_dir):  # Stop when reaching the root directory
                            if os.path.basename(current_dir) == "cdh-lava-core":
                                project_root = current_dir
                                break
                            current_dir = os.path.dirname(current_dir)  # Move one level up

                        if project_root is None:
                            raise FileNotFoundError("Project root with 'cdh-lava-core' not found.")
                            
                        if project_root is None:
                            raise FileNotFoundError("Project root with 'cdh-lava-core' not found.")

                        # Extract the path after 'cdh-lava-core/'
                        match = re.search(r"cdh-lava-core/(.*)", path_to_execute)

                        if match:
                            # Extracted relative path
                            relative_path_after_core = match.group(1)
                            # Append '.py' to the path if it doesn't already end with it
                            if not relative_path_after_core.endswith('.py'):
                                relative_path_after_core += ".py"
                        else:
                            raise ValueError("No valid path extracted from the provided path.")

                        # Construct the full path relative to the project root
                        full_path = os.path.join(project_root, data_product_id, relative_path_after_core)

                        # Use the Python interpreter from the current environment
                        python_executable = sys.executable

                        try_remote = False
                        # If the extracted path is valid, run the Python file
                        if full_path:
                            try:
                                subprocess.run([python_executable, full_path], check=True)
                            except subprocess.CalledProcessError as e:
                                logger.error(f"Failed to execute {full_path}: {e}")
                                try_remote = True
                        else:
                            logger.error(f"No valid path extracted.")
                            try_remote = True
                    
                        if try_remote is True:
