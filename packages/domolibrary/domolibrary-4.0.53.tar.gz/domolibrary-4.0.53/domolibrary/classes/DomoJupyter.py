...
class DomoJupyterWorkspace:
    ...
    def to_json(self):
        ...
        return {
            ...
            "cpu": int(self.cpu),
            "inputConfiguration": [
                confg.to_json() for confg in self.input_configuration
            ],
            "outputConfiguration": [
                confg.to_json() for confg in self.output_configuration
            ],
            "accountConfiguration": [
                confg.to_json() for confg in self.account_configuration
            ],
            "fileshareConfiguration": self.collection_configuration,
        }
...
async def search_workspace_by_name(
    ...
    workspaces = await get_jupyter_workspaces(
        auth=auth,
        debug_api=debug_api,
        session=session,
        debug_num_stacks_to_drop=debug_num_stacks_to_drop,
...
async def update_config(self, keep : List[str] = None, debug_api: bool = False, session : httpx.AsyncClient = None, debug_num_stacks_to_drop = 2):
    config = self.to_json()
    
    # config = {key : value for key , value in config.items() if key in keep or key in ['id'] }
    
    return await jupyter_routes.update_jupyter_workspace_config(auth = self.auth, workspace_id = self.id, config = config, parent_class = self.__class__.__name__, session = session, debug_api = debug_api , debug_num_stacks_to_drop = debug_num_stacks_to_drop)
