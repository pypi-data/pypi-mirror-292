...
# @Last Modified by:   Vivian Li # @Last Modified time: 2024-02-13 19:48:09
import os
import sys
...
class PluginDirectorySuiteLoader(PluginSuiteLoader):
    ...
    def load(self, suite_dir_path: str, f: TextIO) -> List[PluginSuite]:
        ...
        for root, dirs, files in os.walk(suite_dir_path):
            ...
            if PLUGIN_CONFIG.FORMAT in files:
                format_markdown = os.path.join(root, PLUGIN_CONFIG.FORMAT)
            if config and requirement and document and official and format_markdown:
                break
        ...
        if not document:
            e = SUITE_REGISTER_MSG.MISSING.format(file=PLUGIN_CONFIG.DOCUMENT)
            logger.error(e)
            f.write(e)
            missing_file = True
    ...
    def load_suite_from_toml(
        ...
        try:
            ...
            return None
        if conf_model.suite_name != suite_name:
            e = f"suite directory and configuration file should use the same suite name."
            f" {conf_model.suite_name} and {suite_name} are different"
