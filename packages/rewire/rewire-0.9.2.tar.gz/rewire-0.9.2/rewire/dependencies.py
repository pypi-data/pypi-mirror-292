...
class DependencySelector(BaseModel, frozen=True):
    @abstractmethod
    def matches(self, spec: "DependencySpec") -> bool: ...


class DependencySpec(BaseModel, frozen=True):
    selectors: tuple[DependencySelector] = tuple()
...
class Dependency[T](DependencyRef):
    ...
    optional: bool = False
    spec: DependencySpec = DependencySpec()
    ...
    def Result(self) -> Type[T]:
        return Annotated[self.type, self]  # type: ignore
