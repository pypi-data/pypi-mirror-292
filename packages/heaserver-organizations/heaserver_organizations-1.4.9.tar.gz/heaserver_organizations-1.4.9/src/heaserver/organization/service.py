...
from heaserver.service.messagebroker import publisher_cleanup_context_factory
...
from heaobject.root import Permission, ShareImpl, DesktopObjectDict, desktop_object_type_for_name, PermissionContext
...
async def _update_permissions(request: web.Request):
    """
    Updates the current user's permissions.
    ...
    if old_dict is not None:
        ...
        group_ids = [group_id for group_id in chain(new_org.admin_group_ids, new_org.manager_group_ids, new_org.member_group_ids)]
...
async def _update_volume_and_credentials(request: web.Request,
    ...
    if credential is None:
        logger.warn('Replacing missing credentials for account %s and volume %s for user %s', actual_account.id, volume.id, person.id)
