...
def get_runs(directory, run_name):
    ...
    runs = ResultsApi.get_runs(directory)
...
def get_variable_from_run_specific_group(run: Run, variable_1d_name_non_unique, group_1d_name):
    ...
    var_1d = run.get_variable_1d_from_specific_output_group(variable_1d_name_non_unique, group_1d_name)
...
def get_variables_from_group(run: Run, group_1d_name, variable_1d_name, group_2d_name, variable_2d_name):
    ...
    vars_1d = run.get_group(group_1d_name).get_all_variables_1d()

    # Get all 2D variables from a group
    vars_2d = run.get_group(group_2d_name).get_all_variables_2d()
...
def get_independent_variables_from_group(run: Run, group_1d_name, group_2d_name):
    ...
    primary_ind_var_1d = run.get_group(group_1d_name).get_independent_variable(INDEPENDENT_VARIABLE_ID_PRIMARY_INDEPENDENT_VARIABLE)

    # Get the primary independent variable from a 2D group using independent variable identifier
    primary_ind_var_2d = run.get_group(group_2d_name).get_independent_variable(INDEPENDENT_VARIABLE_ID_PRIMARY_INDEPENDENT_VARIABLE)
    ...
    secondary_ind_var_2d = run.get_group(group_2d_name).get_independent_variable(INDEPENDENT_VARIABLE_ID_SECONDARY_INDEPENDENT_VARIABLE)
...
def get_independent_variables_from_variable(run: Run, variable_1d_name, variable_2d_name):
    ...
    # Get the primary independent variable from a 2D group using independent variable identifier
    primary_ind_var_2d = run.get_variable_2d(variable_2d_name).get_independent_variable(INDEPENDENT_VARIABLE_ID_PRIMARY_INDEPENDENT_VARIABLE)

    # Secondary independent variable
    # Get the secondary independent variable from a 2D group using independent variable identifier
    secondary_ind_var_2d = run.get_variable_2d(variable_2d_name).get_independent_variable(INDEPENDENT_VARIABLE_ID_SECONDARY_INDEPENDENT_VARIABLE)
...
def get_data_from_variables(run: Run, variable_1d_name, variable_2d_name):
    ...
    var_2d_all_data = var_2d.get_data_for_all_independent_variable_values()
...
def get_tower_and_blade_data(run: Run, tower_variable_name, tower_member_name, blade_variable_name, blade_station_location):
    ...
    # Get data from a 2D variable (all independent variable values)
    blade_variable_all_data = blade_variable.get_data_for_all_independent_variable_values()
...
# The following examples demonstrate basic operations, emphasising the differences between 1D and 2D variables:
...
directory = "./Runs/demo/powprod5MW"
run_name = "powprod5MW"
run = ResultsApi.get_run(directory, run_name)

get_runs(directory, run_name)
...
blade_variable_name = "blade 1 mx (principal axes)"
...
blade_group_name = "blade 1 loads: principal axes"
