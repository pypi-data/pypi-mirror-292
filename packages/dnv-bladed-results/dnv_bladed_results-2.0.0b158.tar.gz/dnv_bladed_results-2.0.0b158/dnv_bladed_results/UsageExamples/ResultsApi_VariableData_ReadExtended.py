...
# Demonstrates how to get data from:
...
# Demonstrates how to specify a data type to read.
...
def get_data_for_1d_variable(directory, run_name, variable_name):
    ...
    try:
        ...
        print("\nData for 1D variable '" + var_1d.get_name() + "' contains " + str(var_1d.get_data_point_count()) + " points")
...
def get_data_for_2d_variable(directory, run_name, variable_name):
    ...
    try:
        ...
        print("\nData for 2D variable '" + var_2d.get_name() + "' contains " + str(var_2d.get_data_point_count()) + " points")
        ...
        secondary_independent_variable = var_2d.get_independent_variable(INDEPENDENT_VARIABLE_ID_SECONDARY_INDEPENDENT_VARIABLE)
        ...
        if (secondary_independent_variable.get_axis_type() == AXIS_TYPE_ID_LABELLED_STRING_AXIS):
...
def get_data_for_2d_variable_at_specific_independent_variable_value(directory, run_name, variable_name, secondary_independent_variable_value, value_index):
    ...
    try:
        ...
        print("Data for 2D variable '" + variable.get_name() + "' contains " + str(variable.get_data_point_count()) + " points")
...
data_type_specifiers = [[DATA_TYPE_SPECIFIER_READ_AS_SERIALISED_TYPE, "serialised"], [DATA_TYPE_SPECIFIER_READ_AS_FLOAT64, "double"]]
for type_specifier in data_type_specifiers:
    
    # Specify data type precision
    ResultsApi_CacheSettings.set_data_type_for_reading(type_specifier[0])
    ...
    get_data_for_2d_variable("./Runs/demo/powprod5mw", "powprod5mw", "tower mx")

    # 2D dependent variable, secondary independent variable with numeric values
    get_data_for_2d_variable("./Runs/demo/powprod5mw", "powprod5mw", "blade 1 mx (principal axes)")
...
get_data_for_2d_variable_at_specific_independent_variable_value("./Runs/demo/powprod5mw", "powprod5mw", "tower mx", "Mbr 6 End 1", 2)

# 2D variable: Get data for a specified secondary axis value - numeric axis (2D variable)
get_data_for_2d_variable_at_specific_independent_variable_value("./Runs/demo/powprod5mw", "powprod5mw", "blade 1 mx (principal axes)", 4.8400002, 1)
