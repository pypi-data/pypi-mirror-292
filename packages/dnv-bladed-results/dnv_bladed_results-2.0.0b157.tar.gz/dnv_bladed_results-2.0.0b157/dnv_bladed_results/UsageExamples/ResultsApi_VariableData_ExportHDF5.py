...
############   Bladed Results API: Export data using HDF5 (.h5) file format   ############

# export_all_groups_in_run:
# Demonstrates how to export data for all variables in every output group in a run to an HDF5 data file.

# export_all_variables_in_group:
# Demonstrates how to export data for all variables in an output group to an HDF5 data file.

# export_selected_variables:
# Demonstrates how to export data for selected 1D and 2D variables to the same HDF5 data file.


def export_all_groups_in_run(directory, run_name, h5_file_path):
    ...
    try:
        ...
        with h5py.File(h5_file_path, 'w') as h5File:
            ...
            for group in all_groups:
                ...
                num_independent_variables = group.get_number_of_independent_variables()
                variable_names_in_group = group.get_variable_names()
                h5_variables_group = h5File.create_group(group.get_name())
                ...
                for variable_name in variable_names_in_group:
                    if num_independent_variables == 1:
                        ...
                        variable_data = run.get_variable_2d(variable_name).get_data_for_all_independent_variable_values()
        ...
        print("\nSuccessfully exported all groups in run '" + run_name + "' to " + h5_file_path)

    except RuntimeError as e:
        print(e)

def export_all_variables_in_group(directory, run_name, group_name, h5_file_path):
    ...
    try:
        ...
        print("\nSuccessfully exported all variables in output group '" + group_name + "' to " + h5_file_path)
        ...
        with h5py.File(h5_file_path, 'r') as h5_file:
            for variable_name in variables_names_in_group:
                if num_independent_variables == 1:
                    numpy_1d_arrays_equal(run.get_variable_1d(variable_name).get_data(), h5_file[group.get_name() + "/" + variable_name][:])
                else:
                    numpy_2d_arrays_equal(run.get_variable_2d(variable_name).get_data_for_all_independent_variable_values(), h5_file[group.get_name() + "/" + variable_name][:])
...
def export_selected_variables(directory, run_name, variable_name_1d, variable_name_2d, h5_file_path):
    ...
    try:        
        ...
        variable_data_2d = run.get_variable_2d(variable_name_2d).get_data_for_all_independent_variable_values()
...
def create_axes(h5_file: h5py.File, h5_variables_group: h5py.Group, group: Group, variable_names_in_group: np.ndarray[str]):
    ...
    for independent_variable in independent_variables_in_group:
        independent_variable_name = independent_variable.get_name()
        if independent_variable.get_axis_type() == AXIS_TYPE_ID_LABELLED_STRING_AXIS:
        ...
        h5_file[h5_axes_group.name + "/" + independent_variable_name].make_scale(independent_variable_name + " [" + independent_variable.get_siunit() + "]")
...
output_directory="./Runs/output/"
Path(output_directory).mkdir(parents=True, exist_ok=True)

# 1D + 2D groups
export_all_groups_in_run("./Runs/demo/powprod5MW", "powprod5MW", output_directory + "powprod5MW.h5")

# 1D group
export_all_variables_in_group("./Runs/demo/powprod5MW", "powprod5MW", "Hub loads: rotating GL coordinates", output_directory + "Hub_Loads.h5")

# 2D group - numeric secondary axis
export_all_variables_in_group("./Runs/demo/powprod5MW", "powprod5MW", "Blade 1 Loads: Principal axes", output_directory + "Blade_1_Loads.h5")

# 2D group - labelled string secondary axis
export_all_variables_in_group("./Runs/demo/powprod5MW", "powprod5MW", "Tower member loads - local coordinates", output_directory + "Tower_Member_Loads.h5")

# 1D + 2D variable data
export_selected_variables("./Runs/demo/powprod5MW", "powprod5MW", "Rotating hub Mx", "Tower Mx", output_directory + "RotatingHubMx_TowerMx.h5")
