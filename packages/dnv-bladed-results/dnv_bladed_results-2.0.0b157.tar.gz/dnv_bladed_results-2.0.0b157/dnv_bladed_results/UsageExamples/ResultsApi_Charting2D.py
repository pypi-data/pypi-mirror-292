...
# and create a line plot of the data using matplotlib.
...
def plot_bladed_results(
    ...
    for value in secondary_independent_variable_values:
        ...
        ax.plot(x_values, y_values, label=variable.get_name() + " at " + str(value) + "m")
    ...
    # Set axis labels
    plot_x_label = primary_independent_variable.get_name() + " [" + primary_independent_variable.get_siunit() + "]"
    plot_y_label = variable.get_siunit()
    ax.set_xlabel(plot_x_label)
    ax.set_ylabel(plot_y_label, color='black')
    ax.yaxis.set_major_formatter(mtick.FormatStrFormatter('%.2e'))

    # Add legend
    pyplot.title(variable.get_parent_group_name())
