...
# Demonstrates how to read time series data for every independent variable value of a 2D variable (e.g. blade 1 fx for all blade stations),
# and create a surface plot of the data using matplotlib.
...
def read_bladed_results(variable_name, run_directory, run_name):
    ...
    # Dependent variable
    variable_2d: Variable2D_Float32
    variable_2d = run.get_variable_2d(variable_name)

    # Independent variables
    primary_independent_variable = variable_2d.get_independent_variable(INDEPENDENT_VARIABLE_ID_PRIMARY_INDEPENDENT_VARIABLE)
    secondary_independent_variable = variable_2d.get_independent_variable(INDEPENDENT_VARIABLE_ID_SECONDARY_INDEPENDENT_VARIABLE)
...
def plot_bladed_results_surface(
    ...
    y_values = variable_2d.get_data_for_all_independent_variable_values()
    ...
    # Set axis labels
    ax.set_xlabel(primary_independent_variable.get_name() + " [" + primary_independent_variable.get_siunit() + "]")
    ax.set_ylabel(variable_2d.get_siunit())
    ax.yaxis.set_major_formatter(mtick.FormatStrFormatter('%.2e'))
    ax.set_zlabel(secondary_independent_variable.get_name() + " [" + secondary_independent_variable.get_siunit() + "]")
    ax.yaxis.set_major_formatter(mtick.FormatStrFormatter('%.2e'))

    # Render plot
    pyplot.title(variable_2d.get_parent_group_name() + " - all blade stations")
    pyplot.show()
...
run_directory = "./Runs/demo/powprod5MW"
run_name = "powprod5MW"
variable_name = "blade 1 fx (principal axes)"
