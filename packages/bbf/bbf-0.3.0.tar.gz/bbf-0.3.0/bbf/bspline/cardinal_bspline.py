...
class CardinalBSpline(BSpline):
    ...
    def eval(self, x):
        ...
        self._blossom(self._int_grid(x), i, j, B, self.order, self.order, len(self))
    ...
    def deriv(self, x):
        ...
        x = x.astype(self.dtype)
        i, j, B = _init_ijB(self.order * x.shape[-1], dtype=self.dtype)
        self._blossom(self._int_grid(x), i, j, B, self.order - 1, self.order, len(self))
        B[:-1] -= B[1:]
        return self._to_coo_array(x, i, j, B) / self.dx
    ...
    def deriv_m(self, x, m=2):
        ...
        x = x.astype(self.dtype)
        i, j, B = _init_ijB(self.order * x.shape[-1], dtype=self.dtype)
        self._blossom(self._int_grid(x), i, j, B, self.order - m, self.order, len(self))
