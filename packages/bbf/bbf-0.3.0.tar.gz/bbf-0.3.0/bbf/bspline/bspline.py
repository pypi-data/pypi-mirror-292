...
class BSpline:
    ...
    def _to_coo_array(self, x, i, j, B):
    ...
    def eval(self, x):
        ...
        Returns:
            scipy.sparse.coo_array: a sparse, N x p jacobian array
        ...
        self._blossom_grid(self._grid, x, i, j, B, self.order, self.order, len(self))
        return self._to_coo_array(x, i, j, B)
    ...
    def deriv(self, x):
        ...
        x = x.astype(self.dtype)
        i, j, B = _init_ijB(self.order * x.shape[-1], dtype=self.dtype)
        self._blossom_grid(self._grid, x, i, j, B, self.order - 1, self.order, len(self))
        self._deriv_grid(self._grid, i, j, B, x.shape[-1], self.order)
        return self._to_coo_array(x, i, j, B)
    ...
    def deriv_m(self, x, m=2):
        ...
        x = x.astype(self.dtype)
        i, j, B = _init_ijB(self.order * x.shape[-1], dtype=self.dtype)
        self._blossom_grid(self._grid, x, i, j, B, self.order - m, self.order, len(self))
    ...
    def gram(self):
        ...
        Returns:
           scipy.sparse.coo_array: a sparse p x p matrix [p=len(self)]
