...
class MarkdownGenerator:
    ...
    def generate(self) -> Dict[str, str]:
        ...
        for m in self._modules:
            ...
            index += f"- [{m.title} API Reference]({m.filename})\n"
            index += f"  - [{m.title} Models]({m.filename}#{title_anchor}-models)\n"
            ...
            if len(m.cli_commands) > 0:
                ...
                index += f"  - [{m.title} CLI]({m.filename}#{title_anchor}-cli)\n"
            ...
            if len(m.sdk_commands) > 0:
                ...
                index += f"  - [{m.title} SDK]({m.filename}#{title_anchor}-sdk)\n"
