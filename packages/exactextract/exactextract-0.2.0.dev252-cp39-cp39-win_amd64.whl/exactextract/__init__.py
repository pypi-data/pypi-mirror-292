...
def _delvewheel_patch_1_8_0():
    ...
    if sys.version_info[:2] >= (3, 8) and not is_conda_cpython or sys.version_info[:2] >= (3, 10):
        ...
        load_order_filepath = os.path.join(libs_dir, '.load-order-exactextract-0.2.0.dev252')
        if os.path.isfile(load_order_filepath):
            with open(os.path.join(libs_dir, '.load-order-exactextract-0.2.0.dev252')) as file:
