...
class Permissions:
    ...
    def permissionIsActivated(self, *, user:str='', resource:str='', permission:str='') -> Response:
        ...
        if (resource not in self.__users[user]) or (self.anyResource==resource):
            if self.anyResource!=resource and permission not in self.__users[user].get(self.anyResource, {}):
        ...
        if not __permissionInAnyResource:
            ...
            return Ok() if self.__users[user][resource].get(permission,False) else Error(f'permission `{permission}` not activated for the user')

        return Ok() if self.__users[user][self.anyResource].get(permission,False) else Error(f'permission `{permission}` not activated for the user')
