...
class Permissions:
    ...
    def permissionIsActivated(self, *, user:str='', resource:str='', permission:str='') -> Response:
        '''
        attempt to validate a permission

        a permission is validated according to the following steps
        
        * start(resource:`r`, permission:`p`):
            user has resource `r`
                goto checkMain
            else
                resource has permission `p` set
                    return permission `p` value
                else
                    resource `r` has permission '*'
        ...
        * sureFalse:
        ...
        # }[
        #     userResources.get(resource,{}).get(permission,False) 
        #     or 
        #     (resource != self.mainResource and userResources.get(self.anyResource,{}).get(permission,False))
        # ]

        value = None
        if resource in userResources:
            targetResource = userResources[resource]
            if permission in targetResource:
                value = targetResource[permission]
            else:
                if self.anyPermission in targetResource:
                    if (permission in self.__resources[resource]) or (permission in self.__globalPermissions):
                        value = targetResource[self.anyPermission]
                    else:
                        value = False

        if None==value:
            if resource == self.mainResource:
                value = False
            else:
                if self.anyResource in userResources:
                    if permission in userResources[self.anyResource]:
                        value = userResources[self.anyResource][permission]

        if None==value: value = False
