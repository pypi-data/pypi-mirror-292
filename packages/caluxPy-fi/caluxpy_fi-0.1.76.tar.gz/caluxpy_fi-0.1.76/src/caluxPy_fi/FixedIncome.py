...
class FixedIncome:
    ...
    def __init__(self, settlement_date, issuance_date, maturity_date, coupon, ytm, face_value, issuer, methodology, periodicity, payment_type, coupon_type, 
        ...
        try:
            if self.maturity_date and self.settlement_date and self.maturity_date >= self.settlement_date:
                try:
                    self.periodicity,self._oPeriodicity = Support.periodicityConversion(per = periodicity, mode = 'normal')
                except Exception as e:
                ...
                try:
                    if self.methodology not in ['letras', 'bills']:
                        ...
                        valuation_results = self.letras(settlement_date=self.settlement_date,
                                                        maturity_date=self.maturity_date,
                                                        ytm=self.ytm,
    ...
    def presentValue(self, current_coupon, nper, w_coupon, discrepancy, coupon_type, maturity_date, expected_maturity, coupon_days, ytm, periodicity, 
        ...
        while i <= nper:
            ...
            if self._oPeriodicity == 'vencimiento':
                periodicity = 360 if self.methodology == 'actual/360' else 0
                factor = self.residual_tenure
