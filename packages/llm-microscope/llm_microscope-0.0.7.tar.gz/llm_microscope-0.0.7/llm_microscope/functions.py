...
def calculate_anisotropy_torch(emb):
    ...
    Parameters:
    emb: torch tensor of shape (n_samples, n_features) representing the 
embeddings.
...
def intrinsic_dimension(emb, device, debug=False, reduction_factor=5):
    """
    ...
    with torch.no_grad():
        ...
        f_emp = torch.arange(1+useless_items, n + 1 + useless_items, 
device=device) / (n + useless_items)
        ...
        if debug:
            ...
            plt.scatter(mu_log.cpu().detach().numpy(), 
dist_log.cpu().detach().numpy(), marker='.')
            plt.show()
...
def load_enwiki_text():
    dataset_link = "https://github.com/AIRI-Institute/LLM-Microscope/raw/main/enwik88.txt"
    print("The dataset size is about 75 MB, downloading from Github may take some time...")
    print("You can also download it manually from ", dataset_link)
    text = urllib.request.urlopen(dataset_link).read()
    return text
