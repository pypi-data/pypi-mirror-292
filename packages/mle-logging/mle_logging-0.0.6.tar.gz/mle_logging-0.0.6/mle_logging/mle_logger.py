...
class MLELogger(object):
    ...
    def update(
        ...
        if self.verbose and self.print_every_k_updates is not None:
            if (
                self.stats_log.stats_update_counter % self.print_every_k_updates == 0
            ) or self.stats_log.stats_update_counter == 1:
    ...
    def ready_to_log(self, update_counter: int) -> bool:
        ...
        return (update_counter + 1) % self.log_every_j_steps == 0 or update_counter == 0
