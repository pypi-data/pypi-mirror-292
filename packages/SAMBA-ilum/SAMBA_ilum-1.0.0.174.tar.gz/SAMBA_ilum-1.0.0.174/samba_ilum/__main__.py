...
if (tarefa == 1):
   ...
   if (loop_ht == 1):
      ...
      n_Lattice = 2 
      #---------------------------------------------------------------------
      files = [name for name in os.listdir(dir_poscar) if os.path.isfile(os.path.join(dir_poscar, name))]  # Listando os arquivos dentro do diretório "ir_poscar"
      #---------------------------------------------------------------------
      bilayer_materials = list(itertools.combinations(files, 2))
      for material in files:  bilayer_materials.append((material, material))
      #---------------------------------------------------------------------
      for n_loop in range(len(bilayer_materials)):
          Lattice1 = bilayer_materials[n_loop][0]
          Lattice2 = bilayer_materials[n_loop][1]
          #-----------------------------------------------------------
          if (nloop >= 0   and nloop < 10):  nloop = '00' + str(nloop)
          if (nloop >= 10  and nloop < 100): nloop = '0' + str(nloop)
          if (nloop >= 100):                 nloop = str(nloop)
          dir_loop = str(nloop) + Lattice1.replace('.vasp', '') + '__.__' + Lattice2.replace('.vasp', '')
          #----------------------------------------------------------------------------------------------
