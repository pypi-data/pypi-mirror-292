...
if (tarefa == 1):
   ...
   if (loop_ht == 1):
      ...
      for nloop in range(len(bilayer_materials)):
          ...
          #-----------------------------------------------------------
          nloop = nloop +1
          if (nloop >= 0   and nloop < 10):  nloop2 = '00' + str(nloop)
          if (nloop >= 10  and nloop < 100): nloop2 = '0' + str(nloop)
          if (nloop >= 100):                 nloop2 = str(nloop)
          dir_loop = str(nloop2) + '--' + Lattice1.replace('.vasp', '') + '--' + Lattice2.replace('.vasp', '')
          #---------------------------------------------------------------------------------------------------
          try:
              # Executa o código interno
              resultado = subprocess.run(['python3', dir_codes + '/HeteroStructure_Generator.py'], check=True)
