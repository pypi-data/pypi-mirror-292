...
class ArbitraryPath(PathType):
    ...
    The ArbitraryPath class implements the PathType interface and serves
    as a wrapper for a path defined by user-provided Cartesian waypoints.
    This class allows users to specify their own sequence of waypoints
    directly, without additional path calculations.
    """
    ...
    def calculate_path(self, velocity: float, delta_time: float) -> list[Frame]:
        ...
        Parameter ``velocity``:
            The velocity used for path calculation (not utilized in this
            case).
        
        Parameter ``delta_time``:
            The time step used for path calculation (not utilized in this
            case).
...
class BlendedPath(PathType):
    """
    A path type for linear motion between waypoints with a circular blend
    to ensure motion continuity, optionally maintaining tool-to-surface
    orientation.
    
    The BlendedPath class implements the PathType interface to represent a
    path that smoothly transitions between Cartesian waypoints with
    circular blends. This ensures continuity of motion. This path type
    optionally supports maintaining tool-to-surface orientation throughout
    the path.
    """
    @typing.overload
    ...
    def calculate_path(self, velocity: float, delta_time: float) -> list[Frame]:
        """
        Calculate the path waypoints with piecewise linear motion and circular
        blends.
...
class CircularPath(PathType):
    """
    A circular path type with a specified start pose, circle center,
    normal, and rotation angle, optionally maintaining tool-to-surface
    orientation.
    
    The CircularPath class implements the PathType interface to represent
    a circular path with a given start pose, circle center, normal to the
    plane of the circle, and rotation angle. This path type optionally
    supports maintaining tool-to-surface orientation throughout the path.
    """
    @typing.overload
...
class FileReference:
    ...
    The FileReference struct holds information about a file's path, an
    optional scale factor, and whether the file is part of a Jacobi
    project. It is typically used to reference external files like 3D
    models.
    """
    @property
...
class LinearPath(PathType):
    ...
    The LinearPath class implements the PathType interface to represent a
    linear path from a start pose to a goal pose. It calculates waypoints
    along a straight line between the two poses and supports serialization
...
class Obstacle(Element):
    ...
    def collision(self) -> Box | Capsule | Convex | list[Convex] | Cylinder | DepthMap | Sphere:
        ...
        ConvexVector, Cylinder, DepthMap, or Sphere.
        """
    @collision.setter
...
class PathFollowingMotion:
    ...
    constant velocity. There are four different path types that are
    supported: linear, circular, blended and arbitrary. The path-following
    motion is suitable for use cases such as welding, painting, dispensing
    and deburring, where constant end-effector velocity is required for
    ...
    def calculate_path(self, delta_time: float) -> list[Frame]:
        """
        Calculate the path waypoints based on the current feasible velocity
        and delta time.
...
class Planner:
    ...
    perturbation_change_steps: int
    perturbation_scale_change: float
    ...
    def __init__(self, robot: Robot) -> None:
        ...
        Parameter ``robot``:
            The robot to plan the motions for.
        """
    def add_motion(self, motion: Motion | LinearMotion | LowLevelMotion | PathFollowingMotion) -> None:
        """
...
class Studio:
    ...
    class Events:
        ...
        def add_robot(robot: Robot) -> Studio.Action:
            ...
            """
        @staticmethod
        def add_robot_path(points: list[list[float]], robot: Robot = None, name: str = '', color: str = '', stroke: float = -1.0) -> Studio.Action:
            """
    ...
    def add_robot(self, robot: Robot) -> None:
        ...
        Parameter ``robot``:
            The robot to be added.
        """
    def add_robot_path(self, points: list[list[float]], robot: Robot = None, name: str = '', color: str = '', stroke: float = -1.0) -> None:
        """
    ...
    def remove_obstacle(self, obstacle: Obstacle) -> None:
        ...
        Parameter ``obstacle``:
            The obstacle to be removed.
        """
    def remove_robot_path(self, robot: Robot, name: str) -> None:
        """
