...
class CogContainer(kfp.dsl._container_op.Container):
    ...
    def __call__(self, *args, **kwargs):
        """
        Calls the parent class's __call__ method to initialize the container operation,
        and then applies additional model access configurations.

        This method first invokes the base class's __call__ method to create and configure
        the container operation. Afterward, it applies model access environment variables
        by calling `add_model_access()` on the current instance.

        Args:
            *args: Positional arguments to be passed to the base class's __call__ method.
            **kwargs: Keyword arguments to be passed to the base class's __call__ method.

        Returns:
            CogContainer: The instance of CogContainer with model access environment variables
            added. This is the modified container operation that includes additional configurations.
        """
        base_result = super().__call__(*args, **kwargs)

        return base_result.add_model_access()
...
class KubeflowPlugin:
    ...
    def create_component_from_func(
        ...
        training_var.add_model_access = CogContainer().add_model_access
        training_var.add_model_access()
    ...
    def list_pipeline_versions(pipeline_id):
        """
         method to list the pipeline based on pipeline id
        :param pipeline_id: pipeline id
        :return:
        """
