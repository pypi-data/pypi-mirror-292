...
def louvain_mod(G, edge_weights=None, identifier=None, directory_path=None, geometric = False, geo_info = None):
    ...
    for i, component in enumerate(connected_components):
        ...
        if directory_path:
            ...
            with open(f'{directory_path}/{identifier} network stats.txt', 'a') as f:
                f.write(f'\nModularity for component with {num_nodes} nodes: {modularity}')
            f.close()

    louvain_graph(edge_weights, identifier, directory_path, geometric, geo_info)
...
def louvain_graph(edge_weights, identifier, directory_path, geometric, geo_info):
    ...
    if directory_path:
        ...
        with open(f'{directory_path}/{identifier} network stats.txt', 'a') as f:
            f.write(f'\nLouvain predicts {len(communities)} communities')
        f.close()
    ...
    plt.axis('off')
    try:
        save_figure(directory_path, identifier)
