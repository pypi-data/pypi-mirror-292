...
def break_and_label_skeleton(skeleton, directory = None):
    ...
    for x, y, z in nonzero_coords:
        ...
        nearby_sum = np.sum(skeleton[x-1:x+2, y-1:y+2, z-1:z+2])
        
        # If sum is greater than 3, remove this branch and neighbors
...
def label_branches(array, down_factor = None, directory = None):
    """
    ...
    other_array = break_and_label_skeleton(other_array)
...
def label_vertices(array, directory = None):
    """
    ...
    for x, y, z in nonzero_coords:
        ...
        # Count nearby pixels including diagonals
        nearby_sum = np.sum(array[x-1:x+2, y-1:y+2, z-1:z+2])
        
        # If sum is greater than 3, isolate this region
        if nearby_sum > threshold:
            image_copy[x-1:x+2, y-1:y+2, z-1:z+2] = threshold * 3
...
class Network_3D:
    ...
    def show_network(self, geometric = False):
        """
        ...
        if not geometric:
            ...
            simple_network.show_simple_network(self._network_lists)

        else:
            simple_network.show_simple_network(self._network_lists, geometric = True, geo_info = [self._node_centroids, self._nodes.shape])

    def show_communities(self, geometric = False):
        """
        ...
        if not geometric:
            ...
            simple_network.show_community_network(self._network_lists)

        else:
            simple_network.show_community_network(self._network_lists, geometric = True, geo_info = [self._node_centroids, self._nodes.shape])


    def show_communities_louvain(self, geometric = False):
        """
        ...
        if not geometric:
            ...
            modularity.louvain_mod(self._network_lists)
        else:
            modularity.louvain_mod(self._network_lists, geometric = True, geo_info = [self._node_centroids, self._nodes.shape])
    ...
    def show_identity_network(self, geometric = False):
        """
        ...
        """
        if not geometric:
            simple_network.show_identity_network(self._network_lists, self._node_identities, geometric = False)
        else:
            simple_network.show_identity_network(self._network_lists, self._node_identities, geometric = True, geo_info = [self._node_centroids, self._nodes.shape])
