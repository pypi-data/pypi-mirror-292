...
class ScreenRecorder:
    ...
    def __init__(
        ...
        if (self._video_width is None) != (self._video_height is None):
            raise ValueError(
                "ScreenRecorder requires either both video_width and "
                "video_height to be provided or neither to be provided.")
    ...
    def _start_adding_frames_to_video(self) -> None:
        ...
        while self._recording_thread.is_alive() or self._frame_queue.empty() is False:
            ...
            if self._video_width and self._video_height:
                img = Image.open(BytesIO(base64_screenshot))
