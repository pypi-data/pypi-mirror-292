...
class RoundedEMT(EMT):
    """
    EMT calculator that rounds the energy and forces to 8 decimals.
    This is to have reproducible tests on different hardware.
    """

    def calculate(self, atoms=None, *args, **kwargs):
        super().calculate(atoms, *args, **kwargs)

        self.results['energy'] = np.round(self.results['energy'], 8)
        self.results['forces'] = np.round(self.results['forces'], 8)
...
template = fcc100('Au', size=(4, 4, 2), vacuum=10.0)

z_corner = template.positions[:, 2].max() - 1.0

confinement_cell = template.cell.copy()
confinement_cell[2, 2] = 7

confinement_corner = np.array([0, 0, z_corner])
...
random_generator = SymmetryGenerator(**environment.get_confinement(), 
                                     sym_type='slab')
rattle_generator = SymmetryRattleGenerator(**environment.get_confinement(),
                                           let_lose_sym=0.5)
