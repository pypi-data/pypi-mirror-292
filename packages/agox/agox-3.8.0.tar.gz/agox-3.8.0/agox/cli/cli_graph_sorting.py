...
def add_graph_sorting_parser(subparsers):

    parser = subparsers.add_parser('graph-sorting', help='Sorts structures according to their Voronoi graphs.')
    parser.set_defaults(func=cli_graph_sorting)
...
def cli_graph_sorting(args):
    ...
    descriptor = Voronoi(indices = indices, template = template, covalent_bond_scale_factor = covalent_bond_scale_factor, n_points = n_points, angle_from_central_atom=angle, environment=None)
