...
class AR(TimeSeries):
    ...
    def simulate(self, size: int, seed: Optional[int] = None) -> np.ndarray:
        ...
        inverted_params = CArray.of(type_str="double", arr=self._parameters[::-1])
        last_signs = CArray.of(type_str="int", arr=np.array(self._signs[-self._order:]).astype(int))
        self._simulation = CArrayEmpty.of(type_str="int", size=size)
