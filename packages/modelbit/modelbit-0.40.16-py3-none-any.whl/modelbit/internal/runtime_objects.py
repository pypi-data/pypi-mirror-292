...
def _localRtObjectPath(workspaceId: str, contentHash: str) -> str:
  return objectCacheFilePath(workspaceId=workspaceId, contentHash=contentHash)
...
def uploadRuntimeObject(api: MbApi,
  ...
  with storeFileOnSuccess(
      finalPath=_localRtObjectPath(workspaceId=resp.workspaceId, contentHash=contentHash)) as tmpPath:
...
@retry(8, logger)
def downloadRuntimeObject(api: MbApi, contentHash: str, desc: str) -> memoryview:
...
@retry(8, logger)
def downloadRuntimeObjectToFile(api: MbApi, contentHash: str, desc: str, toFile: str) -> None:
  resp = ObjectApi(api).runtimeObjectDownloadUrl(contentHash)
  if not resp or not resp.objectExists:
    raise Exception("Failed to get file URL")
  getSecureDataToFile(dri=resp, desc=desc, toFile=toFile)
