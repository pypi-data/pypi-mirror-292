...
def assertValidRegion(region: Optional[str]):
  if region is None or region in ["app", "us-east-1", "ap-south-1", "localhost", "web"]:
    return
  raise UserFacingError(f"The region '{region}' is invalid.")


def _maybeAuthenticateJob(api: MbApi):
  if inRuntimeJob():
