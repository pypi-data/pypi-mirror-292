...
from modelbit.internal.file_stubs import contentHashFromYaml, sizeFromYaml
...
class GitFilter:
  ...
  def smudge(self, filepath: str, content: bytes) -> Union[bytes, memoryview]:
    contentHash = self.getContentHash(filepath, content)
    if not contentHash:
      return content
    logger.info(f"Smudging {filepath} hash={contentHash}")
    return downloadRuntimeObject(api=self.api, contentHash=contentHash, desc=filepath)
  ...
  def smudgeToFile(self, filepath: str, content: bytes, toFile: str) -> None:
    ...
    return downloadRuntimeObjectToFile(api=self.api, contentHash=contentHash, desc=filepath, toFile=toFile)

  def getContentHash(self, filepath: str, content: bytes) -> Optional[str]:
    try:
