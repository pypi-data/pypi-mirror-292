...
from typing import List, NoReturn, Tuple, Iterator, Dict, Set, Union
import tempfile
from contextlib import contextmanager
from .filter import GitFilter
from modelbit.utils import tempFilePath, getStatusOutputStream, progressBar
...
class GitProtocol(GitProtocolBase):
  ...
  @contextmanager
  def writeSuccessFromFile(self, descPath: str):
    self.write(b"status=success\n")
    self.write(FLUSH_PACKET)
    ...
    with tempFilePath() as tfName:
      ...
      with progressBar(inputSize, desc=(SHELL_FORMAT_FUNCS["cyan"]("Preparing") + f" '{descPath}'")) as t:
    ...
    self.write(FLUSH_PACKET)
    self.write(FLUSH_PACKET)
    print(SHELL_FORMAT_FUNCS["purple"]("Storing") + f" '{descPath}' in git...",
