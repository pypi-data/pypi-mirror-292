...
def doGitClone(workspaceId: str, gitUrl: str, targetDir: str) -> None:
  cloneConfig = [
...
def maybePrepFirstDeployment(targetDir: str):
...
def clone(targetDir: str = "modelbit", origin: Optional[str] = None) -> None:
  ...
  if gitUrl == cloneInfo.mbRepoUrl:
    if canCheckForSshKeys():
      ...
      raise UserFacingError(NeedSshKeyErrorMsg)
  doGitClone(cloneInfo.workspaceId, gitUrl, targetDir)
  print("")
