...
class POAM(IntegrationIssue):
    ...
    def gen_issue_from_row(self, row: tuple, status: str, category: str, index: int, sheet: str) -> Optional[Issue]:
        ...
        """
        false_positive = self.get_row_val(row, "V") if self.get_row_val(row, "V") else "No"
        # Validate false positive as ['Yes', 'No', 'Pending Review' ]
        if false_positive not in ["Yes", "No", "Pending Review"]:
            self.logger.warning(
                "Invalid False Positive value: %s on row %i, sheet %s. setting to No..", false_positive, index, sheet
            )
            false_positive = "No"
