...
class ScannerIntegration(ABC):
    ...
    def handle_vulnerability(
        ...
        if finding.plugin_name or finding.cve:
    ...
    def should_close_issue(self, issue: regscale_models.Issue, current_vuln_ids: Set[int]) -> bool:
        ...
        :rtype: bool
        """
        # Check if we're using an older version of RegScale (before 5.64.0)
        if re.match(r"^\d+\.\d+(\.\d+)?$", self.regscale_version) and self.regscale_version < "5.64.0":
            # For older versions, simply check if the vulnerability ID is not in the current set
            return issue.vulnerabilityId not in current_vuln_ids

        # Do not close other tools issues
        if issue.sourceReport != self.title:
            return False

        # For newer versions, we need to check each vulnerability mapping
        any_open_mappings = False
        for vuln_mapping in regscale_models.VulnerabilityMapping.find_by_issue(
            ...
            # Get the scan history for this vulnerability mapping
            scan_history = regscale_models.ScanHistory.get_object(object_id=vuln_mapping.scanId)

            # If the vulnerability is not in the current set and the scanning tool matches
            if issue.vulnerabilityId not in current_vuln_ids and scan_history.scanningTool == self.title:
                # Close the vulnerability mapping
                vuln_mapping.status = regscale_models.VulnerabilityStatus.Closed
                vuln_mapping.save()
            else:
                # If any mapping should remain open, we note it but continue processing
                any_open_mappings = True

        # After processing all mappings, we decide whether to close the issue
        return not any_open_mappings
    ...
    def get_date_completed(self, finding: IntegrationFinding, issue_status: regscale_models.IssueStatus) -> str:
        """
