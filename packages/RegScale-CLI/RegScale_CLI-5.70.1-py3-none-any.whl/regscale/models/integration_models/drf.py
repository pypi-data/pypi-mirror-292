...
class DRF(IntegrationIssue):
    ...
    def __init__(self, **kwargs):
        ...
        create_properties = [
            ...
            if prop.value
            not in {val.get("dr_number") for val in self.id_other_identifier_map.values() if val.get("dr_number")}
        ]
        ...
        for dev in self.drf_data:
            if dev.extra_data["dr_number"] in {ex.extra_data["dr_number"] for ex in self.existing_deviations}:
                dev.id = [
                    ...
                    if item.extra_data["dr_number"] == dev.extra_data["dr_number"]
                ].pop()
                dev.save()
            else:
                dev.create()
    ...
    def gen_drf_from_row(self, columns: list[str], row: tuple, index: int, sheet: str) -> Optional[Deviation]:
        ...
        dr_number = get_val("DR Number")
        poam_id = get_val("POA&M ID")
        ...
        requested_risk_rating = get_val("Requested Risk Rating/Impact")
        self.report.append({"dr_number": dr_number, "poam_id": poam_id, "status": "Matched"})
        if requested_risk_rating.lower() not in ["low", "moderate", "high"]:
        ...
        return Deviation(
            ...
            operationalImpacts=get_val("Operational Impact Statement"),
    ...
    def get_id_map(self) -> dict:
        ...
        for issue in track(all_issues, description="Building id-otherIdentifier lookup..."):
            if issue.otherIdentifier:
                id_map[issue.otherIdentifier] = {"id": issue.id, "dr_number": None, "prop_id": None}
                ...
                if dr_number_property:
                    id_map[issue.otherIdentifier]["dr_number"] = dr_number_property.value
                    id_map[issue.otherIdentifier]["prop_id"] = dr_number_property.id
        self.logger.info(
