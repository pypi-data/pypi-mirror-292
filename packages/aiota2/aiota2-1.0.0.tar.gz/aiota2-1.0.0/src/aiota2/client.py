...
async def main():
    dota = await AioClient().load_heroes()
    nice_mid_hero = dota.get_hero_by_param("localized_name", "Arc Warden")
    print(nice_mid_hero.id)

    # -

    dota = AioClient(with_models=False)
    someone_id = 1
    player = await dota.get_player(someone_id)
    print(player.get(""))
