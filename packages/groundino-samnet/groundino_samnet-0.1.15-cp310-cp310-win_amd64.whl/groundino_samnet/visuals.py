...
def plot_grid(images,idss,boxes,logits,phrases,**args):
  imag_max = args.get("image_max",20)
  if len(images) > imag_max:
    print(f"Warning: The amount displayed will be truncated to 20. You can change this value using the image_max argument, but there is a risk of not displaying the images correctly.")
    images = images[:20]
    idss = idss[:20]
    boxes = boxes[:20]
    logits = logits[:20]
