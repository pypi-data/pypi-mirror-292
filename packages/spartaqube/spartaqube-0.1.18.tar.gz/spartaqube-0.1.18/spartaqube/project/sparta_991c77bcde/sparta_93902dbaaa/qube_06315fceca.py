...
from project.sparta_991c77bcde.sparta_f7de89f57a import qube_9f47688d3b as qube_9f47688d3b
def sparta_a1a4280826(json_data,userObj):
...
def sparta_2fb0ccccf6(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_f83aea7c4c.sparta_0306567289(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_f83aea7c4c.sparta_0306567289(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_86636ab358(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_f83aea7c4c.sparta_0306567289(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_f83aea7c4c.sparta_0306567289(A);C=[A.userGroup for A in H]
...
def sparta_25a3e8539d(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';J=json_data;B=user_obj;print('json_data SHARE OBJECTS');print(J);L=datetime.now().astimezone(UTC);P=qube_f83aea7c4c.sparta_0306567289(B);M=[A.userGroup for A in P];U=J['member2ShareArr'];I=int(J[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_f83aea7c4c.sparta_0306567289(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_f83aea7c4c.sparta_0306567289(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_96f21d9b7f(json_data,user_obj):
