...
def sparta_2d17186d94(json_data,user_obj,group_id=None):
	...
	F=UserGroup.objects.create(name=E,group_id=C,user_creator=B,date_created=A,last_update=A);UserGroupUser.objects.create(user_group=F,user=B,b_admin=_B,date_created=A,last_update=A);sparta_2764496f8e(G,F,A,B);H={_A:1,_C:C};return H
def sparta_2764496f8e(membersArr,user_group_obj,dateCreation,userMe):
...
def sparta_11e56ae4fc(user_obj):A=user_obj;B=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_B,user_group__is_delete=0).all();C=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_D,user_group__is_delete=0).all();D={_E:B,_F:C};return D
def sparta_0306567289(user_obj):return UserGroupUser.objects.filter(is_delete=0,user=user_obj,user_group__is_delete=0)
def sparta_ef2bda28c1(json_data,user_obj):A=sparta_11e56ae4fc(user_obj);B=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_E]];C=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_F]];return{_A:1,_E:B,_F:C}
def sparta_dbc3781b3f(json_data,user_obj):
	...
	B={_A:-1};return B
def sparta_45ca57cbdd(json_data,user_obj):
	...
	if D.count()>0:
		...
		if H.count()>0:A.name=F;A.save();I=datetime.now();sparta_2764496f8e(G,A,I,C)
	J={_A:1};return J
def sparta_2d69e97136(json_data,user_obj):
	...
	I={_A:1};return I
def sparta_09235ca100(json_data,user_obj):
	...
	D={_A:1};return D
def sparta_385b061c71(json_data,user_obj):
	...
	H={_A:1};return H
def sparta_4e976903f4(json_data,user_obj):
