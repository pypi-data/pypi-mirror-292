...
from project.sparta_991c77bcde.sparta_a92ffe2823 import qube_aa2abec219 as qube_aa2abec219
from project.sparta_991c77bcde.sparta_d3e3d4f476.qube_61c2c134f3 import Email as Email
def sparta_c6659794ff(function):
	def A(request,*E,**C):
		A=request;B=_B
		...
		if not B:
			D=C.get(_I)
			if D is not None:F=qube_aa2abec219.sparta_62dd486c7e(D);login(A,F)
		return function(A,*E,**C)
	return A
def sparta_8e7f45378d(function):
	def A(request,*B,**C):
		A=request
	...
	return A
def sparta_4357ecbf0b(function):
	def A(request,*B,**C):
	...
	return A
def sparta_1d58a06b14(function):
	def A(request,*D,**E):
		A=request;B=_C
		try:
			F=json.loads(A.body);G=json.loads(F[_J]);H=G[_I];C=qube_aa2abec219.sparta_62dd486c7e(H)
			if C is not None:B=_B;A.user=C
		except Exception as I:print('exception pip auth');print(I)
		if B:return function(A,*D,**E)
		else:return HttpResponseRedirect(reverse(_G))
	return A
def sparta_f87defa085(code):
	try:
...
def sparta_3b5d186a89():A=socket.gethostname();B=socket.gethostbyname(A);return B
def sparta_851727a2ca(json_data):
	D='ip_addr';A=json_data;del A[_K];del A[_L]
	try:A[D]=sparta_3b5d186a89()
	except:A[D]=-1
...
def sparta_460701ffee():return conf_settings.IS_GUEST_CODE_REQUIRED
def sparta_86596412a9(guest_code):
	if GuestCodeGlobal.objects.filter(guest_id=guest_code,is_active=_B).count()>0:return _B
	return _C
def sparta_130212c468(guest_code,user_obj):
	...
	return _C
def sparta_fe351475dd(user):
	A=UserProfile.objects.filter(user=user)
	if A.count()==1:return A[0].is_banned
	else:return _C
def sparta_7c6e5af1d4(email,captcha):
	D=sparta_04b342a08a(captcha)
	if D[_A]!=1:return{_A:-1,_D:_H}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_M}
	A=B[0];C=str(uuid.uuid4());A.token_reset_password=C;A.save();sparta_7b9d50346f(A.user,C);return{_A:1}
def sparta_7b9d50346f(user_obj,token_reset_password):B=user_obj;A=Email(B.username,[B.email],'Reset Password','Reset Password Message');A.addOneRow('Reset code','Copy the following code to reset your password');A.addSpaceSeparator();A.addOneRow(token_reset_password);A.send()
def sparta_1dd886adfe(captcha,token,email,password):
	D=sparta_04b342a08a(captcha)
	if D[_A]!=1:return{_A:-1,_D:_H}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_M}
