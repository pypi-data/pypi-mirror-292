...
def sparta_23524f9f12(is_owner=_A):return{_F:is_owner,_B:_E,_C:_E,_D:_E}
def sparta_f335031f46():return{_F:_A,_B:_A,_C:_A,_D:_A}
def sparta_9df69bf830(user_obj,portfolio_obj):
	B=portfolio_obj;A=user_obj
	if B.user==A:return sparta_23524f9f12(_E)
	F=qube_f83aea7c4c.sparta_0306567289(A);E=[A.userGroup for A in F]
	...
	if D.count()==0:return sparta_f335031f46()
	G=D[0];C=G.ShareRights
	if C.is_delete:return sparta_f335031f46()
	return{_F:_A,_B:C.is_admin,_C:C.has_write_rights,_D:C.has_reshare_rights}
def sparta_2e04a83d28(user_obj,universe_obj):
	B=universe_obj;A=user_obj
	if B.user==A:return sparta_23524f9f12()
	F=qube_f83aea7c4c.sparta_0306567289(A);E=[A.userGroup for A in F]
