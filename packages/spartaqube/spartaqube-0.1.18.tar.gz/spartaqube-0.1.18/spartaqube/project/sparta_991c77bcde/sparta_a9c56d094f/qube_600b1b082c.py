...
from project.sparta_e47c1b14a9.sparta_cc215461ed.qube_b4ac0415ab import sparta_b32ed70c39
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_fd38795371(json_data,user_obj):
...
def sparta_7bebf3ecce(message,typeCase=0,companyName=None):
	...
	if E.count()>0:
		...
		A.send()
def sparta_aaca2e0b4a(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_b32ed70c39(model_to_dict(D));del A[G];A[_E]=naturalday(parser.parse(str(A[_E])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_b32ed70c39(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_86b2490d6e(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_b32ed70c39(model_to_dict(L));A[_E]=naturalday(parser.parse(str(A[_E])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_a115c02243(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_7bebf3ecce(F,A.type_ticket,None)
...
def sparta_a7795bc194(json_data,user_obj):
	...
	return{_A:1}
def sparta_d710d1e61d(json_data):
	...
	return{_A:1}
def sparta_0535c4f863(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}