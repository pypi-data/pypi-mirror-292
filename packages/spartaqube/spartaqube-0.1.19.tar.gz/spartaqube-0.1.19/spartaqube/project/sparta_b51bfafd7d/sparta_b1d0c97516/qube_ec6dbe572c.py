...
from project.sparta_b51bfafd7d.sparta_b1d0c97516 import qube_b62d229712 as qube_b62d229712
...
def sparta_9c595e6ce0(user_obj):
	A=qube_5b0b590e98.sparta_e0dadb187d(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_8e0d8672dd(json_data,user_obj):
	D=user_obj;E=sparta_9c595e6ce0(D)
...
def sparta_161f6087c4():return{_A:1,'available_engines':qube_f1bd2a17cc.sparta_161f6087c4()}
def sparta_b72a5fecf7(json_data,user_obj):
	...
	return{_A:1}
def sparta_994eda923f(json_data):
...
def sparta_3520202e7f(json_data):
...
def sparta_e45858c8db(json_data,user_obj):A=json_data;B=datetime.now().astimezone(UTC);C=str(uuid.uuid4());D=DBConnector.objects.create(connector_id=C,host=A[_S],port=A[_T],user=A[_U],password_e=qube_f1bd2a17cc.sparta_66a85ab649(A[_j]),database=A[_V],oracle_service_name=A[_W],keyspace=A[_X],library_arctic=A[_Y],database_path=A[_Z],csv_path=A[_a],csv_delimiter=A[_b],read_only=A[_c],json_url=A[_d],socket_url=A[_e],redis_db=A[_f],token=A[_k],organization=A[_l],lib_dir=A[_m],driver=A[_n],trusted_connection=A[_o],dynamic_inputs=json.dumps(A[_I]),py_code_processing=A[_g],db_engine=A[_N],name=A[_J],description=A[_K],date_created=B,last_update=B,last_date_used=B);E=ShareRights.objects.create(is_admin=_C,has_write_rights=_C,has_reshare_rights=_C,last_update=B);DBConnectorUserShared.objects.create(db_connector=D,user=user_obj,date_created=B,share_rights=E,is_owner=_C);return{_A:1}
def sparta_f819b7b864(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_9c595e6ce0(C)
...
def sparta_99d0cc96ee(json_data,user_obj):
	C=json_data[_H];A=sparta_a8e73fb926(C,user_obj)
...
def sparta_f93e5f09c5(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_p];D=[];E=sparta_a8e73fb926(H,user_obj)
...
def sparta_472519c709(json_data,db_connector_obj):
...
def sparta_8abcf1a90f(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,_B);F=int(A.get(_p,'0'))==1;B=sparta_a8e73fb926(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_472519c709(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			H=A[_y]
	...
	return{_A:1,_E:sparta_62e6758d5a(E)}
def sparta_4ddaaf51e5(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_p,_D);K=A.get(_N,_B);B=sparta_a8e73fb926(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_472519c709(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
...
def sparta_7d27ec8469(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_8e246daa58(json_data,user_obj):
	A=json_data;print('json_data load_table_preview_explorer');print(A);D=A[_H];F=A.get(_O,_B);G=int(A.get(_p,'0'))==1;B=sparta_a8e73fb926(D,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_472519c709(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_E:sparta_62e6758d5a(E)}
def sparta_e6094a995b(json_data,user_obj):
	A=json_data;J=A[_L];D=_B
	if J:D=A[_s];D=qube_6640f264c5.sparta_2bbe3d6001(D)
...
def sparta_b02a8c079f(json_data,user_obj):
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if J.is_admin or J.has_write_rights:
				M=B[_L];C=_B
				if M:C=B[_s];C=qube_6640f264c5.sparta_2bbe3d6001(C)
	...
	return{_A:1}
def sparta_697a82f477(json_data,user_obj):0
def sparta_6f5bce51d5(json_data,user_obj):
	D=user_obj;F=sparta_9c595e6ce0(D)
...
def exec_notebook_and_get_workspace_variables(full_code,data_source_variables,workspace_variables):
	...
	return B
def sparta_9034f8a5eb(json_data,user_obj):
			...
			if N.count()>0:
				...
				if b:0
					...
					for B in A.data_source_list:
						...
						if I:H.append(B[Z])
							...
							if _I in B:
								c=B[_H];Q=sparta_a8e73fb926(c,P);R=[]
				...
				if len(H)>0:
					if E is not _B:
						...
						for B in A.data_source_list:
							I=B[Y]
							if I:W=B[Z];i=h[W];B[_E]=sparta_62e6758d5a(sparta_a7bc85aeac(i,variable_name=W))
...
def sparta_0acc873bbe(json_data,user_obj):
	...
	return{_A:1}
def sparta_d60c9013e1(session_id):
	...
	for(A,F)in E.items():
		...
		if C is not _B:
			...
			if A==_R:B[A]=D
			else:B[A]=sparta_62e6758d5a(sparta_a7bc85aeac(D,variable_name=A))
	return B
def sparta_6c11408da8(session_id):A=sparta_d60c9013e1(session_id);B=sparta_a1f42857f0(b_return_type_id=_C);C=json.loads(A[_AS])[_E][0][0];D=[A for A in B if A['ID']==C][0][_AT];return{_A:1,_AU:A,_AV:A.get(_R,dict()),_Q:D}
def sparta_01cafd8e5d(user_obj,session_id,widget_id):
	...
	if E.count()>0:
		...
		if not C:
			...
			if H.count()>0:C=_C
	if C:B=A.chart_params;B[_R]=json.loads(B[_R]);B[_AA]=json.loads(B[_E])[_AA];F=sparta_d60c9013e1(session_id);I=sparta_a1f42857f0(b_return_type_id=_C);J=json.loads(F[_AS])[_E][0][0];K=[A for A in I if A['ID']==J][0][_AT];L=F.get(_R,dict());B[_R].update(L);return{_A:1,_AU:F,_AV:B,_Q:K}
	else:return{_A:-1,_F:'You do not have access to this template'}
def sparta_b89ebceed6(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_9c595e6ce0(K)
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_6640f264c5.sparta_82ac30d692(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_2c9d62109d(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_M:A}
					...
					try:
						if qube_6640f264c5.sparta_2bbe3d6001(F)==A.widget_password_e:return{_A:1,_M:A}
						else:return{_A:3,_F:_A9,_M:A}
					except:return{_A:3,_F:_A9,_M:A}
			elif C.is_authenticated:
				G=sparta_9c595e6ce0(C)
	...
	return{_A:-1}
def sparta_fde8f47e5c(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_9c595e6ce0(C)
	...
	return{_A:1,_AC:_D}
def sparta_b41fa7b98b(plot_db_chart_obj):
	...
	return C
def sparta_a589c2ad58(json_data,user_obj):
	B=user_obj;D=sparta_9c595e6ce0(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_L];C=_B
				if N:C=B[_s];C=qube_6640f264c5.sparta_2bbe3d6001(C)
...
def sparta_3c6d563159(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_L];C=_B
				if K:C=B[_s];C=qube_6640f264c5.sparta_2bbe3d6001(C)
	...
	return{_A:1}
def sparta_fd06928009(json_data,user_obj):
	...
	return{_A:1}
def sparta_1478205f22(user_obj):
	B=user_obj;C=sparta_9c595e6ce0(B)
	...
	return E
def sparta_0acd60077a(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_fde8f47e5c(A['widget_id'],B);D=C[_AC]
	if D:E=C[_M];A[_G]=E.plot_chart_id;F=sparta_9034f8a5eb(A,B);return{_A:1,_E:[A[_E]for A in F[_A8]]}
	return{_A:-1}
def sparta_5bc4c0da62(json_data,user_obj):
	B='Error quantstats';A=json_data;C=A['service']
	if C=='quantstats':
		try:return qube_b62d229712.sparta_3ee122834a(A,user_obj)
		except Exception as D:print(B);print(traceback.format_exc());print(B);return{_A:-1,_F:str(D)}
	return{_A:1}
def sparta_dbe77047f3(json_data,user_obj):
...
def sparta_a36768db4f(json_data,user_obj):
