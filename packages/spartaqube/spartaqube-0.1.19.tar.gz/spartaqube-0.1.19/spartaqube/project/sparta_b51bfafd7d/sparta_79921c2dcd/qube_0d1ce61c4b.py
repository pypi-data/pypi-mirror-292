...
from project.sparta_f25721a1fe.sparta_f8f3ec6af7.qube_780496a161 import sparta_04ee367c53
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_f78f59be3f(json_data,user_obj):
...
def sparta_67ef01deed(message,typeCase=0,companyName=None):
	...
	if E.count()>0:
		...
		A.send()
def sparta_995ac1482d(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_04ee367c53(model_to_dict(D));del A[G];A[_E]=naturalday(parser.parse(str(A[_E])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_04ee367c53(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_733c0227f5(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_04ee367c53(model_to_dict(L));A[_E]=naturalday(parser.parse(str(A[_E])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_4e54b70755(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_67ef01deed(F,A.type_ticket,None)
...
def sparta_b5b75e9bd3(json_data,user_obj):
	...
	return{_A:1}
def sparta_afe5b2d211(json_data):
	...
	return{_A:1}
def sparta_832d72dea6(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}