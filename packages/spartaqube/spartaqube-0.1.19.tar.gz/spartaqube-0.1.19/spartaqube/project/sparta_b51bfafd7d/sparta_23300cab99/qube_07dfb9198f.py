...
def sparta_724bd6d7ef(json_data,user_obj):
	...
	for A in K:
		B=sparta_04ee367c53(model_to_dict(A));Q=int(A.typeObject)
		if not A.is_seen:print('Add 1');E+=1
		for R in P:B.pop(R,None)
		C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.date_created.astimezone(UTC);B[N]=G;B[O]=sparta_1496d7113f(H)
		if Q==0:0
	...
	for A in L:
		...
		B=sparta_04ee367c53(model_to_dict(A));C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.dateCreated.astimezone(UTC);B[N]=G;B['type_object']=-2;B[O]=sparta_1496d7113f(H);F.append(B)
...
def sparta_1496d7113f(dateCreated):
	...
	return str(A)
def sparta_915f689e26(json_data,user_obj):
