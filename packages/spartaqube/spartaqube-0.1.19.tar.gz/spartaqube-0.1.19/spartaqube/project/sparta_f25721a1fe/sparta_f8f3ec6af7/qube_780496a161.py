...
from spartaqube_app.secrets import sparta_7eb2a2314c
from project.sparta_b51bfafd7d.sparta_7ea6b9fb1e.qube_5f87e9f5c0 import sparta_1419503977
import pytz
UTC=pytz.utc
class dotdict(dict):__getattr__=dict.get;__setattr__=dict.__setitem__;__delattr__=dict.__delitem__
def sparta_04ee367c53(appViewsModels):
	A=appViewsModels
	...
	return A
def sparta_94a7e3f901(thisText):A=os.path.dirname(os.path.dirname(os.path.abspath(__file__)));A=A+str('/log/log.txt');B=open(A,'a');B.write(thisText);B.writelines('\n');B.close()
def sparta_44c3df292c(request):A=request;return{'appName':'Project','user':A.user,'ip_address':A.META['REMOTE_ADDR']}
def sparta_457bc0ac0d():return conf_settings.PLATFORM
def sparta_cd466d20aa():
...
def sparta_9d84973003(request):
	...
	try:
		...
		if F.count()==0:AppVersioning.objects.create(last_check_date=E);A=_A
			...
			D=F[0];G=D.last_check_date;H=E-G;I=sparta_1419503977();J=D.last_available_version_pip
			if not I==J:A=_A
...
def sparta_132f4604cb(captcha):
	D='errorMsg';B='res';A=captcha
	try:
		if A is not _C:
			if len(A)>0:
				E=sparta_7eb2a2314c()['CAPTCHA_SECRET_KEY'];F=f"https://www.google.com/recaptcha/api/siteverify?secret={E}&response={A}";C=requests.get(F)
				if int(C.status_code)==200:
					G=json.loads(C.text)
					if G['success']:return{B:1}
	except Exception as H:return{B:-1,D:str(H)}
	return{B:-1,D:'Invalid captcha'}
def sparta_06418e8acc(password):
...
def sparta_c9380a8e8b(user):
	...
	A[_D]=sparta_cd466d20aa();return A
def sparta_38b2e9f294(user):A=dict();A[_D]=sparta_cd466d20aa();return A
def sparta_7bdcce02d7():
	try:socket.create_connection(('1.1.1.1',53));return _A
	except OSError:pass
	return _B
def sparta_86799690df():A=socket.gethostname();B=socket.gethostbyname(A);return A,B