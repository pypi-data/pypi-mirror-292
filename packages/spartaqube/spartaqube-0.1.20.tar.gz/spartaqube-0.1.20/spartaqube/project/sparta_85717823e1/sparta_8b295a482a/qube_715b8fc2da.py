...
from project.sparta_85717823e1.sparta_45d09a2c36 import qube_4c2bbe8319 as qube_4c2bbe8319
def sparta_9904525201():return str(uuid.uuid4())
def sparta_db7b421b16(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_814a3af342(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
	except Exception as B:return{_A:-1,_C:str(B)}
def sparta_3eea038bd6(path):
	with open(path,'a'):os.utime(path,None)
def sparta_4c2af28da0(json_data,userObj):
	A=json_data;print('CREATE RESOURCE');print(A);C=A[_B];D=sparta_814a3af342(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_3eea038bd6(B)
	...
	return{_A:1}
def sparta_b08a94a46a(json_data,userObj):
	...
	return{_A:1}
def sparta_4e049ea40d(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_814a3af342(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_c9d839e23a(json_data,userObj):A=json_data[_B];B=qube_4c2bbe8319.sparta_ac940d8c33(A);C={_A:1,'folderStructure':B};return C
def sparta_cf604fb719(json_data,userObj):
	A=json_data
	try:
...
def sparta_597c061b61(json_data,userObj):
	...
	return{_A:1}
def sparta_512728f508(json_data,userObj):
	...
	return{_A:1}
def sparta_fcb1eb8e7e(json_data,userObj):
...
def sparta_4458462019(json_data,userObj):
	...
	return{_A:-1}
def sparta_3ce8a6bfb3(json_data,userObj):
