...
from project.sparta_cc504a7958.sparta_154282b52b.qube_f629cfbf67 import sparta_988cda2896,sparta_c0f784926f,sparta_8e603cc550
from project.sparta_85717823e1.sparta_3811b5fb11.qube_851830bece import sparta_5dabb3175c
def sparta_55d6b48d22(json_data,user_obj):
...
def sparta_6bc0f93aba(message,typeCase=0,companyName=None):
	...
	if C.count()>0:
		...
		A.send()
def sparta_560ea065b6(json_data,userObj):
	...
	return B
def sparta_c3ccbba259(json_data,userObj):
	...
	return A
def sparta_9e3e02f683(json_data,userObj):
	D=json_data;F=D['old_spartaqube_code'];G=D['new_spartaqube_code']
	if not sparta_5dabb3175c(F):return{_A:-1,_B:'Invalid current code'}
	...
	return{_A:1}
def sparta_1b38df748c(json_data,userObj):A=userObj;C=json_data['base64image'];K=hashlib.sha256((str(A.id)+'_'+A.email+str(datetime.now())).encode(_D)).hexdigest();D,E=C.split(';base64,');F,L=D.split('/');G=F.split(':')[-1];B=UserProfile.objects.get(user=A);H=datetime.now();I=Avatar.objects.create(avatar=G,image64=E,date_created=H);B.avatar=I;B.save();J={_A:1};return J
def sparta_ff68be8656(json_data,userObj):B=json_data['bDarkTheme'];A=UserProfile.objects.get(user=userObj);A.is_dark_theme=B;A.save();C={_A:1};return C
def sparta_a8b8b64713(json_data,userObj):B=json_data['theme'];A=UserProfile.objects.get(user=userObj);A.editor_theme=B;A.save();C={_A:1};return C
def sparta_a5245cc5d7():B='spartaqube-reset-password';A=B.encode(_D);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_D));return A.decode(_D)
def sparta_1d68025523(json_data):
	A=json_data;C=A[_F];E=A[_G];B=A[_H];F=A[_Q]
	if not sparta_8e603cc550(E):return{_A:-1,_B:_I}
...
def sparta_9265085dfc(json_data):
	A=json_data;E=A[_E];B=A[_F];F=A[_G];G=sparta_988cda2896(E)
	if G[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_c0f784926f(F):return{_A:-1,_B:_I}
...
def sparta_feb36c64ea(request,json_data):
	A=json_data;F=A[_E];D=A[_F];G=A[_G];H=A[_T];E=A[_H];I=A[_Q];J=sparta_988cda2896(F)
	if J[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_c0f784926f(G):return{_A:-1,_B:_I}
