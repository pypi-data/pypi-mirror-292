...
def sparta_aa7beed32b(json_data,user_obj):
	...
	for A in K:
		B=sparta_4758455814(model_to_dict(A));Q=int(A.typeObject)
		if not A.is_seen:print('Add 1');E+=1
		for R in P:B.pop(R,None)
		C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.date_created.astimezone(UTC);B[N]=G;B[O]=sparta_9d67645986(H)
		if Q==0:0
	...
	for A in L:
		...
		B=sparta_4758455814(model_to_dict(A));C=User.objects.get(id=A.user_from.id);G=C.first_name+' '+C.last_name;H=A.dateCreated.astimezone(UTC);B[N]=G;B['type_object']=-2;B[O]=sparta_9d67645986(H);F.append(B)
...
def sparta_9d67645986(dateCreated):
	...
	return str(A)
def sparta_797c4a4be6(json_data,user_obj):
