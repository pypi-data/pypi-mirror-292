...
from project.sparta_85717823e1.sparta_2d9e52fff7 import qube_cdfa3c0f9e as qube_cdfa3c0f9e
...
def sparta_ec6b59ed97(user_obj):
	A=qube_fc7cdf5312.sparta_28ef6c65a9(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_5cce3961bb(json_data,user_obj):
	D=user_obj;E=sparta_ec6b59ed97(D)
	...
	if B.count()>0:
		...
		for G in B:
			...
			F.append({_H:A.connector_id,_S:A.host,_T:A.port,_U:A.user,_V:A.database,_W:A.oracle_service_name,_X:A.keyspace,_Y:A.library_arctic,_Z:A.database_path,_a:A.csv_path,_b:A.csv_delimiter,_c:A.read_only,_d:A.json_url,_e:A.socket_url,_f:A.redis_db,_I:H,_g:A.py_code_processing,_N:A.db_engine,_J:A.name,_K:A.description,_x:G.is_owner})
	return{_A:1,'db_connectors':F}
def sparta_76236a9aad():return{_A:1,'available_engines':qube_35a872c584.sparta_76236a9aad()}
def sparta_28955dab04(json_data,user_obj):
	...
	return{_A:1}
def sparta_9c3361382c(json_data):
...
def sparta_8b32ca1127(json_data):
...
def sparta_ef3b0cb4a4(json_data,user_obj):A=json_data;B=datetime.now().astimezone(UTC);C=str(uuid.uuid4());D=DBConnector.objects.create(connector_id=C,host=A[_S],port=A[_T],user=A[_U],password_e=qube_35a872c584.sparta_e2554bdb04(A[_j]),database=A[_V],oracle_service_name=A[_W],keyspace=A[_X],library_arctic=A[_Y],database_path=A[_Z],csv_path=A[_a],csv_delimiter=A[_b],read_only=A[_c],json_url=A[_d],socket_url=A[_e],redis_db=A[_f],token=A[_k],organization=A[_l],lib_dir=A[_m],driver=A[_n],trusted_connection=A[_o],dynamic_inputs=json.dumps(A[_I]),py_code_processing=A[_g],db_engine=A[_N],name=A[_J],description=A[_K],date_created=B,last_update=B,last_date_used=B);E=ShareRights.objects.create(is_admin=_C,has_write_rights=_C,has_reshare_rights=_C,last_update=B);DBConnectorUserShared.objects.create(db_connector=D,user=user_obj,date_created=B,share_rights=E,is_owner=_C);return{_A:1}
def sparta_d571b42f00(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_ec6b59ed97(C)
...
def sparta_ae02565b80(json_data,user_obj):
	C=json_data[_H];A=sparta_99f2017081(C,user_obj)
...
def sparta_04c450efa8(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_p];D=[];E=sparta_99f2017081(H,user_obj)
...
def sparta_6385f7155b(json_data,db_connector_obj):
...
def sparta_53a4bfc298(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,_B);F=int(A.get(_p,'0'))==1;B=sparta_99f2017081(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_6385f7155b(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			H=A[_y]
	...
	return{_A:1,_E:sparta_5b3f451ffa(E)}
def sparta_f9c85ad1ab(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_p,_D);K=A.get(_N,_B);B=sparta_99f2017081(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_6385f7155b(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
	...
	J=E.describe();return{_A:1,_E:J.to_json(orient=_z)}
def sparta_c294948269(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_61ccca3a7a(json_data,user_obj):
	A=json_data;print('json_data load_table_preview_explorer');print(A);D=A[_H];F=A.get(_O,_B);G=int(A.get(_p,'0'))==1;B=sparta_99f2017081(D,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_6385f7155b(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_E:sparta_5b3f451ffa(E)}
def sparta_77753cda7d(json_data,user_obj):
	A=json_data;J=A[_L];D=_B
	if J:D=A[_s];D=qube_b69aa2c416.sparta_361a8ef888(D)
	K=A[_A0];L=str(uuid.uuid4());B=datetime.now().astimezone(UTC);M=CodeEditorNotebook.objects.create(notebook_id=L,cells=K,date_created=B,last_update=B);E=str(uuid.uuid4());F=A[_A1];G=A['is_gui_plot']
	if G:F=_C
	C=A['plotSlug']
	if len(C)==0:C=A[_t]
	H=slugify(C);C=H;I=1
	while PlotDBChart.objects.filter(slug=C).exists():C=f"{H}-{I}";I+=1
	N=PlotDBChart.objects.create(plot_chart_id=E,type_chart=A[_AJ],name=A[_t],slug=C,description=A[_A2],is_expose_widget=A[_A3],is_public_widget=A[_u],is_static_data=F,has_widget_password=A[_L],widget_password_e=D,data_source_list=A[_AK],chart_params=A[_AL],chart_config=A[_AM],code_editor_notebook=M,is_created_from_api=G,date_created=B,last_update=B,last_date_used=B);O=ShareRights.objects.create(is_admin=_C,has_write_rights=_C,has_reshare_rights=_C,last_update=B);PlotDBChartShared.objects.create(plot_db_chart=N,user=user_obj,share_rights=O,is_owner=_C,date_created=B);return{_A:1,_G:E}
def sparta_fff87288de(json_data,user_obj):
	E=user_obj;B=json_data;K=B[_G];F=PlotDBChart.objects.filter(plot_chart_id=K,is_delete=_D).all()
	if F.count()>0:
		A=F[F.count()-1];I=sparta_ec6b59ed97(E)
		...
		if G.count()>0:
			...
			if J.is_admin or J.has_write_rights:
				M=B[_L];C=_B
				if M:C=B[_s];C=qube_b69aa2c416.sparta_361a8ef888(C)
				H=datetime.now().astimezone(UTC);A.type_chart=B[_AJ];A.name=B[_t];A.description=B[_A2];A.is_expose_widget=B[_A3];A.is_static_data=B[_A1];A.has_widget_password=B[_L];A.is_public_widget=B[_u];A.widget_password_e=C;A.data_source_list=B[_AK];A.chart_params=B[_AL];A.chart_config=B[_AM];A.last_update=H;A.last_date_used=H;A.save();D=A.code_editor_notebook
				if D is not _B:D.cells=B[_A0];D.last_update=H;D.save()
	return{_A:1}
def sparta_495249dc73(json_data,user_obj):0
def sparta_7d1947c453(json_data,user_obj):
	D=user_obj;F=sparta_ec6b59ed97(D)
	...
	for E in A:
		...
		G.append({_G:B.plot_chart_id,_Q:B.type_chart,_J:B.name,'slug':B.slug,_K:B.description,_A4:B.is_expose_widget,_A5:B.is_static_data,_v:B.has_widget_password,_w:B.is_public_widget,_x:E.is_owner,_AN:J.has_write_rights,_A6:H,_A7:I})
	return{_A:1,_AO:G}
def exec_notebook_and_get_workspace_variables(full_code,data_source_variables,workspace_variables):
	...
	return B
def sparta_860a747f8a(json_data,user_obj):
			...
			if N.count()>0:
				...
				if b:0
					...
					for B in A.data_source_list:
						...
						if I:H.append(B[Z])
							...
							if _I in B:
								c=B[_H];Q=sparta_99f2017081(c,P);R=[]
				...
				if len(H)>0:
					if E is not _B:
						...
						for B in A.data_source_list:
							I=B[Y]
							if I:W=B[Z];i=h[W];B[_E]=sparta_5b3f451ffa(sparta_9d9c760226(i,variable_name=W))
				def j(s):s=s.lower();A='-_.() %s%s'%(re.escape('/'),re.escape('\\'));B=re.sub('[^A-Za-z0-9%s]'%A,'_',s);return B
				return{_A:1,_G:F,_Q:A.type_chart,_J:A.name,'slug':A.slug,'name_file':j(A.name),_K:A.description,_A4:A.is_expose_widget,_A5:A.is_static_data,_v:A.has_widget_password,_w:A.is_public_widget,_A8:A.data_source_list,_AP:A.chart_params,_AQ:A.chart_config,_AR:E}
		else:return{_A:-1,_F:_A9}
	return{_A:-1,_F:'Unexpected error, please try again'}
def sparta_ef01b65851(json_data,user_obj):
	...
	return{_A:1}
def sparta_5a89dd23df(session_id):
	...
	for(A,F)in E.items():
		...
		if C is not _B:
			...
			if A==_R:B[A]=D
			else:B[A]=sparta_5b3f451ffa(sparta_9d9c760226(D,variable_name=A))
	return B
def sparta_4590a27006(session_id):A=sparta_5a89dd23df(session_id);B=sparta_25920ddfba(b_return_type_id=_C);C=json.loads(A[_AS])[_E][0][0];D=[A for A in B if A['ID']==C][0][_AT];return{_A:1,_AU:A,_AV:A.get(_R,dict()),_Q:D}
def sparta_5f5210e305(user_obj,session_id,widget_id):
	...
	if E.count()>0:
		...
		if not C:
			...
			if H.count()>0:C=_C
	if C:B=A.chart_params;B[_R]=json.loads(B[_R]);B[_AA]=json.loads(B[_E])[_AA];F=sparta_5a89dd23df(session_id);I=sparta_25920ddfba(b_return_type_id=_C);J=json.loads(F[_AS])[_E][0][0];K=[A for A in I if A['ID']==J][0][_AT];L=F.get(_R,dict());B[_R].update(L);return{_A:1,_AU:F,_AV:B,_Q:K}
	else:return{_A:-1,_F:'You do not have access to this template'}
def sparta_950634e93d(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_ec6b59ed97(K)
			...
			if M.count()>0:
				...
				if U.count()>0:
					...
					for G in A.data_source_list:C=G[J];G[_E]=F[C].to_json(orient=_z)
				return{_A:1,_G:S,_Q:A.type_chart,_J:A.name,_K:A.description,_A4:A.is_expose_widget,_A5:A.is_static_data,_v:A.has_widget_password,_w:A.is_public_widget,_A8:A.data_source_list,_AP:A.chart_params,_AQ:A.chart_config,_AR:_B}
	except Exception as W:print('Error exception > '+str(W));return{_A:-1,_F:str(W)}
def sparta_98f691a587(json_data,user_obj):
	A=user_obj;G=json_data[_G];B=PlotDBChart.objects.filter(plot_chart_id=G,is_delete=_D).all()
	if B.count()>0:
		C=B[B.count()-1];E=sparta_ec6b59ed97(A)
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_b69aa2c416.sparta_590391beea(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_36993de8df(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_M:A}
					...
					try:
						if qube_b69aa2c416.sparta_361a8ef888(F)==A.widget_password_e:return{_A:1,_M:A}
						else:return{_A:3,_F:_A9,_M:A}
					except:return{_A:3,_F:_A9,_M:A}
			elif C.is_authenticated:
				G=sparta_ec6b59ed97(C)
	...
	return{_A:-1}
def sparta_db9f806d6d(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_ec6b59ed97(C)
		...
		if E.count()>0:I=E[0];B=I.plot_db_chart;return{_A:1,_AC:_C,_M:B}
	return{_A:1,_AC:_D}
def sparta_c635cf83c4(plot_db_chart_obj):
	...
	return C
def sparta_9cb1d0cf33(json_data,user_obj):
	B=user_obj;D=sparta_ec6b59ed97(B)
	...
	for C in E:
		...
		F.append({_G:A.plot_chart_id,_Q:A.type_chart,_v:A.has_widget_password,_w:A.is_public_widget,_J:A.name,'slug':A.slug,_K:A.description,_x:C.is_owner,_AN:I.has_write_rights,_A6:G,_A7:H})
	return{_A:1,_AO:F}
def sparta_6d900a9771(json_data,user_obj):
	E=user_obj;B=json_data;K=B[_G];L=B['isCalledFromLibrary'];F=PlotDBChart.objects.filter(plot_chart_id=K,is_delete=_D).all()
	if F.count()>0:
		A=F[F.count()-1];H=sparta_ec6b59ed97(E)
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_L];C=_B
				if N:C=B[_s];C=qube_b69aa2c416.sparta_361a8ef888(C)
				J=datetime.now().astimezone(UTC);A.has_widget_password=B[_L];A.widget_password_e=C;A.name=B[_t];A.plotDes=B[_A2];A.is_expose_widget=B[_A3];A.is_public_widget=B[_u];A.is_static_data=B[_A1];A.last_update=J;A.save()
				if L:0
				else:
					D=A.code_editor_notebook
					if D is not _B:D.cells=B[_A0];D.last_update=J;D.save()
	return{_A:1}
def sparta_9e2f4f19a8(json_data,user_obj):
	D=user_obj;B=json_data;I=B[_G];E=PlotDBChart.objects.filter(plot_chart_id=I,is_delete=_D).all()
	if E.count()>0:
		A=E[E.count()-1];G=sparta_ec6b59ed97(D)
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_L];C=_B
				if K:C=B[_s];C=qube_b69aa2c416.sparta_361a8ef888(C)
	...
	return{_A:1}
def sparta_50c0aeba85(json_data,user_obj):
	...
	return{_A:1}
def sparta_07cfe6112b(user_obj):
	B=user_obj;C=sparta_ec6b59ed97(B)
	...
	for F in D:
		...
		E.append({'id':A.plot_chart_id,_J:A.name,_K:A.description,_A6:G,_A7:H})
	return E
def sparta_ea703e8444(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_db9f806d6d(A['widget_id'],B);D=C[_AC]
	if D:E=C[_M];A[_G]=E.plot_chart_id;F=sparta_860a747f8a(A,B);return{_A:1,_E:[A[_E]for A in F[_A8]]}
	return{_A:-1}
def sparta_82871b242f(json_data,user_obj):
	B='Error quantstats';A=json_data;C=A['service']
	if C=='quantstats':
		try:return qube_cdfa3c0f9e.sparta_a30a940402(A,user_obj)
		except Exception as D:print(B);print(traceback.format_exc());print(B);return{_A:-1,_F:str(D)}
	return{_A:1}
def sparta_bf08818d0f(json_data,user_obj):
	B=json_data;import quantstats as K;B=B[_E];G=B[_AW];L=B[_AX];H=B['columnsX'];I=B[_r];F=L;C=I
...
def sparta_92d20be9c0(json_data,user_obj):
	N='Salary';A=json_data;A=A[_E];O=A[_AW];P=A[_AX];G=A['columnsX'];H=A[_r];C=P;I=H
	...
	def M(this_df,level=0,previous_index_list=_B):
		...
		if A==F-1:
			for H in J:L.append({_E:[0]*len(K),_E:C[H].tolist(),_AD:list(C.index.get_level_values(A)),'hierarchy':D,_AB:H,'label':D[-1]})
		elif A<F-1:
	...
	M(B);print('chart_data');return{_A:1,_AA:L,_AD:K}