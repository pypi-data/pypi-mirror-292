...
from project.sparta_cc504a7958.sparta_154282b52b.qube_f629cfbf67 import sparta_4758455814
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_bdbee64fc1(json_data,user_obj):
...
def sparta_68f9475f30(message,typeCase=0,companyName=None):
	...
	if E.count()>0:
		...
		A.send()
def sparta_114e0c524c(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_4758455814(model_to_dict(D));del A[G];A[_E]=naturalday(parser.parse(str(A[_E])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_4758455814(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_d9ff2f7a54(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_4758455814(model_to_dict(L));A[_E]=naturalday(parser.parse(str(A[_E])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_351d32115d(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_68f9475f30(F,A.type_ticket,None)
...
def sparta_8315768ba0(json_data,user_obj):
	...
	return{_A:1}
def sparta_2dee89d2bb(json_data):
	...
	return{_A:1}
def sparta_eb0bd94d5f(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}