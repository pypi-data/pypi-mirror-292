...
from project.sparta_85717823e1.sparta_28b9632596 import qube_054975c13d as qube_054975c13d
from project.sparta_85717823e1.sparta_02bf41015f import qube_ba5a966af5 as qube_ba5a966af5
from project.sparta_85717823e1.sparta_d090c9de8e.qube_b0e81af9c4 import Email as Email
def sparta_7190da8f89(function):
	def A(request,*E,**C):
		A=request;B=_B
		...
		if not B:
			D=C.get(_I)
			if D is not None:F=qube_ba5a966af5.sparta_730bfe1dc3(D);login(A,F)
		return function(A,*E,**C)
	return A
def sparta_3d3d3187e4(function):
	def A(request,*B,**C):
		A=request
	...
	return A
def sparta_39f820a6f5(function):
	def A(request,*B,**C):
	...
	return A
def sparta_839e67a3e8(function):
	def A(request,*D,**E):
		A=request;B=_C
		try:
			F=json.loads(A.body);G=json.loads(F[_J]);H=G[_I];C=qube_ba5a966af5.sparta_730bfe1dc3(H)
			if C is not None:B=_B;A.user=C
		except Exception as I:print('exception pip auth');print(I)
		if B:return function(A,*D,**E)
		else:return HttpResponseRedirect(reverse(_G))
	return A
def sparta_5dabb3175c(code):
	try:
...
def sparta_0ae2dbe322():A=socket.gethostname();B=socket.gethostbyname(A);return B
def sparta_bc4aeed1fc(json_data):
	D='ip_addr';A=json_data;del A[_K];del A[_L]
	try:A[D]=sparta_0ae2dbe322()
	except:A[D]=-1
...
def sparta_94e934342e():return conf_settings.IS_GUEST_CODE_REQUIRED
def sparta_8979b9f8df(guest_code):
	if GuestCodeGlobal.objects.filter(guest_id=guest_code,is_active=_B).count()>0:return _B
	return _C
def sparta_a20e3b2de4(guest_code,user_obj):
	...
	return _C
def sparta_c05c71abee(user):
	A=UserProfile.objects.filter(user=user)
	if A.count()==1:return A[0].is_banned
	else:return _C
def sparta_15ce6c4fd1(email,captcha):
	D=sparta_988cda2896(captcha)
	if D[_A]!=1:return{_A:-1,_D:_H}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_M}
	A=B[0];C=str(uuid.uuid4());A.token_reset_password=C;A.save();sparta_b3fa15dc53(A.user,C);return{_A:1}
def sparta_b3fa15dc53(user_obj,token_reset_password):B=user_obj;A=Email(B.username,[B.email],'Reset Password','Reset Password Message');A.addOneRow('Reset code','Copy the following code to reset your password');A.addSpaceSeparator();A.addOneRow(token_reset_password);A.send()
def sparta_2b4fe2e074(captcha,token,email,password):
	D=sparta_988cda2896(captcha)
	if D[_A]!=1:return{_A:-1,_D:_H}
	B=UserProfile.objects.filter(user__username=email)
	if B.count()==0:return{_A:-1,_D:_M}
