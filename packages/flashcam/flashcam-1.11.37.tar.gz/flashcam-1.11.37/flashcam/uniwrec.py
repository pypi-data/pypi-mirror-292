...
"""
               ...
               # ======================================================== IMSHOW
                ...
                if (
                    (frame is not None)
                    and (rpi_name != "")
                    and ((key == 1048608) or (key == ord(" "))) and  not CTRL  # PAUSE
                ):
                ...
                if (
                    (frame is not None)
                    and (rpi_name != "")
                    and ((key == ord("s")) or (key == ord("S")) and not CTRL) #save jpg/png here
                ):
                ...
                if (frame is not None) and (rpi_name != ""):
                    if key == ord("C") and not CTRL: # red cross save
                        ...
                        with open(os.path.expanduser(FILE_REDCROSS), "w") as f:
                            f.write(f"{cross[0]}\n{cross[1]}\n")
                            cross = None
                    if key == ord("c")  and not CTRL: # red cross  load
                        if cross is None:
                            FILE_REDCROSS = get_FILE_REDCROSS( videodev )
                            cross = [0, 0]
                ...
                if not (cross is None):
                    if ( (frame is not None) and (rpi_name != "")
                    and (key == ord("h") and not CTRL) #  not cross left
                    ):  # <
                        cross[0] -= 4
                    if ( (frame is not None) and (rpi_name != "")
                    and (key == ord("j") and not CTRL) # not cross down
                    ):  # v
                        cross[1] += 4
                    if ( (frame is not None) and (rpi_name != "")
                    and (key == ord("k") and not CTRL) # not cross up
                    ):  # ^
                        cross[1] -= 4
                    if (  (frame is not None) and (rpi_name != "")
                    and (key == ord("l") and not CTRL) # not cross right
                    ):  # >
                        cross[0] += 4
                    ...
                    and (key == ord("K") and not CTRL)# not cross up
                        ...
                        cross[1] -= 17
                    if (   (frame is not None) and (rpi_name != "")
                    and (key == ord("L") and not CTRL) # not cross righht
                    ):  # >
                        cross[0] += 17
                    #print(frame.shape)
                ...
                if (
                    (frame is not None)
                    and (rpi_name != "")
                    and ((key == ord("t")) or (key == ord("T")))  and not (CTRL) # TRACK q1
                ...
                # --- 2nd 't' press :  is before demand=True, sets allow true
                    ...
                    (frame is not None)
                    and (rpi_name != "")
                    and ((key == ord("t")) or (key == ord("T")) and not (CTRL)) # TRACK q2
                ...
                if ( (frame is not None) and (rpi_name != "")
                     and (key == ord("O")) # break expo optimizatioon - ??
                    ):
                ...
                if ( (frame is not None) and (rpi_name != "")
                     and (key == ord("E")) and not CRTL # expo down
                    ):
                    print("e PRESSED! - ")
                    post_addr = videodev.replace("/video", "/cross")
                ...
                if  (  (frame is not None)
                       and (rpi_name != "") and (key == ord("G") ) # gamma down
                     ):
                ...
                if (
                        (frame is not None) and (rpi_name != "")
                    and (key == ord("p") and not CTRL)  # fixed:FIXED
                ):
                    print("p PRESSED! - ")
                    post_addr = videodev.replace("/video", "/cross")
                ...
                if ((frame is not None) and (rpi_name != "")
                    and (key == ord("P") and not CTRL)  # live:LIVE ??(opposite to fixed)
                    ):
                    print("P PRESSED! - ")
                    post_addr = videodev.replace("/video", "/cross")
                    post_data = {"live": "LIVE"}
