...
class QuantumCircuit:
    ...
    def __str__(self):
        return to_text_diag(self.displayname_list, self.width)

    def add_single_gate(self, gate_pos: Union[int, str], display_name: tuple, *gate_info):
        """
        添加单比特门序列
        :param gate_pos:
    ...
    def add_double_gate(self, gate_pos_0:int, gate_pos_1: int, display_name: str, *gate_info):
        """
        添加两比特门序列
    ...
    def add_triple_gate(self, gate_pos: list, display_name: str, *gate_info):
        """
        添加三比特门序列
        :param gate_pos:
        :param gate_info:
    ...
    def add_measure_gate(self, gate_pos: Union[list, int], display_name: str, *gate_info):
        if isinstance(gate_pos, list):
            min_pos, max_pos = min(gate_pos), max(gate_pos)
        else:
    ...
    def append(self, gate: CommonGate):
        ...
        if gate.name in MEASURE:
            ...
            self.add_measure_gate(gate.qbit, gate.display_name, gate.name)
        ...
        # elif gate['name'] in BASE_SINGLE_GATE:
            ...
            if gate.theta is None:
                ...
                self.add_single_gate(gate.qbit, gate.display_name, gate.name)
                ...
                if isinstance(gate.theta, tuple):
                    ...
                    self.add_single_gate(gate.qbit, gate.display_name ,gate.name, gate.theta)

        elif gate.name in BASE_DOUBLE_GATE:
            if not gate.theta:
                self.add_double_gate(gate.qbit[0], gate.qbit[1], gate.display_name, gate.name)
            else:
                self.add_double_gate(gate.qbit[0], gate.qbit[1], gate.display_name,
                                     gate.name, gate.theta)

        elif gate.name in BASE_TRIPLE_GATE:
            if not gate.theta:
                self.add_triple_gate(gate.qbit, gate.display_name, gate.name)
