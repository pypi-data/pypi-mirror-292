...
class Platform(StrEnum):
	"Enum for referencing platforms such as linux or windows"
	...
	def current(cls: type[Self]) -> Self:
		"Get the platform python is running on"
		...
		match sys.platform:
			...
			case "linux" | "linux2":
				return cast(Self, cls.LINUX)

			case "darwin":
				return cast(Self, cls.MAC)

			case "os2" | "os2emx":
				return cast(Self, cls.OS2)

			case "win32" | "cygwin" | "msys":
				return cast(Self, cls.WINDOWS)
