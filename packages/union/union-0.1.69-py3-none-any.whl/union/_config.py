...
def _check_unionmeta_byoc(config: Optional[str]):
    ...
    union_env_var, _ = _get_union_config_env_var()
...
def _record_endpoint_info_config(f: Callable[P, Config]) -> Callable[P, Config]:
    """Saves the endpoint into _UNION_CONFIG."""

    @wraps(f)
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> Config:
        config = f(*args, **kwargs)
        _UNION_CONFIG.config_endpoint = config.platform.endpoint
        return config

    return wrapper


@_record_endpoint_info_config
def _get_config_obj(config: Optional[str] = None, default_to_union_semantics: bool = False) -> Config:
    ...
    if config is not None:
        ...
        if (api_value_tuple := _get_union_api_env_var()) and (api_value_tuple[0] not in ("", None)):
            _UNION_CONFIG.config_source = f"{api_value_tuple[1]} env variable"
            return _decode_union_api_key(api_value_tuple[0])
            ...
            return Config.auto(api_config_tuple[0])
